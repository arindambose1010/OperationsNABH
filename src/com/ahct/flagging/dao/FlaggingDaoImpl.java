...
public class FlaggingDaoImpl implements FlaggingDao
{
	...
	Logger logger = Logger.getLogger (FlaggingDaoImpl.class );
...
//            		System.out.println("Save not successful in flagging");
	...
	public List<FlaggingVO> getAllFlaggedCases(FlaggingVO flaggingVO)
		{
			...
			try{
			sb.append(" select ef.flagId as flagId,ecd.cmbDtlName as currentStatus,ef.caseId as caseId,ef.currentStatusOfFlag as currentStatusOfFlag,");
			...
			/*kola*/
			if((flaggingVO.getMainCatName()!=null && !flaggingVO.getMainCatName().equalsIgnoreCase("") && !flaggingVO.getMainCatName().equalsIgnoreCase("-1")) || 
					(flaggingVO.getProcName()!=null && !flaggingVO.getProcName().equalsIgnoreCase("") && !flaggingVO.getProcName().equalsIgnoreCase("-1")) || (flaggingVO.getCatName()!=null && !flaggingVO.getCatName().equalsIgnoreCase("") && !flaggingVO.getCatName().equalsIgnoreCase("-1"))) {
				
				sb.append(",EhfCaseTherapy ect ");
			}
			...
			/*kola*/
			if((flaggingVO.getMainCatName()!=null && !flaggingVO.getMainCatName().equalsIgnoreCase("") && !flaggingVO.getMainCatName().equalsIgnoreCase("-1")) && (flaggingVO.getProcName()!=null && flaggingVO.getProcName().equalsIgnoreCase("-1"))&&(flaggingVO.getCatName()!=null && flaggingVO.getCatName().equalsIgnoreCase("-1"))) {
				
				sb.append(" and ect.caseId=ef.caseId and ect.activeyn='Y' and ect.asriCatCode='"+flaggingVO.getMainCatName()+"' ");
				
			}
			if((flaggingVO.getMainCatName()!=null && !flaggingVO.getMainCatName().equalsIgnoreCase("")
					&& (flaggingVO.getCatName()!=null && !flaggingVO.getCatName().equalsIgnoreCase("") && !flaggingVO.getCatName().equalsIgnoreCase("-1")) && (flaggingVO.getProcName()!=null && !flaggingVO.getProcName().equalsIgnoreCase("") && flaggingVO.getProcName().equalsIgnoreCase("-1")))  ) {
				
				sb.append(" and ect.caseId=ef.caseId and ect.activeyn='Y' and ect.asriCatCode='"+flaggingVO.getMainCatName()+"' ");
				sb.append(" and ect.icdCatCode='"+flaggingVO.getCatName()+"' ");
			}
			if( (flaggingVO.getMainCatName()!=null && !flaggingVO.getMainCatName().equalsIgnoreCase("") && !flaggingVO.getMainCatName().equalsIgnoreCase("-1") ) && 
					(flaggingVO.getProcName()!=null && !flaggingVO.getProcName().equalsIgnoreCase("") && !flaggingVO.getProcName().equalsIgnoreCase("-1") ) && (flaggingVO.getCatName()!=null && !flaggingVO.getCatName().equalsIgnoreCase("") && !flaggingVO.getCatName().equalsIgnoreCase("-1")) ) {
				
				sb.append(" and ect.caseId=ef.caseId and ect.activeyn='Y' and ect.asriCatCode='"+flaggingVO.getMainCatName()+"' ");
				sb.append(" and ect.icdProcCode='"+flaggingVO.getProcName()+"' ");
				sb.append(" and ect.icdCatCode='"+flaggingVO.getCatName()+"' ");
			}
	...
	public Number getNoOfFlaggedCases(String lStrEmpId)
		{
			...
			sb.append(" efd.current_Status_Of_Flag NOT IN (?,?) ");
            sb.append(" and efd.case_Id=ec.case_Id and ec.case_Hosp_Code= ");
            sb.append(" (select emd1.hosp_Id from ehfm_Medco_Dtls emd1 where emd1.medco_Id=? and active_yn=?)");
            
			flaggingVOLst=genericDao.executeSQLQueryList(FlaggingVO.class,sb.toString(),new String[]{config.getString("verified_Status"),config.getString("deFlagged_Status"),lStrEmpId,"Y"});
	...
	public FlaggingVO getFlaggedCasesForColor(String caseId)
		{
			...
			FlaggingVO flaggingVO=new FlaggingVO();
			StringBuffer sb=new StringBuffer();
			sb.append(" select count(*) as FLAGGEDCASES from EhfFlagDtls efd where efd.caseId=? ");
			sb.append(" and efd.currentStatusOfFlag not in (?,?) ");
			flaggingVOLst=genericDao.executeHQLQueryList(FlaggingVO.class,sb.toString(),new String[]{caseId,config.getString("verified_Status"),config.getString("deFlagged_Status")});
			...
			if(flaggedCases.intValue()==0)
				{
					...
					sb1.append(" select count(*) as FLAGGEDCASES from EhfFlagDtls efd where efd.caseId=? ");
					sb1.append(" and efd.currentStatusOfFlag in (?,?) ");
					
					flaggingVOLst=genericDao.executeHQLQueryList(FlaggingVO.class,sb1.toString(),new String[]{caseId,config.getString("verified_Status"),config.getString("deFlagged_Status")});
	...
	public void changeMoneyCollectionFlow()
	{
		...
		logger.info("Scheduler to Redirect Money Collection Casesd ended in TS");
	}
