...
public class BioMetricRegistrationServiceImpl implements  BioMetricRegistrationService{
	...
	public List<BioMetricVo> getDispUserDetails(String userId,String dispUserId)
	{
		...
		args[0]=userId.toUpperCase();
		args[1]=dispUserId;
		query.append("select distinct a.user_Id as USERID,a.first_Name || ' ' || a.last_name as EMPLOYEENAME,a.mobile_no as MOBILE,a.EMAIL_ID as EMAILID,");
		query.append(" d.dsgn_name as DESIGNATION,e.disp_name as DEPARTMENT");
		query.append(" from EHFM_DISP_EMPLOYEE_MASTER a,EHFM_DISP_DESIGNATION d,EHFM_DISPENSARY_DTLS e,ehfm_disp_usr_mpg m");
		query.append(" where a.user_code=e.disp_code and a.desig_id=d.dsgn_id and m.disp_id=e.disp_id");
		query.append(" and a.in_service='Y'");
		query.append(" and a.user_id=?");
		query.append(" and m.user_id=?");
	...
	public String biometricDispAttendance(BioMetricVo bioMetricVo){
		String isRecord="";
		isRecord=bioMetricDao.biometricDispAttendance(bioMetricVo);
		return isRecord;
	}
	public String validateDispAttendance(BioMetricVo bioMetricVo)
	{
		String status=null;
		status=bioMetricDao.validateDispAttendance(bioMetricVo);
		return status;
	}
...
//			gLogger.error("Exception occured in getAttendanceReport() method in  BioMetricRegistrationServiceImpl() class "+ e.getMessage());
	...
	try
	{
		...
		Iterator resultItr=null;
		 ...
		 query.append("from EhfmDispUsrMpg dg where dg.id.userId='"+lStrUserId+"' and dg.id.activeyn ='Y' and dg.id.grpId in ('"+roleId+"') ");
...
//		GLOGGER.error("Exception Occurred in getReferredCenterDtls() in PatientDaoImpl class."+e.getMessage());
	...
	@Override
	public String getMacIdForDisp(LabelBean userId) 
	{
		String hospMacId=null;
		String hospId=null;
		String hospDetails=null;
		...
		List<BioMetricVo> bioMetricVo=new ArrayList<BioMetricVo>();
		StringBuffer query=new StringBuffer();
		query.append("select a.dispId as hospId,a.dispMacId as hospMacId from EhfmDispensaryDtls a where a.dispId=? ");
   ...
   query.append("select ds.id.dsgnId as ID ,ds.dsgnName as VALUE FROM EhfmDispDesignation ds");
   vacantList = genericDao.executeHQLQueryList(LabelBean.class,query.toString());
	...
	public boolean biometricDispEnroll(BioMetricVo bioMetricVo) {
		boolean isEnroll=false;
		isEnroll=bioMetricDao.biometricDispEnroll(bioMetricVo);
		return isEnroll;
	}
}
