...
if("searchEmployee".equalsIgnoreCase(lStrActionVal))
{   
	String empId=request.getParameter("loginName");
	bioAtt=request.getParameter("bioAtt");
	String sysMacId=request.getParameter("sysMacId");
	List<BioMetricVo> bioMetricVo=new ArrayList<BioMetricVo>();
	bioMetricVo=bioMetricService.getDispUserDetails(empId,lStrEmpId);
	...
	bioMetricForm.setCugNumber(bioMetricVo.get(0).getMOBILE().toString());
	bioMetricForm.setMobNumber(bioMetricVo.get(0).getMOBILE().toString());
	...
	bioMetricForm.setEmailId(bioMetricVo.get(0).getEMAILID());
/*	bioMetricForm.setHospId(hospId);
	bioMetricForm.setHospMacId(empCapMacId);*/
	String photoUrl=bioMetricVo.get(0).getPHOTO();
	}
	else{
		request.setAttribute("resMsg", "N");
	}
	if(bioAtt!=null&&!bioAtt.equalsIgnoreCase("")&&bioAtt.equalsIgnoreCase("Y"))
		lStrActionVal="dispbioMetricAttendance";
	else
	 lStrActionVal="DispbioCapture";
}


if("DispbioCapture".equalsIgnoreCase(lStrActionVal))
{
...
hospitalList=bioMetricService.getReferredCenterDtls(lStrUserId,roleId);
session.setAttribute("hospitalList",hospitalList);
hospDetails=bioMetricService.getMacIdForDisp(hospitalList.get(0));
List<LabelBean> desgNamesList=bioMetricService.getVacantDtls();
...
List<BioMetricVo> bioMetricVo=new ArrayList<BioMetricVo>();
bioMetricVo=bioMetricService.getDispUserDetails(userId,lStrEmpId);

bioMetricForm.setHospId(hospId);
bioMetricForm.setHospMacId(hospMacId);
request.setAttribute("hospId",hospId);
request.setAttribute("hospMacId",hospMacId);
 request.setAttribute("roleId",roleId);
 request.setAttribute("sysMacId",sysMacId);
...
if("enrollDispUser".equalsIgnoreCase(lStrActionVal))
{
...
BioMetricVo bioMetricVo=new BioMetricVo();
bioMetricVo.setUserId(userId);
bioMetricVo.setMOBILE(Long.parseLong(bioMetricForm.getMobNumber()));
...
bioMetricVo.setEMAILID(bioMetricForm.getEmailId());

try{
isEnroll=bioMetricService.biometricDispEnroll(bioMetricVo);
}
catch(Exception e)
{
e.printStackTrace();
...
BioMetricVo bioMetricVo=new BioMetricVo();
bioMetricVo.setUserId(userId);
bioMetricVo.setMOBILE(Integer.parseInt(bioMetricForm.getMobNumber()));
...
if("dispbioMetricAttendance".equalsIgnoreCase(lStrActionVal))
{
String userId=(String) session.getAttribute("UserID");
...
if("dispvalidateAttendance".equalsIgnoreCase(lStrActionVal))
{
String userId=(String) session.getAttribute("UserID");
...
status=bioMetricService.validateDispAttendance(bioMetricVo);
...
if("disprecordAttendance".equalsIgnoreCase(lStrActionVal))
{
String userId=(String) session.getAttribute("UserID");
...
if(isMatched)
{
ResultMsg=bioMetricService.biometricDispAttendance(bioMetricVo);
