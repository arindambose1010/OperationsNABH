...
public class TelephonicDaoImpl implements TelephonicDao {
	...
	public TelephonicVO getTeleIntimationCases(TelephonicVO telephonicVO) throws Exception{
		...
		try{
			...
			query.append("  select distinct case when (sysdate-trr.crtDt) <= 6 then 'Telephonic Intimation-Initiated' ");
			query.append(" when (sysdate-trr.crtDt) > 6 then 'Telephonic Intimation Cancelled' end as teleStatus, ");
			...
			query1.append(" and trr.asriCatCode=ETCM.id.asriCatCode and trr.asriCatCode=ETPM.id.asriCode and ETPM.id.process='IP' and ETPM.id.state=trr.schemeId and trr.nabhHosp='Y' ");
...
//			System.out.println(count);
	...
	public TelephonicVO getRegTeleIntimationCases(TelephonicVO telephonicVO)throws Exception {
		...
		try{
			...
			query1.append(" where trr.nabhHosp='Y' and  trr.hospId=EH.hospId and EL.id.locId=trr.districtCode  and trr.ICDCatCode=ETCM.id.icdCatCode and trr.ICDProcCode=ETPM.id.icdProcCode" );
			query1.append(" and trr.asriCatCode=ETCM.id.asriCatCode and trr.asriCatCode=ETPM.id.asriCode and trr.schemeId=ETPM.id.state and ETPM.id.process in ('IP','DOP') ");
			...
			if(telephonicVO.getHealthCardNo()!=null && !"".equals(telephonicVO.getHealthCardNo()))
			{
				query1.append(" and trr.cardNo LIKE '%"+telephonicVO.getHealthCardNo()+"%'");
			}
			if(telephonicVO.getFromDate()!=null && !telephonicVO.getFromDate().equals("") && telephonicVO.getToDate()!=null && !telephonicVO.getToDate().equals(""))
			  { 
					fromDate=telephonicVO.getFromDate();
