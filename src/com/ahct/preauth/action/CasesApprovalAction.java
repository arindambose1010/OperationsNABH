...
//										System.out.println(fVO1.getFlagColour());
				...
				if(dop>0){
					request.setAttribute("dentalFlg", "Y");
				}
				else{
					request.setAttribute("dentalFlg", "N");
				}
				
				
				
				CasesSearchVO casesSearchVO1 = new CasesSearchVO();
				casesSearchVO1.setCaseId(caseId);	////////////@@@@@@@@@@@@@@@@@@@
				casesSearchVO1.setLoginName(employeeNo);
				casesSearchVO1.setCaseForDissFlg("Y");
				List<CasesSearchVO> caselist= casesApprovalService.getAllCaseSearchDetails(casesSearchVO1);
				if(caselist.size()>0)
				{
			...
			if(lStrActionVal!= null && "getnewUser".equalsIgnoreCase(lStrActionVal))
			{
				CasesSearchVO casesSearchVO1 = new CasesSearchVO();
				casesSearchVO1.setCaseId(request.getParameter("caseId"));
				casesSearchVO1.setLoginName(request.getParameter("employeeNo"));
				casesSearchVO1.setCaseForDissFlg(request.getParameter("fromPastHistory"));
				List<CasesSearchVO> caselist= casesApprovalService.getAllCaseSearchDetails(casesSearchVO1);
				if(caselist.size()>1)
				{
					request.setAttribute("newUser","N") ;
			...
			if(lStrActionVal!= null && "getPastHistory".equalsIgnoreCase(lStrActionVal))
			{
				...
				List<CasesSearchVO> casesList=casesApprovalService.getAllCaseSearchDetails(casesSearchVO);
				if(casesList.size()==0)
				{
