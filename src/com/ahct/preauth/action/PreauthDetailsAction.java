		  ...
		  //  String lStrUserRole = null;
	        ...
	        if(session.getAttribute("userState").toString()!=null)
			{
			   ...
			   // CommonService  commonService = (CommonService)ServiceFinder.getContext(request).getBean("commonService");
		    	 ...
		    	 */
		    	String pStrPatientId="";
		    	preauthCaseId = request.getParameter("caseId");
		    	EhfCase ehfCase=null;
				EhfTelephonicRegn ehfTelephonicRegn=null;
				if(preauthCaseId!=null && !preauthCaseId.equals(""))
					ehfCase=preauthClinicalNotesService.getCaseDetails(preauthCaseId);
				...
				if(ehfCase!=null)
				{
					...
					if(ehfPatient!=null)
					{
						if(ehfPatient.getIntimationId()!=null && !"".equalsIgnoreCase(ehfPatient.getIntimationId()))
						{
							...
							preauthDetailsForm.setTeleRegDate(sdf.format(ehfTelephonicRegn.getCrtDt()));
		    	...
		    	*/
				...
				if(lStrFlagStatus!= null && "savePreauthDtls".equals(lStrFlagStatus)) 
		        {
					...
					if(!"".equalsIgnoreCase(ipFlag) && ("IPM".equalsIgnoreCase(ipFlag) || "IPD".equalsIgnoreCase(ipFlag) ))
						preauthVO.setIpNumber(preauthDetailsForm.getIpNum());
					...
					if(preauthDetailsForm.getDeathDt() != null )
					preauthVO.setDeathDate(preauthDetailsForm.getDeathDt().replace("-", "/"));
					List<PreauthVO> lstSurgyDtls = preauthService.getcaseSurgertDtls(caseId);
					if(lstSurgyDtls!=null && lstSurgyDtls.size()>0)
		    		{
						for(PreauthVO localVO :lstSurgyDtls)
	    					{
								if(localVO.getIcdProcCode() != null && config.getString("Dialysis.Procs").contains("~"+localVO.getIcdProcCode()+"~"))
								{
									preauthDetailsForm.setDiaProcCode(localVO.getIcdProcCode());
									preauthVO.setIcdProcCode(localVO.getIcdProcCode());
									Long cyclesDone=0L;
									cyclesDone = preauthService.getDialysisCyclesDone(caseId);
									preauthVO.setCycles(cyclesDone);
									preauthVO.setTotCycles(config.getString(localVO.getIcdProcCode()+"_cycles"));
									preauthVO.setIcdAmt(localVO.getIcdAmt());
								}
	    					}
		    		}
				...
				if(lStrFlagStatus!= null && "saveDrugBreakupPreauthDtls".equals(lStrFlagStatus)) 
		        {
					String caseId=request.getParameter("caseId");
					String ipFlag=request.getParameter("ipFlag");
					preauthVO.setCruUsr(lStrEmpId);
					...
					String rslt = preauthService.saveDrugPreauthDtls(preauthVO);
					if(!"".equalsIgnoreCase(rslt) && "true".equalsIgnoreCase(rslt))
					{
						//loggingService.logTime(actionDone, preauthVO.getCaseId(), caseStartTime, caseEndTime, lStrEmpId, "EHS_OperationsNABH");
				...
				if(lStrFlagStatus!= null && "updateRemarks".equals(lStrFlagStatus))  
		        {
					...
					preauthVO.setCruUsr(lStrEmpId);
					preauthVO.setCaseId(caseId);
					preauthVO.setCaseStatusId(caseStatus);
				...
				if(lStrFlagStatus!= null && "updateAttach".equals(lStrFlagStatus))  
		        {
					String caseId=request.getParameter("caseId");
					String sNum=request.getParameter("input");
					//String filename=request.getParameter("arg");
					...
					int editIdd;
					editIdd=Integer.parseInt(request.getParameter("arg1"));
		    		while(preauthDetailsForm.getEditId(editIdd) !=null )
					{
								AttachmentVO attachmentVO=new AttachmentVO();											
								lFormFile=preauthDetailsForm.getEditId(editIdd);                                    
					...
					preauthVO.setCruUsr(lStrEmpId);
					preauthVO.setCaseStatusId(sNum);
					String rsltNew = preauthService.updateEnhAttch(preauthVO);
					if(!"".equalsIgnoreCase(rsltNew) && "true".equalsIgnoreCase(rsltNew))
					{
						preauthDetailsForm.setMsg("Attachment updated");
					}
			    	lStrFlagStatus="preauthDetailsEhf";
		        }
				
				if(lStrFlagStatus!= null && "uploadDialyDocs".equals(lStrFlagStatus))
				{
					String caseId=request.getParameter("caseId");
					String cardNo=request.getParameter("cardNo");
					preauthVO.setCaseId(caseId);
					preauthVO.setEnrollCardNo(cardNo);
					preauthVO.setCruUsr(lStrEmpId);
					FormFile lFormFile=null;
					String lFileName="";
					String lDir=null;
					...
					int lCount=0;
			    		...
			    		int i=0;
			    		while(preauthDetailsForm.getPatientPhoto(i) !=null )
						{
								AttachmentVO attachmentVO=new AttachmentVO();											
								lFormFile=preauthDetailsForm.getPatientPhoto(i);                                     
								lFileName=lFormFile.getFileName();
								if(lFileName!=null && !lFileName.equals(""))
								{
									lCount=lFileName.lastIndexOf(".");
			    		...
			    		int k=0;
			    		while(preauthDetailsForm.getConsentDoc(k) !=null )
						{
								AttachmentVO attachmentVO=new AttachmentVO();											
								lFormFile=preauthDetailsForm.getConsentDoc(k);                                     
								lFileName=lFormFile.getFileName();
								if(lFileName!=null && !lFileName.equals(""))
								{
									lCount=lFileName.lastIndexOf(".");
			    		...
			    		String msg= preauthService.saveDialysisDtls(preauthVO);
			    		if(!"".equalsIgnoreCase(msg) && "Success".equalsIgnoreCase(msg))
			    		{
			    			preauthDetailsForm.setMsg(msg);
			    		}
			    		lStrFlagStatus="preauthDetailsEhf";
				}
				if (lStrFlagStatus != null &&  ("uploadDrugExcel".equals(lStrFlagStatus))) {
			    	 
				 	  String caseId = request.getParameter("caseId");
				 	  
		            if (preauthDetailsForm.getAttachment() != null) {
		            	
                    String lStrTodatDate = "";
                    String errFlag="N";
		            Date date = new Date();
	                ...
	                String lStrYear = lStrTodatDate.substring(8, 10);
		                  
		          	String filepath="/storageNAS-TS-Production/DRUGS_Upload";
		          	File directory = new File (filepath);
		          	String dir =caseId+"_"+ date.getTime()+  "_" + preauthDetailsForm.getAttachment().getFileName();
		          	File lFileFS = new File(directory,dir);
		            FileOutputStream fos = new FileOutputStream(lFileFS + "/");
		            fos.write(preauthDetailsForm.getAttachment().getFileData());
		            System.out.println("Content :"+preauthDetailsForm.getAttachment().getContentType());
		            //Checking EXCEL format
		            FileInputStream fis = new FileInputStream(directory+"/"+dir);
		            List<LabelBean> drugsvoList =  new ArrayList<LabelBean>();
		            ...
		            try{
		                ...
		                request.setAttribute("resMsg", "Improper Excel Sheet.\\nPlease upload proper Excel Sheet as per template");
		                //return mapping.findForward("uploadBRS");
					     lStrFlagStatus="preauthDetailsEhf";
		            ...
		            //Skipping First row(copied comment)
		            int rej = 0;
		            if (rowIterator.hasNext()) {
		                Row row = rowIterator.next();
		                if (row.getCell(0, Row.RETURN_BLANK_AS_NULL) != null && 
		                    !row.getCell(0, 
		                                 Row.RETURN_BLANK_AS_NULL).equals("")) {
		                    String drugId= 
		                        row.getCell(0, Row.RETURN_BLANK_AS_NULL).getStringCellValue().toUpperCase();
		                    if(!drugId.contains("DRUGID")){
		                        modHds = "DRUGID column not found";
		                    }
		                   
		                } else {
		                    msg = "DRUGID column heading cannot be null";
		                    modHds = "DRUGID column heading cannot be null";
		                }
		                if (row.getCell(1, Row.RETURN_BLANK_AS_NULL) != null && 
		                    !row.getCell(1, 
		                                 Row.RETURN_BLANK_AS_NULL).equals("")){
		                    String Quant= 
		                        row.getCell(1, Row.RETURN_BLANK_AS_NULL).getStringCellValue().toUpperCase();
		                    if(!Quant.contains("QUANTITY")){
		                        modHds = "Quantity column not found";
		                    }
		                }else {
		                    msg = "Quantity column heading cannot be null";
		                    modHds = "Quantity column heading cannot be null";
		                }
		            ...
		            if(modHds==null || "".equals(modHds)){
		                String dtType = "Please verify the following Errors:\\n------------------------------------";
		                int count=1;
		            	while(rowIterator.hasNext()){
                          LabelBean drugsVO=new LabelBean();
                          drugsVO.setPatientId(request.getParameter("patientId"));
                          drugsVO.setHospId(request.getParameter("hospId"));
                          drugsVO.setCaseId(caseId);
		                    Row row = rowIterator.next();
		                    try{
		                        if (row.getCell(0, Row.RETURN_BLANK_AS_NULL) != null && 
		                            !row.getCell(0, 
		                          ...
		                          //  System.out.println(row.getCell(0, Row.RETURN_BLANK_AS_NULL).getStringCellValue());
		                            String drugId = 
		                                row.getCell(0, Row.RETURN_BLANK_AS_NULL).getStringCellValue().toUpperCase();
		                                //System.out.println(row.getCell(0, Row.RETURN_BLANK_AS_NULL).getStringCellValue());
		                                drugsVO.setDrugId(drugId.toUpperCase());
		                        }
		                        else{
		                        	errFlag="DRUGID can not be null, Please check EXCEL sheet";
		                        }
		                        }catch (Exception e){
		                            dtType += "\\n* Improper Drug Format.";
		                            //LOGGER.error("Error while Uploading Bank Statement: ERROR:"+e.getMessage());
		                        }
		                      try{  
		                        if (row.getCell(1, Row.RETURN_BLANK_AS_NULL) != null && 
		                            !row.getCell(1, 
		                                         Row.RETURN_BLANK_AS_NULL).equals("") && 
		                            !row.getCell(1, 
		                                         Row.RETURN_BLANK_AS_NULL).toString().trim().equals("")){
		                            String quant =row.getCell(1, Row.RETURN_BLANK_AS_NULL).getNumericCellValue() +"";
		                            
		                            float f = Float.parseFloat(quant);
		                            int intRate = (int) f;
		                            if(intRate!=0&&!quant.contains("-")){
		                            drugsVO.setDrugQuantity((float)intRate);
		                            }
		                            else{
		                            	errFlag="QUANTITY can not be 0 or null or negative  values";
	                      	
		                            }

		                          }
		                        else{
		                        	errFlag="QUANTITY can not be null or negitive values or fractions(Only Numbers Allowed), Please check EXCEL sheet";
		                        }
		                        }catch (Exception e){
		                            dtType += "\\n* Improper Quantity Column Type.\\n  Correct to Number Type";
		                            //LOGGER.error("Error while Uploading Bank Statement: ERROR:"+e.getMessage());
		                        }
		                        drugsvoList.add(drugsVO);
		                   count++ ;
		                   if(count>1001){
		                	   
		                	   System.out.println("Limit exceeded more than 1000 " +count);
		                	   break;
		                   }
		            	}
		            	if(count==1||count>1001){
		            		if(count==1)
		                    request.setAttribute("resMsg", "No Drugs found in Excel Sheet \\nPlease upload Excel Sheet as per template");
		            		else
		            	    request.setAttribute("resMsg", "Drugs should not exceed 1000 per Excel sheet ");		
		            	}
		            	   else{
		            	   //System.out.println(drugsvoList.size());
                         int drugCount=0;
		            	if(drugsvoList.size()>0&&"Please verify the following Errors:\\n------------------------------------".equals(dtType)&&errFlag.equalsIgnoreCase("N")){
		            		String checkAmountExceed=preauthService.checkAmountExceed(drugsvoList);
		            		if(checkAmountExceed!=null&&!checkAmountExceed.equalsIgnoreCase("")&&checkAmountExceed.equals("Y")){
		            		String patientId = request.getParameter("patientId");
		          	 	 	String hospId = request.getParameter("hospId"); 
		          	 	 List<LabelBean> resList=new ArrayList<LabelBean>();
		          	 	 	for(int i=0;i<drugsvoList.size();i++){
		              	 	LabelBean drugVoreq = new LabelBean();
		              	 	drugVoreq.setCaseId(caseId);
		              	 	drugVoreq.setDrugId(drugsvoList.get(i).getDrugId());
		              	 	drugVoreq.setDrugQuantity(drugsvoList.get(i).getDrugQuantity());
		              	 	//drugVoreq.setUnitPrice(Float.parseFloat(unitPrice));
		              	 	drugVoreq.setPatientId(patientId);
		              	 	drugVoreq.setHospId(hospId);
		              	 	drugVoreq.setUserId(lStrEmpId);
		              	 	drugVoreq.setExcelFlag("Y");
		              	 	drugVoreq.setFilepath(lFileFS.toString());
		              	 	drugVoreq.setFileName(dir);
		              	 	drugCount++;
		              	 	drugVoreq.setDrugcount(drugCount);
		          	 	 	
		          	        resList = preauthService.saveDrugDetails(drugVoreq);
		          	        
		          	 	 	}
		          	 	 if(resList!=null && !resList.isEmpty() && resList.size()>0){
		    			
		    				request.setAttribute("drugList",resList);
                            request.setAttribute("resMsg", "Drug File Uploaded Successfully");                        
		    				request.setAttribute("excelFlag","Y");

		    	        }
		          	 	 else{
		          	 	
                                request.setAttribute("resMsg", "Drug  File  Upload Failed");
		          	 	 }
				       lStrFlagStatus="preauthDetailsEhf";
		            		}
		            		else{
		            			
		            			request.setAttribute("resMsg", checkAmountExceed);
		            		}
		            	}else{
		            		if(errFlag!=null&&!"N".equalsIgnoreCase(errFlag))
		            			request.setAttribute("resMsg", errFlag);
		            		else
                            request.setAttribute("resMsg", dtType);
		            	}
		             }	
		            }else{
		                if("NODATA".equals(modHds))
		                    request.setAttribute("resMsg", "Cannot upload empty Excel Sheet \\nPlease upload Excel Sheet as per template");
		                else    
		                   request.setAttribute("resMsg", "Please upload Excel Sheet as per template");
		            }
		              }
				       lStrFlagStatus="preauthDetailsEhf";
				...
				if (lStrFlagStatus != null
						&& lStrFlagStatus.equalsIgnoreCase("viewAttachment")) {
	            	 ...
	            	 */
	            		...
	            		lStrFilePath = "/storageNAS-TS-Production/DRUGS_Upload/SampleEXCEL.xls"; 
	            	    File file = null ;
			          ...
			          if(fileExt.equalsIgnoreCase(ASRIConstants.FILE_EXT_JPG) || fileExt.equalsIgnoreCase(ASRIConstants.FILE_EXT_JPEG) 
			        		  || fileExt.equalsIgnoreCase(ASRIConstants.FILE_EXT_GIF) || fileExt.equalsIgnoreCase(ASRIConstants.FILE_EXT_PDF)||fileExt.equalsIgnoreCase(ASRIConstants.FILE_EXT_XLS)){
			              attachMode="inline";
			          }
...
//			    	System.out.println(preauthDetailsForm.getSplInvest());
			    ...
			    //	preauthDetailsForm.setGenRemarks(preauthDetailsForm.getGenRemarks());
			    	...
			    	preauthDetailsForm.setSplInvest("");
			    ...
			    // Save Implants for surgical cases by Teja
			    if(lStrFlagStatus!= null && "saveSurgicalImplants".equals(lStrFlagStatus)){
			    	preauthVO.setCaseId(preauthDetailsForm.getCaseId());
			    	preauthVO.setEnhancementDtls(preauthDetailsForm.getEnhancementDtls());
			    	preauthVO.setCruUsr(lStrEmpId);
					...
					List<AttachmentVO> lAttachList=new ArrayList<AttachmentVO>();
					int lCount=0;
			    /*	for(int i=0;i<preauthDetailsForm.getMedclExpnsAttch().length-1;i++)
			    	{*/
			    		int i=0;
			    		while(preauthDetailsForm.getMedclExpnsAttch(i) !=null )
						{
			    	...
			    	preauthVO.setAttachmentsList(lAttachList);
			    	preauthDetailsForm.setEnhancementDtls("");
			    	preauthService.saveImplantsPreauth(preauthVO);
			    	lStrFlagStatus="preauthDetailsEhf";
...
//			    	System.out.println(preauthDetailsForm.getState());
			  ...
			  //Holding and unholding cases---- teja
				 if(lStrFlagStatus!= null && "holdUnholdCase".equals(lStrFlagStatus)) 
			     {   
					if(lStrgrpList.contains(config.getString("Group.EO")))
						{
							roleId=config.getString("Group.EO");
						}
					 String caseId=request.getParameter("caseId");
...
//					 System.out.println(roleId);
					 ...
					 String nimsHospIdNew=preauthService.getHospScheme(caseId);
					 ...
					 if(resultMsg != null && resultMsg.trim().length() > 0){
						 if("Y".equalsIgnoreCase(resultMsg)){
							 ...
							 else if(holdStatus != null && holdStatus.trim().length() >0 && (PreauthConstants.Release).equalsIgnoreCase(holdStatus)){
								 message = "Case has been Released";
							 }
							 else if(holdStatus != null && holdStatus.trim().length() >0 && (PreauthConstants.pending).equalsIgnoreCase(holdStatus)){
								 message = "Case has been pending to medco";
							 }
							 else if(holdStatus != null && holdStatus.trim().length() >0 && (PreauthConstants.verify).equalsIgnoreCase(holdStatus)){
								 message = "Case has been Recommended Released";
							 }
							 else if(holdStatus != null && holdStatus.trim().length() >0 && (PreauthConstants.reject).equalsIgnoreCase(holdStatus)){
								 message = "Case has been Recommended to Discharge";
							 }
							 else{
								 message = "Case has been Recommended Released";
							 }
...
//					 System.out.println(casesForApprovalFlag);
			    	 ...
			    	 if(ehfPatientDtls != null)
			    	 {
			    		 ...
			    		 preauthDetailsForm.setCardNo(ehfPatientDtls.getCardNo());
			    	 }
			    	 List<LabelBean> catgoryList = patientService.getCatName(caseId);
						if(catgoryList.size() > 0)	
						{
							...
							preauthDetailsForm.setSpecialityCode(catgoryList.get(0).getICDCODE());
							preauthDetailsForm.setIcdProcName1(catgoryList.get(0).getNEWPROC());
							if(catgoryList.get(0).getCOMPROLE() != null)
							{
							request.setAttribute("ipOtherRemarks1",catgoryList.get(0).getCOMPROLE());
							}
						}
						String critical1=patientService.getCriticalFlg(caseId);
				    	 if(critical1!=null && !"".equalsIgnoreCase(critical1))
				    	 {
				    		 request.setAttribute("critical1",critical1);
					...
					//Added for IP Medical Scenarios 	
					/* if (ehfPatientDtls!=null && ehfPatientDtls.getPatientIpop() != null && !"".equalsIgnoreCase(ehfPatientDtls.getPatientIpop())  && "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()))
					 {
						 claimFlowVO = preauthService.getDrugDetailsData(claimFlowVO);
						 claimFlowVO = preauthService.getInvestigationDetails(claimFlowVO);
						 claimFlowVO = preauthService.saveEnhanceAmounts(claimFlowVO);
			    	 ...
			    	 if(caseId!=null && !"".equalsIgnoreCase(caseId))
			    		{
			    		 	...
			    		 	String nimsHospId=preauthService.getHospScheme(caseId);
			    		 	...
			    		 	request.setAttribute("nimsHospId", nimsHospId);
			    		}
		    	 ...
		    	 if(!"".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()) && ("IP".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()) || "IPD".equalsIgnoreCase(ehfPatientDtls.getPatientIpop())) )
		    	 {
...
//							System.out.println(dentalrntdbvalue);
							...
							preauthDtlsVO = preauthService.getPatientFullDtls(caseId, ehfPatient.getPatientId(),fromDisp);		
							request.setAttribute("occlusiondtls", occlusiondtls);
							request.setAttribute("deciduousdentdtls", preauthDtlsVO);
				    	}
			    	 List<LabelBean> catList = commonService.getCategorys(null,lStrEmpId,caseId);
					...
					request.setAttribute("dentalUnitsList",dentalUnitsList);
				    	...
				    	//preauthDetailsForm.setIpNum(preauthVO.getIpNumber());
				    	/**
				    	...
				    	preauthDetailsForm.setDeathDt(preauthVO1.getDeathDate());
				    	preauthDetailsForm.setIpNum(preauthVO1.getPatientIPNo());
				    	...
				    	request.setAttribute("consumableFlg", preauthVO1.getConsumableFlg());
				    	if(preauthVO1.getConsumableFlg()!=null&&!"".equalsIgnoreCase(preauthVO1.getConsumableFlg())&&preauthVO1.getConsumableFlg().equalsIgnoreCase("Y"))
				    	{
				    	String drugAmount= preauthService.getOldDrugsAmount(caseId);
				    	//preauthDetailsForm.setDrugAmount(drugAmount);

	                    String consumableAmt= preauthService.getEnhancementSum(caseId, "IPM");
	                    preauthDetailsForm.setOldConsumableAmt(consumableAmt);
	                    
	                    Double oldtotEnhamt= Double.parseDouble(consumableAmt)+ Double.parseDouble(drugAmount);
	                    preauthDetailsForm.setOldTotalAmt(oldtotEnhamt.toString());
				    	...
				    	preauthDetailsForm.setOrganTransYN(preauthVO1.getOrganTransYN());//For organ transplantation
				    	 if(ehfCase!=null && (!"".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()) && "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()))&&!preauthVO.getCaseStatusId().equalsIgnoreCase("CD2")&&!preauthVO.getCaseStatusId().equalsIgnoreCase("CD2589")&&lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")))
					 		{  
				    		 if((!"".equalsIgnoreCase(preauthVO.getCaseStatusId()) &&  preauthVO.getCaseStatusId().equalsIgnoreCase("CD8") && ehfCase.getAmountLockedFor()==null)   || 
									((!"".equalsIgnoreCase(preauthVO.getCaseStatusId()) &&  preauthVO.getCaseStatusId().equalsIgnoreCase("CD207") && ehfCase.getAmountLockedFor()==null)))
				    		    {     preauthDetailsForm.setAmountLockedFor(ehfCase.getAmountLockedFor());
									if(Long.valueOf(preauthVO1.getPREAUTHAPPVAMT())!=null && ehfCase.getAmountLockedFor()!=null ){
										if( ((critical1!=null&&!critical1.equalsIgnoreCase("")&&critical1.equalsIgnoreCase("Y")))   || 
												((  critical1!=null&&!critical1.equalsIgnoreCase("")&&critical1.equalsIgnoreCase("N"))) )
											
										{ 
	                                     if(  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <= 200000 ){
							    		    	 preauthDetailsForm.setNextAmountLocked(200000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(400000l);
							    		    }
							    		    if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 200000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  400000){
							    		    	 preauthDetailsForm.setNextAmountLocked(400000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(600000l);
							    		    } 
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 400000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  600000){
							    		    	 preauthDetailsForm.setNextAmountLocked(600000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(800000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 600000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  800000){
							    		    	 preauthDetailsForm.setNextAmountLocked(800000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1000000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 800000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1000000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1000000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1100000l);

							    		    }
							 /*   		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 700000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  800000){
							    		    	 preauthDetailsForm.setNextAmountLocked(800000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(900000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 800000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  900000){
							    		    	 preauthDetailsForm.setNextAmountLocked(900000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1000000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 900000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1000000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1000000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1100000l);

							    		    }*/
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1000000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1100000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1100000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1200000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1100000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1200000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1200000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1300000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1200000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1300000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1300000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1400000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1300000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1400000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1400000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1500000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1400000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1500000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1500000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1600000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1500000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1600000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1600000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1700000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1600000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1700000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1700000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1800000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1700000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1800000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1800000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(1900000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1800000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  1900000){
							    		    	 preauthDetailsForm.setNextAmountLocked(1900000l);
							    		    	 preauthDetailsForm.setMaxAmountAllowed(2000000l);

							    		    }
							    		    else  if( Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) > 1900000 &&  Long.valueOf(preauthVO1.getPREAUTHAPPVAMT()) <=  2000000){
							    		    	 preauthDetailsForm.setNextAmountLocked(2000000l);
										...
										if(Long.valueOf(preauthVO1.getPREAUTHAPPVAMT())!=null && Long.valueOf(preauthVO1.getPREAUTHAPPVAMT())<=200000 ){
										preauthDetailsForm.setNextAmountLocked(200000l);
										}
										else{
											preauthDetailsForm.setNextAmountLocked(preauthVO1.getPREAUTHAPPVAMT()+1);
				    		    ...
				    		    if(ehfCase.getAmountLockedFor()!=null){
				    		    	preauthDetailsForm.setAmountLockedFor(ehfCase.getAmountLockedFor());
				    		        long amountLocked=ehfCase.getAmountLockedFor();
				    		    if( amountLocked !=0 && amountLocked < 200000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <=  200000){
				    		    	 preauthDetailsForm.setNextAmountLocked(200000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(400000l);
				    		    }
				    		    if( amountLocked >= 200000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  200000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  400000){
				    		    	 preauthDetailsForm.setNextAmountLocked(400000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(600000l);
				    		    } 
				    		    else if( amountLocked >= 400000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  400000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  600000 ){
				    		    	 preauthDetailsForm.setNextAmountLocked(600000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(800000l);

				    		    }
				    		    else if( amountLocked >= 600000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  600000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  800000){
				    		    	 preauthDetailsForm.setNextAmountLocked(800000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1000000l);

				    		    }
				    		    else if( amountLocked >= 800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1000000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1000000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1100000l);

				    		    }
				    		  /*  else if( amountLocked >= 700000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  700000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  800000){
				    		    	 preauthDetailsForm.setNextAmountLocked(800000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(900000l);

				    		    }
				    		    else if( amountLocked >= 800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  900000){
				    		    	 preauthDetailsForm.setNextAmountLocked(900000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1000000l);

				    		    }
				    		    else if( amountLocked >= 900000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  900000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1000000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1000000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1100000l);

				    		    }
				    		  */  else if( amountLocked >= 1000000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1000000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1100000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1100000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1200000l);

				    		    }
				    		    else if( amountLocked >= 1100000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1100000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1200000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1200000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1300000l);

				    		    }
				    		    else if( amountLocked >= 1200000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1200000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1300000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1300000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1400000l);

				    		    }
				    		    else if( amountLocked >= 1300000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >= 1300000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1400000 ){
				    		    	 preauthDetailsForm.setNextAmountLocked(1400000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1500000l);

				    		    }
				    		    else if( amountLocked >= 1400000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1400000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1500000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1500000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1600000l);

				    		    }
				    		    else if( amountLocked >= 1500000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1500000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1600000 ){
				    		    	 preauthDetailsForm.setNextAmountLocked(1600000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1700000l);

				    		    }
				    		    else if( amountLocked >= 1600000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1600000 &&  Long.parseLong(preauthVO1.getEnhAmt()) < 1700000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1700000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1800000l);

				    		    }
				    		    else if( amountLocked >= 1700000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1700000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1800000){
				    		    	 preauthDetailsForm.setNextAmountLocked(1800000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(1900000l);

				    		    }
				    		    else if( amountLocked >= 1800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1800000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  1900000 ){
				    		    	 preauthDetailsForm.setNextAmountLocked(1900000l);
				    		    	 preauthDetailsForm.setMaxAmountAllowed(2000000l);

				    		    }
				    		    else if( amountLocked >= 1900000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  1900000 &&  Long.parseLong(preauthVO1.getEnhAmt()) <  2000000){
				    		    	 preauthDetailsForm.setNextAmountLocked(2000000l);

				    		    }
				    		    else if( amountLocked >= 2000000 &&  Long.parseLong(preauthVO1.getEnhAmt()) >=  2000000 ){
				    		    	 preauthDetailsForm.setNextAmountLocked(0l);

				    		    }
				    		    
				    		    }else{
				    		    	if(  critical1!=null&&!critical1.equalsIgnoreCase("")&&critical1.equalsIgnoreCase("Y") )
				    		    		 preauthDetailsForm.setNextAmountLocked(100000l);
				    		    	else
				    		    		 preauthDetailsForm.setNextAmountLocked(50000l);
				    		    }
				    		 }   
				    		 	SimpleDateFormat sdf=new SimpleDateFormat("dd/MM/yyyy");
					    			SimpleDateFormat sdf1 = new SimpleDateFormat("HH:mm");
					 			...
					 			preauthDetailsForm.setHoldRejectFlag(preauthVO1.getHoldRejectFlag());		
					 		}
				    			...
				    			for(PreauthVO localVO :lstSurgyDtls)
				    				{
				    					...
				    					//For Dialysis Cases Biometric Option
				    					if(localVO.getIcdProcCode() != null && config.getString("Dialysis.Procs").contains("~"+localVO.getIcdProcCode()+"~"))
				    					{
				    						request.setAttribute("dialysisProc", "Y");
				    						preauthDetailsForm.setDiaProcCode(localVO.getIcdProcCode());
				    						EhfPatient ehfPatient=null;
											ehfPatient=(EhfPatient)patientService.getPatientDetails(patientId);
											List<PreauthVO> dialysisList = preauthService.getDialysisReportList(caseId);
											Long cycles = 1L;
											cycles = preauthService.getDialysisCycles(caseId);
											preauthDetailsForm.setCycles(cycles);
											if(dialysisList.size()>0)
											{
												if(dialysisList != null)
												preauthDetailsForm.setDailysisList(dialysisList);
											}
											List<BioMetricVo> captureList =  preauthService.getOutTimeCapture(caseId);
											if(captureList != null && captureList.size()>0)
											{
												if(captureList !=null)
												{
													preauthDetailsForm.setCaptureList(captureList);
													preauthDetailsForm.setPatientInTime(captureList.get(0).getINTIME());
												}
											}
				    					...
				    					{
				    						request.setAttribute("dialysisProc", "N");
				    					}
				    				}
				    		}
							else
				    	...
				    	for (int i=0; i<lstSurgyDtls.size();i++)
				    	{
				    		if(lstSurgyDtls.size()>0)
					    	{
					    		if(lstSurgyDtls != null)
					    		{
					    			if(lstSurgyDtls.get(i).getRobotSurgery() != null  && "Y".equalsIgnoreCase(lstSurgyDtls.get(i).getRobotSurgery()))
					    			{
					    				request.setAttribute("roboFlg", lstSurgyDtls.get(i).getRobotSurgery());
					    				break;
					    			}
					    			else
					    			{
			    		...
			    		List<PreauthVO> lstenhancementDtls = preauthService.getEnhancementList(caseId,patientType);
				    	...
				    	if(config.getString("preauth_role_status_mapping").contains(roleStatus))
				    	{
				    		...
				    		preauthDetailsForm.setRegDate(namDeclarationDtls.getDate().toString());
				    		 ...
				    		 {
				    			if(preauthVO1.getPendingFlag()==null ||preauthVO1.getPendingFlag().equals("") )
				    		{
				    		 ...
				    		 */
				    		if(preauthVO.getCaseStatusId().equalsIgnoreCase(config.getString("preauth_inpatient_caseReg")))
					    		 {
					    			...
					    			if(preauthVO1.getOrganTransYN()!=null && preauthVO1.getOrganTransYN().equalsIgnoreCase("Y"))
					    				status=false;
					    			if("IPD".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()))
					    				status=true;
						    		if(status==true)
						    			{
						    				/**
				    	...
				    	// viewType by Teja
				    	else if(config.getString("preauth_approved_medco_role").contains(roleStatus)){
				    		request.setAttribute("viewType", "preAppMedco");
				    	}
				    	else if(config.getString("preauth_hold_status_mapping").contains(roleStatus))
				    		{
					    		/**
						    	 * get buttons list  based on particular roles and case status
						    	 */
						    	List<LabelBean> buttonsList =  commonService.getDynamicWrkFlowDtls(preauthVO.getCaseStatusId(), lStrUserGroup,config.getString("preauth_main_module"),config.getString("preauth_sub_module"));
						    	preauthDetailsForm.setButtonsLst(buttonsList);
						    	request.setAttribute("viewType","others" );
				    		}
				    	else
			    			request.setAttribute("viewType", "disable");
				    	...
				    	//Changed by namratha to retrieve drugs list based on case id
				    	List<DrugsVO> drugs=preauthService.getMedicalDrugs(preauthDetailsForm.getCaseId(),"Preauth");
				    	if(drugs.size()>0)
				    	{	
				    		if(drugs != null)
				    			preauthDetailsForm.setDrugLt(drugs);
				    	    	for(int i=0;i<drugs.size();i++){
				    	    		drgAmt=drgAmt+Long.parseLong(drugs.get(i).getAMOUNT());
				    	    	}
				    	    	request.setAttribute("oldDrugAmt", drgAmt);
				    	}
				    	List<LabelBean> drugsList=commonService.getDrugs();
						request.setAttribute("drugsList", drugsList);
				    	
				    	/*	if( (("CD8".equalsIgnoreCase(preauthVO.getCaseStatusId()) )|| ("CD207".equalsIgnoreCase(preauthVO.getCaseStatusId()) )  )
				    				&& isMedco && "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()) )
			    				{
							    	List<LabelBean> hospStay = preauthService.getHospStayist(config.getString("enhType_HospitalStay"));
							    	request.setAttribute("hospStay", hospStay);
							    	List<LabelBean> imageologyStay = preauthService.getHospStayist(config.getString("enhType_Imageology"));
							    	...
							    	//preauthDetailsForm.setEnhancementFlag("Y");
				    		}*/

//				    	System.out.println(lStrUserGroup+"group");
//				    	System.out.println(preauthVO.getCaseStatusId() +"status");
						List<LabelBean> drugTypeLstFinal = new ArrayList<LabelBean>();

/*						List<LabelBean> drugTypeLst = preauthService.getDrugTypeList(preauthVO1.getHospitalId());
*/				    	//request.setAttribute("drugType", drugTypeLst);
						/*if (drugTypeLst != null && drugTypeLst.size() > 0) {
			               System.out.println(drugTypeLst.size());
							for (LabelBean labelBean : drugTypeLst) {
								if (labelBean.getID() != null && labelBean.getVALUE() != null) {
									System.out.println(drugTypeLst.size());
									System.out.println(labelBean.getID() + "~" + labelBean.getVALUE());
									drugTypeLstFinal.add(labelBean.getID() + "~" + labelBean.getVALUE());
									//drugTypeLstFinal=drugTypeLstFinal+"@"+labelBean.getID() + "~" + labelBean.getVALUE();

								}
							}
						}*/
/*				         request.setAttribute("drugTypeLstFinal", drugTypeLst);
 *                       
*/		                String previousId=null;
						if(preauthVO.getCaseStatusId()!=null && preauthVO.getCaseStatusId().equalsIgnoreCase(config.getString("ACC.SOFHeadId")))
								{
							 previousId = preauthService.getpreviousStatus(caseId);
								}
				    	...
				    	||(lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")) && preauthVO.getCaseStatusId().equalsIgnoreCase(config.getString("preauth_eo_pending")))
				    	||(lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")) && preauthVO.getCaseStatusId().equalsIgnoreCase(config.getString("hold_case_eo_pending")))
				    	|| (lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")) && previousId!=null && previousId.equalsIgnoreCase("Y"))
				    			)
				    	...
				    	||(lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")) && "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()) ))
				    	/*||(lStrUserGroup != null && lStrUserGroup.equalsIgnoreCase(config.getString("preauth_medco_role")) && !preauthVO.getCaseStatusId().equalsIgnoreCase(config.getString("preauth_eo_pending"))))*/
				    	{
...
//						    		System.out.println(x);
				    	...
				    	{  if( "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop())&&preauthDetailsForm.getHospitalId()!=null&&!"".equalsIgnoreCase(preauthDetailsForm.getHospitalId())&&preauthDetailsForm.getHospitalId().equalsIgnoreCase("EHS24")&&preauthVO.getCaseStatusId().equalsIgnoreCase("CD2589"))
				    		request.setAttribute("viewType", "enhMedco");	
				    	   else
				    		request.setAttribute("viewType", "disable");	
				    		if(config.getString("preauth_approved_medco_role").contains(roleStatus)){
					    		request.setAttribute("viewType", "preAppMedco");
					    	}
				    		else
				    		{
				    			if(!"".equalsIgnoreCase(config.getString("preauth_statusName_"+lStrUserGroup)) && config.getString("preauth_statusName_"+lStrUserGroup) != null )
				    					request.setAttribute("viewType",config.getString("preauth_statusName_"+lStrUserGroup) );
				    			else if(holdAccess)
				    				request.setAttribute("viewType","others");
				    			if(config.getString("preauth_hold_status_mapping").contains(roleStatus))
					    		   {
					    			request.setAttribute("viewType","others");
					    		   }
				    		}
			    	...
			    	// return mapping.findForward("preauthDtlsEhf");
				    	 ...
				    	 String drugAmt = preauthService.getDrugsAmount(caseId);
				    	 
				    	 if(drugAmt!=null && !"".equals(drugAmt)){
				    		 preauthDetailsForm.setDrugAmount(String.valueOf(Double.valueOf(drugAmt).longValue()));
				    		 request.setAttribute("drugAmount",String.valueOf(Double.valueOf(drugAmt).longValue()));
				    	 }
				    	 /**
				    	 ...
				    	 {
				    		 preauthDetailsForm.setCritical(critical);
				    		 preauthDetailsForm.setComorBidAppvAmt("0");
				    	 }
...
//				    	 System.out.println("Viewtype"+x);
			   ...
			   //By ramalakshmi For checking mandatory attachments
			    if(lStrFlagStatus!= null && "checkMandatoryAttch".equals(lStrFlagStatus)) 
		        {
			    	 String caseId = request.getParameter("caseId");
			    	 String ipFlag = request.getParameter("ipFlag");
			    	 String attachType = request.getParameter("attachType");
			    	 String drugFlag=request.getParameter("drugbrkFlag");
			    	 String msg = preauthService.checkMandatoryAttch(caseId,attachType,ipFlag,drugFlag);
			    	 request.setAttribute("AjaxMessage",msg);
			    	 return mapping.findForward("ajaxResult");	
		        }
									...
									for(CasesSearchVO casesSearchVODOP:list)
										{
											if(casesSearchVODOP.getProcess()!=null)
												{
												if(casesSearchVODOP.getProcess().equalsIgnoreCase("IPM") || casesSearchVODOP.getProcess().equalsIgnoreCase("IPD"))
														{
															dop=dop+1;
														}
													else
        ...
        if(lStrFlagStatus!= null && "getIcdCatList".equals(lStrFlagStatus)) 
        	...
        	{
        	lstIcdCat = commonService.getCategorys(catId,lStrEmpId,caseID);
        	}
			...
			int count=0;
				...
				{
					if(("S19").equalsIgnoreCase(catId))
					{
					if(transHospLst.contains(hospCode)){
					for(int j=0; j<transHospLst.size();j++)
					{
						if(transHospLst.get(j).equalsIgnoreCase(hospCode)){							
								if(("S19").equalsIgnoreCase(catId) && (catList.get(i).contains(transCodeLst.get(j))))
								{
									
									if(count==0 && i==0)
									{	
										
										icdCatList.add(count,catList.get(i));
									}
									else if(count==0 && i!=0)
									{
										icdCatList.add(count,catList.get(i).replace("@", ""));
									}
									else
									{
										icdCatList.add(count,catList.get(i));
...
//						System.out.println(temp);
						...
						icdCatList.add(i,catList.get(i));
					}
				}
...
//			System.out.println("doctor list size is :"+doctorList.size());
			...
			if( (!"".equalsIgnoreCase(catId) && config.getString("Robotic.Surgeries").contains("~"+catId+"~"))
	  ...
	  {
		  lstIcdCat = commonService.getProceduresNABH(icdCatCode, asriCode,hospId,scheme,dentEnhFlag,ipFlag);
	  }
        ...
        if(lStrFlagStatus!= null && "getOnlineCaseSheet".equals(lStrFlagStatus)) 
        {
			...
			request.setAttribute("forPrint",request.getParameter("forPrint"));
        	 if(request.getParameter("caseSheetFlag")!=null && !"".equalsIgnoreCase(request.getParameter("caseSheetFlag")) && "Y".equalsIgnoreCase(request.getParameter("caseSheetFlag")))
        	 {
	             ...
	             com.ahct.patient.vo.PatientVO patientDtlsVo = new com.ahct.patient.vo.PatientVO();
	             request.setAttribute("dentalFlg",request.getParameter("dentalFlg") );
					if(request.getParameter("dentalFlg")!=null && "Y".equalsIgnoreCase(request.getParameter("dentalFlg"))){
						patientDtlsVo=patientService.getPatientDentalDtls(patientId);
						/*patientForm.setPreviousDentalTreatment(patientDtlsVo.getPreviousDentalTreatment());
						patientForm.setOcclusion(patientDtlsVo.getOcclusion());
						patientForm.setTmj(patientDtlsVo.getTmj());
...
//					System.out.println(patientData.getFamilyHis());
				...
				if(commonDtlsVO.getPATTYPE().equalsIgnoreCase("IP") || commonDtlsVO.getPATTYPE().equalsIgnoreCase("IPD"))
				{
...
//			 System.out.println(casesForApprovalFlag);
		...
		claimFlowVO.setUserId(lStrEmpId);
		claimFlowVO.setPreauthType(module);
					...
					if(preauthList.size()>0)
						{
							for(int i=0;i<preauthList.size();i++)
							{
								...
								if(preauthList.get(i).getDIFF()!=null && preauthList.get(i).getDIFF().intValue()>0 && preauthList.get(i).getDIFF().intValue() <= 7)
								{
									msg = "success";
								}
								else
		...
		if(lStrFlagStatus!= null && "searchDrugs".equals(lStrFlagStatus)) 
		{
			String drugSearch = request.getParameter("drugSearch");
			String hospId = request.getParameter("hospId");
			Map<String, Object> paramMap = new HashMap<String, Object>();
			Map<String, Object> resultMap = new HashMap<String, Object>();
			paramMap.put("nameSearch", drugSearch);
			paramMap.put("hospId",hospId);
			resultMap = preauthService.getLuceneSearch(paramMap);
			String json = "";

			Gson gson = new Gson();
			
			json = gson.toJson(resultMap.get("drugDetails"));
			request.setAttribute("AjaxMessage", json);

			return mapping.findForward("ajaxResult");
		}
		if(lStrFlagStatus!= null && "getDrugQuantList".equals(lStrFlagStatus)) 
		{
			List<String> drugTypeLstFinal = new ArrayList<String>();
			String hospId = request.getParameter("hospId");
/*			List<LabelBean> drugTypeLst = preauthService.getDrugTypeList(hospId);
	    	//request.setAttribute("drugType", drugTypeLst);
			if (drugTypeLst != null && drugTypeLst.size() > 0) {
               System.out.println(drugTypeLst.size());
				for (LabelBean labelBean : drugTypeLst) {
			...
			*/
			String caseId = request.getParameter("caseId");
			String json="";
			 List<LabelBean> resList = preauthService.getSavedDrugDetails(caseId);
		       // if(resList!=null && !resList.isEmpty() && resList.size()>0){
					 Gson gson = new Gson();
					 
				 	HashMap<String,Object> lParamMap = new HashMap<String,Object>();
/*				 	lParamMap.put("drugsList",drugTypeLstFinal);
*/				 	lParamMap.put("cardDetails",resList);
		...
		if(lStrFlagStatus!= null && "getDrugPriceList".equals(lStrFlagStatus)) 
		{
			String drugId=request.getParameter("drugId");
			List<String> drugPriceLstFinal = new ArrayList<String>();
			List<LabelBean> drugPriceLst = preauthService.getDrugPriceList(drugId);
	    	//request.setAttribute("drugType", drugTypeLst);
			if (drugPriceLst != null && drugPriceLst.size() > 0) {

				for (LabelBean labelBean : drugPriceLst) {
					if (labelBean.getID() != null) {

						drugPriceLstFinal.add(labelBean.getID());

					}
				}
			}
			if (drugPriceLstFinal != null && drugPriceLstFinal.size() > 0)
				request.setAttribute("AjaxMessage", drugPriceLstFinal);
	    	
			 
	    	 return mapping.findForward("ajaxResult");
		}
		if(lStrFlagStatus!= null && "getDrugTotalAmt".equals(lStrFlagStatus)) 
		{
			String drugId = request.getParameter("drugId");
    	    String drugQuant = request.getParameter("drugQuant");
    	
    	Integer amount=0;
    	/*if(quantId!=null && !"".equalsIgnoreCase(quantId) && (config.getString("enhQuantId").contains("~"+quantId+"~")))
    		amount= preauthService.getQuantAmountForNabh(quantId,nabhFlag,caseId);
    	else*/
    		amount =  preauthService.getDrugQuantAmount(drugId);
    	Integer totDrugAmt = 0;
    	if(drugQuant != null && !drugQuant.equalsIgnoreCase(""))
    	{
    		totDrugAmt = Integer.parseInt(drugQuant) * amount;
    	}
    	request.setAttribute("AjaxMessage", totDrugAmt);
		return mapping.findForward("ajaxResult");	
		}
		...
		if(lStrFlagStatus!= null && "saveDrugDetails".equals(lStrFlagStatus)) 
        {
			String drugId = request.getParameter("drugId");
	 	 	String caseId = request.getParameter("caseId");
	 	 	...
	 	 	String patientId = request.getParameter("patientId");
	 	 	String hospId = request.getParameter("hospId");
	 	 	LabelBean drugVo = new LabelBean();
	 	 	drugVo.setCaseId(caseId);
	 	 	drugVo.setDrugId(drugId);
	 	 	drugVo.setDrugQuantity(Float.parseFloat(drugQuantity));
	 	 	drugVo.setUnitPrice(Float.parseFloat(unitPrice));
	 	 	drugVo.setPatientId(patientId);
	 	 	drugVo.setHospId(hospId);
	 	 	drugVo.setUserId(lStrEmpId);
	        List<LabelBean> resList = preauthService.saveDrugDetails(drugVo);
	        if(resList!=null && !resList.isEmpty() && resList.size()>0){
				 Gson gson = new Gson();
				 String json="";
			 	HashMap<String,Object> lParamMap = new HashMap<String,Object>();
		...
		if(lStrFlagStatus!= null && "deleteDrugDetails".equals(lStrFlagStatus)) 
        {
			String drugId = request.getParameter("drugId");
	 	 	String caseId = request.getParameter("caseId");
	 	 	String unitPrice = request.getParameter("unitPrice");
	 	 	...
	 	 	drugVo.setHospId(hospId);
	 	 	drugVo.setUserId(lStrEmpId);
	        List<LabelBean> resList = preauthService.deleteDrugDetails(drugVo);
/*	        if(resList!=null && !resList.isEmpty() && resList.size()>0){
*/				 Gson gson = new Gson();
				 String json="";
			 	HashMap<String,Object> lParamMap = new HashMap<String,Object>();
...
/*	        }*/
		...
		if("getICDCatByAsriCode".equalsIgnoreCase(lStrFlagStatus)){
			String asriCatCode=request.getParameter("lStrAsriCatId");
//			System.out.println(asriCatCode);
			String hospCode=request.getParameter("hospId");
			...
			List<String> icdCatList=new ArrayList<String>(); 
			List<String> doctorList=null;List<String> bothList=null;
		 ...
		 if(lStrFlagStatus!= null && "saveMedicalDetails".equals(lStrFlagStatus)) 
	     {
                String caseId=request.getParameter("caseId");
			    preauthVO.setMedAsriCode(request.getParameter("medicalSpclty"));
			    preauthVO.setCaseId(caseId);
			    preauthVO.setCruUsr(lStrEmpId);
				preauthVO.setMedCatCode(request.getParameter("medicalCat"));
				preauthVO.setIcdProcName(request.getParameter("ICDProcName1"));
				preauthVO.setIpOtherRemark(request.getParameter("ipOtherRemarks"));
				Long caseTheSeqNextVal=0L;
				caseTheSeqNextVal = Long.parseLong(patientService.getSequence("CASE_THERAPY_ID"));
				preauthVO.setMedSeq(caseTheSeqNextVal);
			    String lStrCaseId=preauthService.saveMedicalCaseDetails(preauthVO);
		 ...
		 if(lStrFlagStatus!= null && "getSavedDrugDetails".equals(lStrFlagStatus)) 
	     {
			 String caseId = request.getParameter("caseId");
			 List<LabelBean> resList = preauthService.getSavedDrugDetails(caseId);
		        if(resList!=null && !resList.isEmpty() && resList.size()>0){
