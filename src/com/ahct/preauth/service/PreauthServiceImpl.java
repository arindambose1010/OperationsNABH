...
import com.ahct.model.EhfCaseDialyDtls;
import com.ahct.model.EhfCaseDialyDtlsId;
import com.ahct.model.EhfCaseDocAttch;
import com.ahct.model.EhfDischargeSummary;
import com.ahct.model.EhfPatientBreakupdrugsNabh;
import com.ahct.model.EhfPatientBreakupdrugsNabhPK;
import com.ahct.model.EhfPatientDrugsNabh;
import com.ahct.model.EhfmBreakupDrugMst;
import com.ahct.model.EhfmDentalProcCriteria;
import com.ahct.model.EhfmDentalProcCriteriaPK;
import com.ahct.model.EhfmMainTherapyNabh;
import com.ahct.model.EhfmMainTherapyNabhId;
...
import com.ahct.model.EhfmHospQuantityNabh;
...
public List<PreauthVO> getcaseSurgertDtls(String caseId)  
{
	...
	try{
		...
		query.append(" ,pm.hospstayAmt as hospStayAmt , pm.commonCatAmt as  commonCatAmt, pm.icdAmt as icdAmt, pm.noOfDays as noOfDays,case when ec.roboticSurgery = 'Y' then 'Y' else 'N' end  as robotSurgery ");
		...
		query.append(" where ec.caseId ='"+caseId+"' and ec.asriCatCode = cm.id.asriCatCode and ec.asriCatCode=pm.id.asriCode and ec.icdCatCode = cm.id.icdCatCode ");
		...
		query.append(" and (pm.id.state=ecc.schemeId or ecc.schemeId='1') and pm.id.process in ('IP','DOP','IPD') ");
		...
		preauthVO.setStatusToSave(ehfmWorkflowStatus.getNextStatus());
			...
			if(ehfCase != null)
			{
						...
						)
				{
					...
					StringBuffer	query = new StringBuffer();
					query.append(" select emt.asri_Code    as TEST,  emt.icd_Amt  as ICDAMT ,emt.icd_proc_code as  ICDPROCCODE ");
					query.append("   from EHFM_MAIN_THERAPY_NABH emt, Ehf_Case_Therapy ect, Ehf_Case ee ");
					query.append("  where ect.case_Id ='"+preauthVO.getCaseId()+"'   and ect.activeyn = 'Y' and  emt.icd_Amt is not null  and emt.asri_Code = ect.asri_Cat_Code ");
					query.append("  and ee.case_Id = ect.case_Id   and ect.rob_surg in ('Y')  and emt.state = ee.scheme_Id  and emt.icd_Proc_Code = ect.icd_Proc_Code and emt.icd_Cat_Code = ect.icd_Cat_Code ");
					query.append("  order by emt.asri_Code,to_number(emt.icd_Amt) desc ");
			 		List<PreauthVO> robotSurgList= genericDao.executeSQLQueryList(PreauthVO.class, query.toString());
			 		if(robotSurgList.size()>0)
			 		{
			 			roboticSurgery=true;
			 		}
					if( (totAmt > Integer.parseInt(config.getString("preauth_package_amt_limit"))) || roboticSurgery==true )
					{
															...
															if(lbLst.size()>0)
																{	
																	for(LabelBean lb:lbLst)
																		{
																			String smsText=null;
										    	 					     	smsText="Enhacement case with Case Number : "+ehfCase.getCaseId()+" has been Recommended for Approval by "+ehfmUsers.getFirstName()+" "+ehfmUsers.getLastName()+" , "+currUsr+" and the case is pending in your worklist.";
			...
			if(preauthVO.getEnhancementDtls() != null && !preauthVO.getEnhancementDtls().equalsIgnoreCase(""))
			{
				...
				if(preauthVO.getEnhancementDtls() != null && !preauthVO.getEnhancementDtls().equalsIgnoreCase(""))
				{
					...
					for(int i=0; i<str2.length;i++)
					{
						...
						ehfCaseEnhancementDtls.setEnhQuantCode(str3[2]);
						ehfCaseEnhancementDtls.setNoOfUnits(Integer.parseInt(str3[3]));
						ehfCaseEnhancementDtls.setAmount(Double.parseDouble(str3[4]));
						ehfCaseEnhancementDtls.setCrtDt(new java.sql.Timestamp(date.getTime()));
						ehfCaseEnhancementDtls.setCrtUser(preauthVO.getCruUsr());
						ehfCaseEnhancementDtls.setActiveYN(config.getString("activeY"));
						if(preauthVO.getAttachmentsList()!=null && preauthVO.getAttachmentsList().size() > 0)
						{
							if(preauthVO.getAttachmentsList().get(i)!=null)
							{
...

{
	...
	try
	{
		if(preauthVO.getCaseId()!=null && !"".equalsIgnoreCase(preauthVO.getCaseId()))
		{
				...
				if(preauthVO.getSplInvstId() != null && !preauthVO.getSplInvstId().equalsIgnoreCase(""))
				{
					...
					while(str.hasMoreTokens())
					{
						...
						while(str1.hasMoreTokens())
						{
							...
							ehfCaseTherapy.setRoboticSurgery(str1.nextToken());
							...
							EhfmMainTherapyNabhId ehfmMainTherapyNabhId=new EhfmMainTherapyNabhId(ehfCaseTherapy.getAsriCatCode(),ehfCaseTherapy.getIcdProcCode(), "CD202",process);
							EhfmMainTherapyNabh ehfmMainTherapyNabh=genericDao.findById(EhfmMainTherapyNabh.class,EhfmMainTherapyNabhId.class,ehfmMainTherapyNabhId);
	    	...
	    	query.append(" emt.noOfDays as noOfDays, emt.id.asriCode as test, emt.id.icdProcCode as testKnown, emt.procName as ipOpFlag, ect.procUnits as opProcUnits,emt.medical_surg as medicalSurgicalFlag ");
	    	...
	    	query.append(" and ee.caseId=ect.caseId and emt.id.state=ee.schemeId and   emt.icdAmt is not null ");
	    	query.append(" and emt.id.icdProcCode = ect.icdProcCode and emt.icdCatCode = ect.icdCatCode order by to_number(emt.icdAmt) desc ");
	    	...
	    	if(surgList!=null && surgList.size()>0)
	    	{
	    		for(int i=0; i<surgList.size(); i++)
	    		{
	    			...
	    			{
	    				if(surgList.get(i).getTestKnown().contains("S30.1.1"))
	    				{
	    					query= new StringBuffer();
	    					query.append(" select distinct emt.hospstayAmt as hospStayAmt, emt.commonCatAmt as commonCatAmt, emt.icdAmt as icdAmt,ee.schemeId as scheme, ");
	    			    	query.append(" emt.noOfDays as noOfDays, emt.id.asriCode as test, emt.id.icdProcCode as testKnown, emt.procName as ipOpFlag, ect.procUnits as opProcUnits,emt.medical_surg as medicalSurgicalFlag ");
	    			    	...
	    			    	query.append(" where ect.caseId = ? and ect.activeyn = ? and emt.id.asriCode = ect.asriCatCode  ");
	    			    	query.append(" and ee.caseId=ect.caseId and emt.id.state=ee.schemeId and   emt.icdAmt is not null ");
	    			    	query.append(" and ect.icdProcCode <> 'S30.1.1' ");
	    			    	query.append(" and emt.id.icdProcCode = ect.icdProcCode and emt.icdCatCode = ect.icdCatCode order by to_number(emt.icdAmt) desc ");
	    			    	args=new String[2];
	    			    	args[0]=preauthVO.getCaseId();
	    			    	args[1]="Y";
	    			    	surgList= genericDao.executeHQLQueryList(PreauthVO.class, query.toString(), args);
	    			    	sandostatinInj = true;
	    				}
	    	...
	    	if(surgList!=null && surgList.size()>0)
	    	{
	    		for(int i=0; i<surgList.size(); i++)
	    		{
	        		...
	        		//Modified by Namratha for surgical Calculations
	    			if(surgList.get(i).getIcdAmt()!=null && !"".equalsIgnoreCase(surgList.get(i).getIcdAmt()))
	    			{
	    				Long icdAmt=Long.parseLong(surgList.get(i).getIcdAmt());
	    				medicalSurgFlagNABH=surgList.get(i).getMedicalSurgicalFlag();
	        		if(surgList.get(i).getMedicalSurgicalFlag()!=null && ClaimsConstants.PREAUTH_SURGICAL.equalsIgnoreCase(surgList.get(i).getMedicalSurgicalFlag()))
	        		{	        	
	        			
	        			// To calculate Single procedure or max of multiple procedures
	        				if(i==0)
	        				{	        				
		        			if(surgList.get(i).getOpProcUnits()!=null && !"".equalsIgnoreCase(surgList.get(i).getOpProcUnits()) && !"-1".equalsIgnoreCase(surgList.get(i).getOpProcUnits()))
		        				surgAmt= surgAmt+ (icdAmt * Long.parseLong(surgList.get(i).getOpProcUnits()));
		        			else
		        				surgAmt= surgAmt+ icdAmt;
	        				}
	        				
	        				else if(i>0)
	        				{
	        					//75% for Knee pending Namratha
	        					/*if(surgList.get(i).getTestKnown()!=null && (surgList.get(i).getTestKnown().contains(ClaimsConstants.KNEE_REPLACEMENT_CODE) || surgList.get(i).getTestKnown().contains(ClaimsConstants.HIP_REPLACEMENT_CODE) ))
	        					{
	        						if(surgList.get(i).getOpProcUnits()!=null && !"".equalsIgnoreCase(surgList.get(i).getOpProcUnits()) && !"-1".equalsIgnoreCase(surgList.get(i).getOpProcUnits()))
				        				surgAmt= surgAmt+ (((icdAmt*3)/4) * Long.parseLong(surgList.get(i).getOpProcUnits()));
				        			else
				        				surgAmt= surgAmt+ ((icdAmt*3)/4);
	        					}*/
	        					
	        					//50% for multiple procedures
	        					if(surgList.get(i).getOpProcUnits()!=null && !"".equalsIgnoreCase(surgList.get(i).getOpProcUnits()) && !"-1".equalsIgnoreCase(surgList.get(i).getOpProcUnits()))
			        				surgAmt= surgAmt+ (icdAmt/2 * Long.parseLong(surgList.get(i).getOpProcUnits()));
			        			else
			        				surgAmt= surgAmt+ icdAmt/2;
			...
			query.append("  order by emt.asri_Code,to_number(emt.icd_Amt) desc ");
	 		...
	 		if(robotSurgList.size()>0)
	 		{
	 			robotSurgAmt=100000L;
	 			roboticSurgery=true;
	 		}
	    	//Added by Namratha for surgical calculation
	    	if(medicalSurgFlagNABH!=null && ClaimsConstants.PREAUTH_SURGICAL.equalsIgnoreCase(medicalSurgFlagNABH))
	    	{
	    		//totAmt= surgAmt;
	    		surgAmt=0L;
		        args=new String[2];
		    	...
		    	args[1]="Y";
		    	int flag=0;
		    	Boolean chekHosp=false;
		    	Long icdAmt=0L;
		    	query = new StringBuffer();
		    	query.append(" select ep.cardNo as patCardNo,ec.caseHospCode as hospitalCode  from EhfCase ec, EhfPatient ep  ");
		    	query.append(" WHERE ep.id.patientId = ec.casePatientNo  and ec.id.caseId = '"+preauthVO.getCaseId()+"' ");
		    	List<PreauthVO> presentHosp= genericDao.executeHQLQueryList(PreauthVO.class, query.toString());
		    	if(presentHosp.size()>0)
		    	{
		    		if(presentHosp != null)
		    		{
		    			...
		    			String cardNo ="";
		    			String hospId ="";
		    			cardNo=presentHosp.get(0).getPatCardNo();
		    			hospId=presentHosp.get(0).getHospitalCode();
		    			if(!"".equalsIgnoreCase(cardNo) && cardNo !=null)
		    			{
			    			lstrQuery.append(" select distinct e1.caseHospCode as hospitalCode from EhfCase e1, EhfPatient p1, EhfCaseTherapy t1 ");
			    			lstrQuery.append(" WHERE e1.casePatientNo = p1.id.patientId and e1.id.caseId = t1.caseId and e1.id.caseId not in ('"+preauthVO.getCaseId()+"')  and e1.preauthRejDate is null  ");
			    			lstrQuery.append(" and e1.preauthCancelDt is null and p1.cardNo = '"+cardNo+"' and t1.icdProcCode in ('81.54','81.55','81.56') and e1.csDisDt is null and e1.caseStatus not in ('CD34') and t1.activeyn='Y' and e1.caseHospCode = '"+hospId+"' ");
			    			List<PreauthVO> prevHosp= genericDao.executeHQLQueryList(PreauthVO.class, lstrQuery.toString());
			    			if(prevHosp.size()>0)
			    			{
			    				chekHosp=true;
			    			}
						...
						query.append(" select emt.asri_Code    as TEST,  emt.icd_Amt  as ICDAMT ,emt.icd_proc_code as  ICDPROCCODE ");
						query.append("   from EHFM_MAIN_THERAPY_NABH emt, Ehf_Case_Therapy ect, Ehf_Case ee ");
						query.append("  where ect.case_Id =?   and ect.activeyn = ? and  emt.icd_Amt is not null  and emt.asri_Code = ect.asri_Cat_Code ");
						if(sandostatinInj == true)
							query.append(" and ect.icd_Proc_Code <> 'S30.1.1' ");
						query.append("  and ee.case_Id = ect.case_Id     and emt.state = ee.scheme_Id  and emt.icd_Proc_Code = ect.icd_Proc_Code and emt.icd_Cat_Code = ect.icd_Cat_Code ");
						query.append("  order by emt.asri_Code,to_number(emt.icd_Amt) desc ");
				 		List<PreauthVO> amtList= genericDao.executeSQLQueryList(PreauthVO.class, query.toString(), args);
						query = new StringBuffer();
						query.append("  select distinct emt.asri_Code as TEST from EHFM_MAIN_THERAPY_NABH emt, Ehf_Case_Therapy ect, Ehf_Case ee  where ect.case_Id = ? ");
						if(sandostatinInj == true)
							query.append(" and ect.icd_Proc_Code <> 'S30.1.1' ");
						query.append("  and ect.activeyn = ?  and  emt.icd_Amt is not null and emt.asri_Code = ect.asri_Cat_Code and ee.case_Id = ect.case_Id ");
						query.append("  and emt.state = ee.scheme_Id and emt.icd_Proc_Code = ect.icd_Proc_Code and emt.icd_Cat_Code = ect.icd_Cat_Code order by emt.asri_Code ");  		
				 		List<PreauthVO> surgList1= genericDao.executeSQLQueryList(PreauthVO.class, query.toString(), args);
				 		String icdProcCode="";
				 		List<String> icdList=new ArrayList<String>();
				 		if(config.getString("Preauth.OrthoSurgProcs")!=null)
							{								
								StringTokenizer str1=new StringTokenizer(config.getString("Preauth.OrthoSurgProcs"),"~");
								while(str1.hasMoreTokens()){
									String id=str1.nextToken();
									if(id!=null)
										icdList.add(id);
								}
							}
				 		if((amtList!=null && amtList.size()>0) && (surgList1!=null && surgList1.size()>0) )
				 		{				 			
				 				for(int j=0;j<surgList1.size();j++)
				 				{ 
				 					flag=0;
				 					for(int i=0;i<amtList.size();i++)
						 			{
				 						if(amtList.get(i).getICDAMT()!=null)
				 						{
				 							icdAmt=Long.parseLong(amtList.get(i).getICDAMT());
				 						}
				 						if(amtList.get(i).getICDPROCCODE()!=null)
				 						{
				 							icdProcCode=amtList.get(i).getICDPROCCODE();
				 						}
				 					if(amtList.get(i).getTEST().equals(surgList1.get(j).getTEST()))
				 					{
				 						flag++;
				 						if(flag==1)
				 						{				 							
				 							if(icdList!=null)
				 							{
					 							if(icdList.contains(icdProcCode) && chekHosp==false)
					 								surgAmt=surgAmt+icdAmt;	
					 							else if(icdList.contains(icdProcCode) && chekHosp==true)
					 								surgAmt=surgAmt+((icdAmt)/2);	
					 							else
					 								surgAmt=surgAmt+icdAmt;
				 							}
				 						...
				 						{
				 							//Added for ortho knee (left,right) and hip replacement
				 						
				 							if(icdList!=null)
				 							{	
				 								/*if(icdList.contains(icdProcCode))
				 									surgAmt=surgAmt+((icdAmt*3)/4);	
				 								else*/
				 									surgAmt=surgAmt+(icdAmt/2);	
				 							}
				 				...
				 				if(sandostatinInj == true)
				 				{
				 					Long sandostatinAmt=38000L;
				 					totAmt= surgAmt+robotSurgAmt+sandostatinAmt;
				 				}
				 				else
				 				totAmt= surgAmt+robotSurgAmt;
				 		}
				 		else
				 		{
				 			totAmt= 0L;
	    	...
	    	{
	    		totAmt= surgAmt+hospStayAmount+commonCatAmount+(noOfDays*nabhAmt)+robotSurgAmt;
			...
			SimpleDateFormat sdfw=new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
			SimpleDateFormat sdfw1=new SimpleDateFormat("dd/MM/yyyy");
		    String crtDate=sdfw.format(date);
		    Date crtDt = sdfw.parse((sdfw.format(date)));
		    
		    if(ehfCase != null && ehfmWorkflowNextStatus !=null)
			{
		    	ehfCase.setCsAdmDt(sdfw1.parse(preauthVO.getAdmissionDate()));
	    	 ...
	    	 */
	    	 if(currStatus!=null && currStatus.equalsIgnoreCase(config.getString("preauth_inpatient_caseReg"))
	    			 ...
	    			 && ("IP".equalsIgnoreCase(preauthVO.getIpOpFlag()) || "IPD".equalsIgnoreCase(preauthVO.getIpOpFlag())) )
	    		 
	    	 	{
	    		 	boolean statusGovTG=checkTGPatReg(ehfCase.getCaseHospCode(),ehfCase.getCaseId());
	    		 	if(statusGovTG==true ||  "IPD".equalsIgnoreCase(preauthVO.getIpOpFlag()))
	    		 		{
	    		 			/**
		    				 ...
		    				 if(lstClaimDtls != null && lstClaimDtls.size() >0)
		    				 	{
			    					 ...
			    					 locQuery.append(" and ep.patientIpop in ('IP','IPD') and ec.caseId  not in ('"+ehfCase.getCaseId()+"') and ec.claimNo is not null  ");
		    					...
		    					if(ehfCasePreauthAmounts != null)
		    						{
		    							...
		    							if( (totAmtAppr > Integer.parseInt(config.getString("preauth_package_amt_limit"))) || roboticSurgery==true )
		    								{
			...
			if(ehfmWorkflowNextStatus != null)
			{
				...
				if(asrimCombo != null)
				{   preauthVO.setNextStatusMedical(ehfmWorkflowNextStatus.getNextStatus());
			...
			EhfCase ehfCase1 = genericDao.findById(EhfCase.class, String.class, preauthVO.getCaseId());
			if(ehfCase1!=null&& ehfCase1.getCaseStatus()!=null&&!ehfCase1.getCaseStatus().equalsIgnoreCase("")&&ehfCase1.getCaseStatus().equalsIgnoreCase("CD651"))
...
public String checkMandatoryAttch(String pStrCaseId , String pStrattachType,String ipFlag,String drugFlag)
{
					...
					if(drugFlag!=null&&drugFlag.equals("Y")){
						query.append(" and cm.cmbDtlId  in ('CD1263')");
					}
...
private String saveAstitAudit(PreauthVO preauthVO)
{
	...
	if("IPM".equalsIgnoreCase(preauthVO.getIpOpFlag()))
	{
		...
		if(tempList.size() > 0)
		  ...
		  List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>(); 					
			 criteriaList.add(new GenericDAOQueryCriteria("caseId",preauthVO.getCaseId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));

			 List<EhfCase> ehfCase = genericDao.findAllByCriteria(EhfCase.class, criteriaList);
			 if(ehfCase!=null){
			 if(ehfCase.get(0).getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCase.get(0).getCriticalFlg())&&"Y".equalsIgnoreCase(ehfCase.get(0).getCriticalFlg())&&Integer.parseInt(preauthVO.getPreauthTotalPackageAmt())>100000){
				 ehfAudit.setActId(preauthVO.getStatusToSave());
				}
				else if(ehfCase.get(0).getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCase.get(0).getCriticalFlg())&&"N".equalsIgnoreCase(ehfCase.get(0).getCriticalFlg())&&Integer.parseInt(preauthVO.getPreauthTotalPackageAmt())>50000){
					ehfAudit.setActId(preauthVO.getStatusToSave());
				}else{
					if((preauthVO.getNextStatusMedical()!=null&&!preauthVO.getNextStatusMedical().equalsIgnoreCase("") &&preauthVO.getNextStatusMedical().equalsIgnoreCase(config.getString("preauth_partail_save"))))
						ehfAudit.setActId(preauthVO.getNextStatusMedical());
					else
...
private String updateAsritCase(PreauthVO preauthVO)
{
	...
	{   List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>(); 
		...
		//EhfCaseTherapy ehfCasetherapy = genericDao.findById(EhfCaseTherapy.class, String.class, preauthVO.getCaseId());
		...
		if(ehfCase != null)
		{
				...
				if(preauthVOLst.size()>0)
					{
						...
						if(nabhFlg!=null && (preauthVO.getStatusToSave().equalsIgnoreCase(config.getString("preauth_medco_initiated"))
								...
								|| preauthVO.getStatusToSave().equalsIgnoreCase(config.getString("preauth_organ_group_pending_updated"))
								|| preauthVO.getStatusToSave().equalsIgnoreCase(config.getString("preauth_organ_ptd_pending_updated"))
								|| preauthVO.getStatusToSave().equalsIgnoreCase(config.getString("preauth_organ_eo_pending_updated"))
								|| preauthVO.getStatusToSave().equalsIgnoreCase(config.getString("preauth_organ_ceo_pending_updated"))
			...
			if("IPM".equalsIgnoreCase(preauthVO.getIpOpFlag()))
			{
					...
					if(ehfCasePreauthAmounts != null)
						{
							...
							ehfCasePreauthAmounts.setPtdAppvTotalPackgAmt(totAmtAppr);
							if(((totAmtAppr > Integer.parseInt(config.getString("preauth_package_amt_limit"))) || roboticSurgery==true ) && (ehfPatient!=null && ehfPatient.getPatientIpop()!=null && "IP".equalsIgnoreCase(ehfPatient.getPatientIpop())))
								{
    								ehfCase.setCeoApprovalFlag(config.getString("activeY"));
									preauthVO.setStatusToSave(config.getString("Preauth.Medco.Recommend.Approve"));
							...
							else if(((totAmtAppr > Integer.parseInt(config.getString("preauth_package_amt_limit"))) || roboticSurgery==true ) && (ehfPatient!=null && ehfPatient.getPatientIpop()!=null && "IPM".equalsIgnoreCase(ehfPatient.getPatientIpop())))
							{
								ehfCase.setCeoApprovalFlag(config.getString("activeY"));
								//preauthVO.setStatusToSave(config.getString("Preauth.Medco.Recommend.Approve"));
				...
				//}
				if (!"".equalsIgnoreCase(asriCatCode) && ehfCase.getCriticalFlg()!=null&&!ehfCase.getCriticalFlg().equalsIgnoreCase("")&&ehfCase.getCriticalFlg().equalsIgnoreCase("Y"))
				{
							...
							SMSServices SMSServicesobj = new SMSServices();
   							...
   							if(hospDtls != null)
   							{
   								...
   								{	
									 ...
									 SMSServicesobj.sendSingleSMS(msg,cntctNo[i] );
   								}
   							}
					     }*/
					}
				}
				else if (!"".equalsIgnoreCase(asriCatCode)&&ehfCase.getCriticalFlg()!=null&&!ehfCase.getCriticalFlg().equalsIgnoreCase("")&&ehfCase.getCriticalFlg().equalsIgnoreCase("N"))
				{
				...
				if(ehfCase.getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCase.getCriticalFlg())&&"Y".equalsIgnoreCase(ehfCase.getCriticalFlg())&&Integer.parseInt(preauthVO.getPreauthTotalPackageAmt())>100000){
					ehfCase.setCaseStatus(preauthVO.getStatusToSave());
				}
				else if(ehfCase.getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCase.getCriticalFlg())&&"N".equalsIgnoreCase(ehfCase.getCriticalFlg())&&Integer.parseInt(preauthVO.getPreauthTotalPackageAmt())>50000){
					ehfCase.setCaseStatus(preauthVO.getStatusToSave());
				}else{
					if((preauthVO.getNextStatusMedical()!=null&&!preauthVO.getNextStatusMedical().equalsIgnoreCase("") &&preauthVO.getNextStatusMedical().equalsIgnoreCase(config.getString("preauth_partail_save"))))
						ehfCase.setCaseStatus(preauthVO.getStatusToSave());
	...
	if(preauthVO.getCaseId()!=null && !"".equalsIgnoreCase(preauthVO.getCaseId()))
	{
		...
		if(ehfmWorkflowStatus != null)
		{
			...
			if(ehfmWorkflowStatus.getNextStatus() != null && (ehfmWorkflowStatus.getNextStatus().equalsIgnoreCase(config.getString("preauth_nam_forwarded"))
					...
					|| ehfmWorkflowStatus.getNextStatus().equalsIgnoreCase(config.getString("preauth_Organ_ppd_approved"))
		...
		if(ehfCase != null && ehfmWorkflowStatus != null)
		{
			...
			if(ehfmWorkflowStatus.getNextStatus() != null && (ehfmWorkflowStatus.getNextStatus().equalsIgnoreCase(config.getString("preauth_ptd_approved"))))
			{
				...
				if(scheme!=null &&
						...
						||
						(config.getString("TG").equalsIgnoreCase(scheme)&&(totAmt > Integer.parseInt(config.getString("preauth_package_amt_limit_TG"))))) || roboticSurgery==true
						)
					{
						ehfCase.setCeoApprovalFlag(config.getString("activeY"));
			...
			StringBuffer claimNum = new StringBuffer();
			 ...
			 String lStrYear = cal.get ( Calendar.YEAR ) + "" ;
			  query = new StringBuffer();
			 query.append(" select lm.locShrtName as ID ,ep.districtCode as VALUE  ,ep.patientScheme as PATIENTSCHEME,ep.cardNo as LVL ");
			...
			if(lstClaimDtls != null && lstClaimDtls.size() >0)
			{
				...
				String paitentSchemetype =null;
				 ...
				 query = new StringBuffer();
				 query.append(" select ec.preauthFwdDate as crtDt , ec.claimNo as ID , ep.patientScheme as PATIENTSCHEME ,ep.patientIpop as VALUE, year(ec.preauthFwdDate) as COUNT ,current_date() as crtDt  ");
				 query.append(" from EhfCase ec , EhfPatient ep where ec.casePatientNo = ep.patientId and ep.cardNo ='"+lstClaimDtls.get(0).getLVL()+"' ");
				 query.append(" and ep.patientIpop in ('IP','IPD') and ec.caseId  not in ('"+ehfCase.getCaseId()+"') and ec.claimNo is not null  ");
				 ...
				 if(lstClaimDtls.size()>0)
				 paitentSchemetype=lstClaimDtls.get(0).getPATIENTSCHEME();
				 gLogger.info(paitentSchemetype);
		...
		if(ehfmWorkflowStatus.getNextStatus() != null && !"".equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus()) &&
					(config.getString("preauth_ppd_approved").equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus()) || config.getString("preauth_Organ_ppd_approved").equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus())||
							config.getString("preauth_Organ_ppd_reject").equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus())||
							config.getString("preauth_Organ_ppd_pending").equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus())||
								...
								config.getString("preauth_ppd_pending").equalsIgnoreCase(ehfmWorkflowStatus.getNextStatus())))
		{
			...
			if(seqVal!=null && !"".equalsIgnoreCase(seqVal))
			{
				...
				if(("Y").equals(organTrans))
				{
					if((currStatus.equalsIgnoreCase(config.getString("preauth_pex_forwarded")) || currStatus.equalsIgnoreCase(config.getString("preauth_ppd_organ_pending_updated")) ) && preauthVO.getButtonVal()!=null && "CD32".equalsIgnoreCase(preauthVO.getButtonVal()))
					{
						ehfPanelDocCaseDtls.setCasePmntStatus(config.getString("preauth_ppd_organ_approve"));
					}
				/*	if(currStatus.equalsIgnoreCase(config.getString("preauth_pex_forwarded")) && preauthVO.getButtonVal()!=null && "CD31".equalsIgnoreCase(preauthVO.getButtonVal()))
					{
						preauthVO.setStatusToSave(config.getString("preauth_ppd_organ_reject"));    //verifying organ case by PEX
					...
					{
						ehfPanelDocCaseDtls.setCasePmntStatus(config.getString("preauth_ppd_organ_reject"));
					}
					else if(currStatus.equalsIgnoreCase(config.getString("preauth_pex_forwarded")) && preauthVO.getButtonVal()!=null && "CD31".equalsIgnoreCase(preauthVO.getButtonVal()))
					{
						ehfPanelDocCaseDtls.setCasePmntStatus(config.getString("preauth_ppd_organ_pending"));
					}
														...
														if(lbLst!=null)
															{
																if(lbLst.size()>0)
																	{	
																		for(LabelBean lb:lbLst)
																			{
																			    templateId="1407161769818615014";
																				String smsText=null;
...
public String checkClinicalNotes(String pStrCaseId)
{
	...
	try
	{
		...
		long startTime =0L;
		query.append(" select a.caseRegnDate as date  from EhfCase a where a.caseId ='"+pStrCaseId+"' " );
...
/*
 * @Author Sravan
 * Retrieving total Medical Expenses amount
 */
public ClaimsFlowVO getInvestigationDetails(ClaimsFlowVO claimsFlowVO){
	...
	if(investDetails != null && investDetails.size() > 0 ){
		...
		claimsFlowVO.setTotalConsumableAmount(appvAmt);
	return claimsFlowVO;
}
//To get drug details
public ClaimsFlowVO getDrugDetailsData(ClaimsFlowVO claimFlowVO) {
	...
	else
	{
		...
		claimFlowVO.setTotalDrugAmt(appvAmt);
	}
	return claimFlowVO;
}
	...
	try
	{
		if(caseId!=null && !"".equalsIgnoreCase(caseId))
		{
			...
			{   if(ehfCase.getPckAppvAmt()!=null)
				preauthVO.setPREAUTHAPPVAMT(ehfCase.getPckAppvAmt());
				...
				preauthVO.setHoldRejectFlag(ehfCase.getHoldRejectFlag());
				preauthVO.setConsumableFlg(ehfCase.getConsumableFlg());
				if(ehfCase.getEnhReqAmt() != null)
				preauthVO.setEnhAmt(ehfCase.getEnhReqAmt().toString());
				if(ehfCase.getEnhAmounts() != null)
					preauthVO.setEnhAmt(ehfCase.getEnhAmounts().toString());
				...
				if(ehfCase.getCsDeathDt() != null && !ehfCase.getCsDeathDt().equals("") )
					preauthVO.setDeathDate(sdf.format(ehfCase.getCsDeathDt()));
...
public List<LabelBean> getHospStayist(String pStrenhType)
{
	...
	try
	{
		query.append(" select a.id.hospStayId as ID , a.stayType as VALUE   from EhfmHospStay a where a.enhType ='"+pStrenhType+"' order by a.stayType asc ");
...
public List<LabelBean> getHospStayQuantList(String pStrenhTypeId)
{
	...
	try
	{
		query.append(" select a.quantId as ID , a.quantName as VALUE   from EhfmHospQuantityNabh a where a.enhTypeId='"+pStrenhTypeId+"' and a.activeYn='Y' order by a.quantName asc ");
 ...
 public Integer getQuantAmount(String quantId)
 {
	 ...
	 try
	 {
		 EhfmHospQuantityNabh ehfmHospQuantity = genericDao.findById(EhfmHospQuantityNabh.class, String.class, quantId); 
 ...
 public List<PreauthVO> getEnhancementList(String pStrcaseId,String patientType)
 {
	 ...
	 try
	 {
		 query.append("  select ee.type as enhType ,hs.stayType as enhCode , qm.quantName as enhQuant , cast(ee.noOfUnits as string ) as enhUnits , cast(ee.amount as string ) as enhAmt,  ee.newConsumableFlg as newConsumableFlg, ee.newAttachpath as attachPathNew ");	
...
public List<LabelBean> getSymptomsDtls(String lStrCaseId) {
	...
	try{
		...
		query.append(" and ESD.id.caseId=?");
		symptomList = genericDao.executeHQLQueryList(LabelBean.class, query.toString(),args1);
	}catch(Exception ex){
		ex.printStackTrace();
	}
	return symptomList;
...
public PreauthVO getCaseDetails(String caseId)
{
	...
	if(ehfCase!=null && ehfCasePreauthAmounts!=null && ehfCaseClaim!=null )
	{
...
public String updateSentBackClaims(ClaimsFlowVO claimFlowVO)
{
	...
	if(!("followUp").equalsIgnoreCase(claimType))
	{
		...
		if(claimFlowVO.getPreauthType()!=null&&!claimFlowVO.getPreauthType().equalsIgnoreCase("")&&claimFlowVO.getPreauthType().equalsIgnoreCase("hold"))
			nextStatus=config.getString("EHF.preauth.CEOHoldPendingUpdated");
		else
...
claimStatus =  ClaimsConstants.preauthCEOSentBackStatuses;
		...
		query.append(" )) or ");
		query.append("ec.case_id='"+caseId+"' and ( ec.case_status in (") ;
holdStatus =  ClaimsConstants.holdSentBackStatuses;
		for (int i = 0; i < holdStatus.length; i++) {
			query.append(" '" + holdStatus[i] + "' ");
			if (i != holdStatus.length - 1)
				query.append(",");
		}
		query.append(" ) ) " );
...
public String getNABHhosptls(String intiid, String caseNo) {
	...
	public String getHospScheme(String caseId) {
		...
		nimsHospId=preauthDtlsDao.getHospScheme(caseId);
		return nimsHospId;
	}
...
//						gLogger.error("Exception Occurred in checkTGPatReg() in PreauthServiceImpl class."+e.getMessage());
		...
		public List<CasesSearchVO> getPreauthDtls(String cardNo) 
		{
			...
			query.append(" and epe.patient_ipop in ('IP','IPD') and epe.scheme_id = 'CD202' and epe.card_no='"+cardNo+"') ");
		...
		public PreauthVO getPatientFullDtls(String lStrCaseId, String patientId,String fromDisp) {
			PreauthVO preauthVO = preauthDtlsDao.getPatientFullDtls(lStrCaseId,patientId,fromDisp);
			return preauthVO;
		}
		...
		try{
			...
			EhfCasePreauthAmounts ehfcaseAmounts = genericDao.findById(EhfCasePreauthAmounts.class, String.class, preauthVO.getCaseId());
			  List<GenericDAOQueryCriteria> criteriaList1 = new ArrayList<GenericDAOQueryCriteria>(); 					
				 criteriaList1.add(new GenericDAOQueryCriteria("caseId",preauthVO.getCaseId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));

				 List<EhfCaseTherapy> ehfCasetherapy = genericDao.findAllByCriteria(EhfCaseTherapy.class, criteriaList1);
			if(ehfcaseAmounts.getPtdAppvTotalPackgAmt()!=null&&!"".equalsIgnoreCase(ehfcaseAmounts.getPtdAppvTotalPackgAmt().toString()))
			{
				preauthVO.setPREAUTHAPPVAMT(Double.valueOf(ehfcaseAmounts.getPtdAppvTotalPackgAmt()).longValue());
				preauthVO.setPreauthAppvDt(ehfcaseAmounts.getPtdAppvDate().toString());
			}
			else if(ehfcaseAmounts.getCeoAppvTotalAmt()!=null&&!"".equalsIgnoreCase(ehfcaseAmounts.getCeoAppvTotalAmt().toString())){
				preauthVO.setPREAUTHAPPVAMT(Double.valueOf(ehfcaseAmounts.getCeoAppvTotalAmt()).longValue());
				preauthVO.setPreauthAppvDt(ehfcaseAmounts.getCeoappvDt().toString());
			}

			if(ehfCaseObj != null&&!ehfCaseObj.getCaseStatus().equalsIgnoreCase("CD2589"))
			{  
/*				if(Long.valueOf(preauthVO.getPREAUTHAPPVAMT())!=null&&preauthVO.getPREAUTHAPPVAMT()>0){
					if (((!"".equalsIgnoreCase(asriCatCode) && asriCatCode.contains("MMC"))&& (Long.parseLong(preauthVO.getEnhAmt()) >= 100000)&& (Long.valueOf(preauthVO.getPREAUTHAPPVAMT()) < Long.parseLong(preauthVO.getEnhAmt())))
							||((!"".equalsIgnoreCase(asriCatCode) &&  asriCatCode.contains("MMNC")) && (Long.parseLong(preauthVO.getEnhAmt()) >= 50000)&&(Long.valueOf(preauthVO.getPREAUTHAPPVAMT()) < Long.parseLong(preauthVO.getEnhAmt()))) )
					{
						
						ehfCaseObj.setExceedFlg("Y");
						if(preauthVO.getEnhAmounts() != null)
						{		
							
								ehfCaseObj.setCaseStatus(PreauthConstants.HOLD_STATUS);
								ehfCaseObj.setAmountLockedFor(Long.parseLong(preauthVO.getEnhAmt()));
								Long actOrder = (long) 0;
								List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>();
				...
				if( ((!"".equalsIgnoreCase(asriCatCode) &&  asriCatCode.contains("MMC")) && (Long.parseLong(preauthVO.getEnhAmt()) >= 100000))   || 
						((!"".equalsIgnoreCase(asriCatCode) &&  asriCatCode.contains("MMNC")) && (Long.parseLong(preauthVO.getEnhAmt()) >= 50000)) )
					
				{
					...
					{		
						if( (Long.parseLong(preauthVO.getEnhAmounts()) < 200000 &&  Long.parseLong(preauthVO.getEnhAmt()) >= 200000) ||
								...
								(Long.parseLong(preauthVO.getEnhAmounts()) < 1200000 &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1200000) ||
								(Long.parseLong(preauthVO.getEnhAmounts()) < 1500000 &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1500000) ||
								(Long.parseLong(preauthVO.getEnhAmounts()) < 1700000 &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1700000) ||
								(Long.parseLong(preauthVO.getEnhAmt()) >= 2000000) )
						if(	(ehfCaseObj.getAmountLockedFor() == null && Long.parseLong(preauthVO.getEnhAmt()) >= 200000) || 
								(ehfCaseObj.getAmountLockedFor() == null && asriCatCode.contains("MMC") && (Long.parseLong(preauthVO.getEnhAmt()) >= 100000)) ||
								(ehfCaseObj.getAmountLockedFor() == null && asriCatCode.contains("MMNC") && (Long.parseLong(preauthVO.getEnhAmt()) >= 50000) ) ||
						(	((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 200000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 200000) ||
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 500000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 500000) || 	
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 700000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 700000) || 
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1000000) &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1000000) ||
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1200000) &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1200000) ||
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1500000) &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1500000) ||
							((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1700000) &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1700000) ||
							(Long.parseLong(preauthVO.getEnhAmt()) >= 2000000) ) )
				...
				if((!"".equalsIgnoreCase(ehfCaseObj.getCaseStatus()) &&  ehfCaseObj.getCaseStatus().contains("CD8"))   || 
				((!"".equalsIgnoreCase(ehfCaseObj.getCaseStatus()) &&  ehfCaseObj.getCaseStatus().contains("CD207")))){
					
					if((Long.valueOf(preauthVO.getPREAUTHAPPVAMT()) < Long.parseLong(preauthVO.getEnhAmt()))){
						if( ((!"".equalsIgnoreCase(asriCatCode) && ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("Y")))   || 
								((!"".equalsIgnoreCase(asriCatCode) && ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("N"))) )
							
						{
							...
							{		
								...
								if(	(ehfCaseObj.getAmountLockedFor() == null && Long.parseLong(preauthVO.getEnhAmt()) >= 200000) || 
								(	((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 200000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  200000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 400000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  400000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 600000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  600000) ||
							        ((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 800000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  800000) ||
                                    ((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1000000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1000000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1100000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1100000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1200000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1200000) || 	
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1300000)  &&  Long.parseLong(preauthVO.getEnhAmt()) >= 1300000) || 
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1400000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1400000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1500000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1500000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1600000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1600000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1700000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1700000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1800000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1800000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 1900000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  1900000) ||
									((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 2000000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  2000000) ||
									(Long.parseLong(preauthVO.getEnhAmt()) >= 2000000) ) )
					...
					if( ((!"".equalsIgnoreCase(asriCatCode) &&  ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("Y")) && (Long.parseLong(preauthVO.getEnhAmt()) >= 100000)&&(ehfCaseObj.getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCaseObj.getCriticalFlg())) )   || 
							((!"".equalsIgnoreCase(asriCatCode)&& ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("N")) && (Long.parseLong(preauthVO.getEnhAmt()) >= 50000)&&(ehfCaseObj.getCriticalFlg()!=null&&!"".equalsIgnoreCase(ehfCaseObj.getCriticalFlg()))) )
						
					{
						...
						{		
							/*if( (Long.parseLong(preauthVO.getEnhAmounts()) < 200000 &&  Long.parseLong(preauthVO.getEnhAmt()) >= 200000) ||
									...
									(Long.parseLong(preauthVO.getEnhAmt()) >= 2000000) )*/
							if(	(ehfCaseObj.getAmountLockedFor() == null && Long.parseLong(preauthVO.getEnhAmt()) >= 200000) || 
									(ehfCaseObj.getAmountLockedFor() == null && ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("Y") && (Long.parseLong(preauthVO.getEnhAmt()) >= 100000)) ||
									(ehfCaseObj.getAmountLockedFor() == null && ehfCaseObj.getCriticalFlg()!=null&&!ehfCaseObj.getCriticalFlg().equalsIgnoreCase("")&&ehfCaseObj.getCriticalFlg().equalsIgnoreCase("N") && (Long.parseLong(preauthVO.getEnhAmt()) >= 50000) ) ||
							   (	((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 200000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  200000) ||
										((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 400000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  400000) ||
                                        ((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 600000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  600000) ||
                                        ((ehfCaseObj.getAmountLockedFor() != null && ehfCaseObj.getAmountLockedFor() < 800000) &&  Long.parseLong(preauthVO.getEnhAmt()) >=  800000) ||
			...
			if(preauthVO.getEnhancementDtls() != null && !preauthVO.getEnhancementDtls().equalsIgnoreCase(""))
			{
				...
				if(preauthVO.getEnhancementDtls() != null && !preauthVO.getEnhancementDtls().equalsIgnoreCase(""))
				{
					...
					for(int i=0; i<str2.length;i++)
					{
						...
						if(ehfCaseObj.getCaseStatus().equalsIgnoreCase("CD2589"))
						ehfCaseEnhancementDtls.setNewConsumableFlg(config.getString("activeY"));
						genericDao.save(ehfCaseEnhancementDtls);
						msg="Success";
					}
...
//		    		 gLogger.error("CaseId"+preauthVO.getCaseId());
		        ...
		        EhfCase asritcase=genericDao.findById(EhfCase.class,String.class,preauthVO.getCaseId());
		             ...
		             if( preauthVO.getIpNumber()!=null)
				        	asritcase.setIpNo(preauthVO.getIpNumber());  
		        ...
		        if(preauthVO.getDeathDate() != null && !"".equalsIgnoreCase(preauthVO.getDeathDate()))
		        	asritcase.setCsDeathDt(sdfw.parse(preauthVO.getDeathDate()));
		     //   if(preauthVO.getEnhAmt() != null)
...
//		    	gLogger.error("Error in saveDischargeSummaryDtls() of PreauthClinicalNotesDAOImpl"+e.getMessage());        
		...
		public String holdUnholdCase(PreauthVO preauthVO){
			...
			if(ehfcase!=null)
			{
				if(preauthVO.getButtonVal() != null && preauthVO.getButtonVal().trim().length() > 0 && (PreauthConstants.HOLD_STATUS).equalsIgnoreCase(preauthVO.getButtonVal()))
				{
					...
					ehfcase.setExceedFlg("Y");
					if(caseStatus !=null && caseStatus.equalsIgnoreCase("HD1") && preauthVO.getUserRole()!=null && !preauthVO.getUserRole().equalsIgnoreCase("GP2"))
					{
						ehfcase.setHoldPd("Y");
					}
				}
				else if(preauthVO.getButtonVal() != null && preauthVO.getButtonVal().trim().length() > 0 && (PreauthConstants.Release).equalsIgnoreCase(preauthVO.getButtonVal()))
				{
					caseStatus = PreauthConstants.MEDCO_PREAUTH_APPROVED_STATUS;
					ehfcase.setExceedFlg("");
					
				}
				else if(preauthVO.getButtonVal() != null && preauthVO.getButtonVal().trim().length() > 0 && (PreauthConstants.verify).equalsIgnoreCase(preauthVO.getButtonVal()))
				{
					caseStatus = PreauthConstants.HOLD_CASE_VRIFIED_BY_EO;
					ehfcase.setExceedFlg("Y");
					
				}
				else if(preauthVO.getButtonVal() != null && preauthVO.getButtonVal().trim().length() > 0 && (PreauthConstants.pending).equalsIgnoreCase(preauthVO.getButtonVal()))
				{
					caseStatus = PreauthConstants.HOLD_CASE_PENDING_TO_MEDCO_BY_EO;
					ehfcase.setExceedFlg("Y");
					
				}
				else if(preauthVO.getButtonVal() != null && preauthVO.getButtonVal().trim().length() > 0 && (PreauthConstants.reject).equalsIgnoreCase(preauthVO.getButtonVal()))
				{
					caseStatus = PreauthConstants.EO_RECOMMEND_DISCHARGE;
					ehfcase.setExceedFlg("");
					ehfcase.setHoldRejectFlag("Y");
		...
		/*			ehfCasePreauthAmounts = genericDao.findById(EhfCasePreauthAmounts.class,String.class,preauthVO.getCaseId());
						...
						{*/
							caseStatus = PreauthConstants.SEND_HOLD_CASE_TO_EO;
							ehfcase.setExceedFlg("Y");
					/*	}*/
				...
				asritAudit.setLangId("en_US");
				genericDao.save(asritAudit);
				if(preauthVO.getHospitalId()!=null && !"".equalsIgnoreCase(preauthVO.getHospitalId()) && !"EHS34".equalsIgnoreCase(preauthVO.getHospitalId()))
				{
				if((preauthVO.getUserRole()!=null && preauthVO.getUserRole().equalsIgnoreCase("GP156")) && (caseStatus!=null && (caseStatus.equalsIgnoreCase("HD1") || caseStatus.equalsIgnoreCase("CD3241"))))
			{
				String seqVal=commonService.getSequence("panel_doc_pmnt_seq");
				if(seqVal!=null && !"".equalsIgnoreCase(seqVal))
				{
					...
					ehfPanelDocCaseDtls.setCaseId(preauthVO.getCaseId());
					ehfPanelDocCaseDtls.setDocId(preauthVO.getCruUsr());
					ehfPanelDocCaseDtls.setCasePmntStatus(caseStatus);
					ehfPanelDocCaseDtls.setCaseAppOrPendDate(lCurrentDate1);
					ehfPanelDocCaseDtls.setHospId(ehfcase.getCaseHospCode());
					ehfPanelDocCaseDtls.setScheme("CD202");
					ehfPanelDocCaseDtls.setCrtUsr(preauthVO.getCruUsr());
					ehfPanelDocCaseDtls.setCrtDt(lCurrentDate1);
					genericDao.save(ehfPanelDocCaseDtls);
				}
			}
				}
...
//		    		 gLogger.error("CaseId"+preauthVO.getCaseId());
			...
			if(preauthVO.getCaseStatusId() != null && preauthVO.getCaseStatusId().trim().length() > 0 && (PreauthConstants.HOLD_CASE_PENDING_TO_MEDCO_BY_EO).equalsIgnoreCase(preauthVO.getCaseStatusId()))
			{
				casestus=PreauthConstants.HOLD_CASE_EO_PENDING_UPDATE_BY_MEDCO;
			}
			else if(preauthVO.getCaseStatusId() != null && preauthVO.getCaseStatusId().trim().length() > 0 && (PreauthConstants.HOLD_CASE_PENDING_TO_MEDCO_BY_CEO).equalsIgnoreCase(preauthVO.getCaseStatusId()))
			{
				casestus=PreauthConstants.HOLD_CASE_CEO_PENDING_UPDATE_BY_MEDCO;
			}else{
				casestus="CD324";
			}
		        EhfCase asritcase=genericDao.findById(EhfCase.class,String.class,preauthVO.getCaseId());
		        		
		                asritcase.setCaseStatus(casestus);
		        		asritcase.setLstUpdUsr(preauthVO.getCruUsr());
		            
		            EhfAudit asritAudit=new EhfAudit();
		            EhfAuditId asritAuditPK=new EhfAuditId(lActOrder,preauthVO.getCaseId());
		             asritAudit.setId(asritAuditPK);
		             asritAudit.setActId(casestus);
...
//		    	gLogger.error("Error in saveEnhanceAmounts() of PreauthServiceImpl"+e.getMessage());        
		...
		public String getStatusCase(String caseId) 
		{
			...
			if(ehfCaseObj != null)
			{
				...
				{
					msg="The case is locked due to exceeding the ceiling amount and pending with AHCT for approval";
				}
			}
			return msg;
		}
		...
		{
			StringBuffer query=new StringBuffer();
			List<PreauthVO> dialysisList=null;
			try{
				query.append(" select to_char(ec.cycle) as COUNT, ec.PATIENT_PHOTO as PATIENTPHOTOPATH, ec.PATIENT_CONSENT as PATIENTCONSENTPATH,ec.FILE_PHOTO_NAME as PATIENTPHOTONAME,ec.FILE_CONSENT_NAME as PATIENTCONSENTNAME,to_char(ec.CRT_DT,'dd-MM-YYY HH:mm:ss') as TESTTIME ");
				query.append(" from ehf_case_dailysis_dtls ec WHERE ec.case_id = '"+caseId+"'  order by ec.crt_dt ");
				dialysisList=genericDao.executeSQLQueryList(PreauthVO.class, query.toString());	
			}
			catch(Exception e)
			{
				e.printStackTrace();
		...
		public List<BioMetricVo> getOutTimeCapture(String caseId) 
		{
			Long lActOrder = 1L;
			StringBuffer lQueryBuffer = new StringBuffer();
			List<BioMetricVo> captureList=null;
			SimpleDateFormat sdfw=new SimpleDateFormat("dd/MM/yyyy HH:mm:ss ");
			String args[] = new String[1];
			lQueryBuffer.append("  select to_char(max(to_number(cycle))) as VALUE  from EHF_CASE_DAILYSIS_DTLS er  WHERE case_id = ? ");
			args[0] = caseId;
			List<BioMetricVo> actOrderList = genericDao.executeSQLQueryList(BioMetricVo.class, lQueryBuffer.toString(), args);
			
			if(actOrderList.size()>0)
			{
				if(actOrderList.get(0).getCOUNT() != null)
				{
					lQueryBuffer = new StringBuffer();
					lQueryBuffer.append(" select to_char(CRT_DT,'DD-MM-YYYY hh24:mi:ss')  as INTIME from EHF_CASE_DAILYSIS_DTLS  where case_id='"+caseId+"' and cycle='"+actOrderList.get(0).getVALUE()+"' ");
					captureList = genericDao.executeSQLQueryList(BioMetricVo.class, lQueryBuffer.toString());
				}
			}
			return captureList;
		}
		...
		public Long getDialysisCycles(String caseId)
		{
			...
			StringBuffer lQueryBuffer = new StringBuffer();
			String args[] = new String[1];
			//lQueryBuffer.append(" select max(to_number(au.id.cycle)) as cycles from EhfCaseDialyDtls au where au.id.caseId=? ");
			lQueryBuffer.append(" select to_char(max(to_number(cycle))) as DAYS  from EHF_CASE_DAILYSIS_DTLS er  WHERE case_id = ? ");
			args[0] = caseId;
			List<BioMetricVo> actOrderList = genericDao.executeSQLQueryList(BioMetricVo.class, lQueryBuffer.toString(), args);
			if(actOrderList.size()>0)
			{
				if(actOrderList.get(0).getDAYS() != null )
				cycles=Long.parseLong(actOrderList.get(0).getDAYS());
			}
			return cycles;
		}
		...
		public Long getDialysisCyclesDone(String caseId)
		{
			...
			StringBuffer lQueryBuffer = new StringBuffer();
			String args[] = new String[1];
			lQueryBuffer.append(" select to_char(max(to_number(cycle))) as DAYS  from EHF_CASE_DAILYSIS_DTLS er  WHERE case_id = ? and er.PATIENT_CONSENT is not null and er.PATIENT_PHOTO is not null ");
			args[0] = caseId;
			List<BioMetricVo> actOrderList = genericDao.executeSQLQueryList(BioMetricVo.class, lQueryBuffer.toString(), args);
			if(actOrderList.size()>0)
			{
				if(actOrderList.get(0).getDAYS() != null )
				days=Long.parseLong(actOrderList.get(0).getDAYS());
			}
			return days;
		}
		@Override
		public String saveDialysisDtls(PreauthVO preauthVO) 
		{
			...
			try
			{
				lQueryBuffer.append(" select to_char(max(to_number(cycle))) as DAYS from EhfCaseDialyDtls au where au.id.caseId=? ");
				args[0] = preauthVO.getCaseId();
				List<BioMetricVo> actOrderList = genericDao.executeHQLQueryList(BioMetricVo.class, lQueryBuffer.toString(), args);
				if (actOrderList != null && !actOrderList.isEmpty() && actOrderList.get(0).getDAYS() != null) 
				{
					if (actOrderList.get(0).getDAYS() != null && !"".equalsIgnoreCase(actOrderList.get(0).getDAYS()))
							lActOrder = Long.parseLong(actOrderList.get(0).getDAYS()) + 1;
				}
			
				EhfCaseDialyDtls ehfCaseBio = new EhfCaseDialyDtls();
				
				ehfCaseBio.setId(new EhfCaseDialyDtlsId(lActOrder,preauthVO.getCaseId()));
				ehfCaseBio.setCardNo(preauthVO.getEnrollCardNo());
				if(preauthVO.getAttachmentsList().size() > 0)
				{
					AttachmentVO attachVO = preauthVO.getAttachmentsList().get(0);
					ehfCaseBio.setPatientPhotoPath(attachVO.getAttachPath());
					ehfCaseBio.setPatientPhotoName(attachVO.getFileName());
					
				}
				if(preauthVO.getDrugsAttchList().size() > 0)
				{
					AttachmentVO attachVO = preauthVO.getDrugsAttchList().get(0);
					ehfCaseBio.setPatientConsentPath(attachVO.getAttachPath());
					ehfCaseBio.setPatientConsentName(attachVO.getFileName());
				}
				ehfCaseBio.setCrtDt(new Timestamp(new Date().getTime()));
				ehfCaseBio.setCrtUsr(preauthVO.getCruUsr());
				genericDao.save(ehfCaseBio);
				msg="Success";
		...
		public String getPatientType(String asriCode,String icdProcCode,String scheme)
		{
			String patientType="";
			StringBuffer query=new StringBuffer();
			query.append( " select et.process as ID from EHFM_MAIN_THERAPY_NABH et  WHERE et.asri_code='"+asriCode+"' and ICD_PROC_CODE='"+icdProcCode+"' AND STATE='"+scheme+"' ");
		...
		public List<LabelBean> getDrugTypeList(String hospId) {
			...
			try
			{
				query.append(" select a.drugName as VALUE, a.drugId as ID  from EhfmBreakupDrugMst a,EhfmHospitals b where a.hospId=b.hospGroup and a.activeYn='Y' and b.hospId='"+hospId+"'");
				lstDrugType = genericDao.executeHQLQueryList(LabelBean.class, query.toString());
			}catch(Exception e)
			{
				e.printStackTrace();
			}
		...
		@Override
		public List<LabelBean> getDrugPriceList(String drugId) {
			// TODO Auto-generated method stub
						List<LabelBean> lstPriceType = new ArrayList<LabelBean>();
						StringBuffer query = new StringBuffer();
						try
						{
							query.append(" select distinct a.price as ID from EhfmBreakupDrugMst a where activeYn='Y' and drugId='"+drugId+"'");
							lstPriceType = genericDao.executeHQLQueryList(LabelBean.class, query.toString());
						}catch(Exception e)
						{
							e.printStackTrace();
						}
		...
		@Override
		public Integer getDrugQuantAmount(String drugId) {
			// TODO Auto-generated method stub

			 int amount =0;
			/* try
			 {
				 EhfmBreakupDrugMst ehfmBreakupQuantity = genericDao.findById(EhfmBreakupDrugMst.class, String.class, drugId); 
				 if(ehfmBreakupQuantity != null)
				 {
					 amount = ehfmBreakupQuantity.getPrice(); 	 
				 }
		...
		public List<LabelBean> saveDrugDetails(LabelBean drugVO) {
			...
			//for excel data saving
			if(drugVO.getExcelFlag()!=null&&"Y".equalsIgnoreCase(drugVO.getExcelFlag())){
			EhfmHospitals ehfmHos = genericDao.findById(EhfmHospitals.class,String.class,drugVO.getHospId());
    		criteriaList.add(new GenericDAOQueryCriteria("drugId",drugVO.getDrugId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
    		criteriaList.add(new GenericDAOQueryCriteria("hospId",ehfmHos.getHospGroup(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
    		List<EhfmBreakupDrugMst> ehfmdrugs= genericDao.findAllByCriteria( EhfmBreakupDrugMst.class, criteriaList) ;	
			if(ehfmdrugs.size()>0&&ehfmdrugs.get(0).getDrugId()!=null&&!"".equalsIgnoreCase(ehfmdrugs.get(0).getDrugId())){
				float f = Float.parseFloat(ehfmdrugs.get(0).getPrice());
				drugVO.setUnitPrice(f);
			EhfPatientBreakupdrugsNabh ehfPatientBreakupdrugsNabh = genericDao.findById(EhfPatientBreakupdrugsNabh.class, EhfPatientBreakupdrugsNabhPK.class,new EhfPatientBreakupdrugsNabhPK(drugVO.getCaseId(),drugVO.getDrugId(),drugVO.getUnitPrice()));
			Date date = new Date();
			if(ehfPatientBreakupdrugsNabh!=null){
				if(ehfPatientBreakupdrugsNabh.getActiveYN()!=null && "N".equalsIgnoreCase(ehfPatientBreakupdrugsNabh.getActiveYN())){
					ehfPatientBreakupdrugsNabh.setDrugQuantity(drugVO.getDrugQuantity());
					ehfPatientBreakupdrugsNabh.setActiveYN("Y");
				}else{
					ehfPatientBreakupdrugsNabh.setDrugQuantity(drugVO.getDrugQuantity()+Float.parseFloat(ehfPatientBreakupdrugsNabh.getDrugQuantity().toString()));
					ehfPatientBreakupdrugsNabh.setActiveYN("Y")	;
				}	
				ehfPatientBreakupdrugsNabh.setLstUpdDt(new java.sql.Timestamp(date.getTime()));
				ehfPatientBreakupdrugsNabh.setLstUpdUsr(drugVO.getUserId());
				
			}else{
				ehfPatientBreakupdrugsNabh = new EhfPatientBreakupdrugsNabh();
				ehfPatientBreakupdrugsNabh.setActiveYN("Y");
				ehfPatientBreakupdrugsNabh.setCrtDt(new java.sql.Timestamp(date.getTime()));
				ehfPatientBreakupdrugsNabh.setCrtUser(drugVO.getUserId());
				ehfPatientBreakupdrugsNabh.setDrugQuantity(drugVO.getDrugQuantity());
				ehfPatientBreakupdrugsNabh.setHospId(drugVO.getHospId());
				ehfPatientBreakupdrugsNabh.setPatientId(drugVO.getPatientId());
				ehfPatientBreakupdrugsNabh.setId(new EhfPatientBreakupdrugsNabhPK(drugVO.getCaseId(),drugVO.getDrugId(),drugVO.getUnitPrice()));
			}
			ehfPatientBreakupdrugsNabh.setDrugAmount(Float.parseFloat(ehfPatientBreakupdrugsNabh.getId().getUnitPrice().toString()) * Float.parseFloat(ehfPatientBreakupdrugsNabh.getDrugQuantity().toString()));
			ehfPatientBreakupdrugsNabh = genericDao.save(ehfPatientBreakupdrugsNabh);
			int count=1;
			if(ehfPatientBreakupdrugsNabh!=null&&drugVO.getDrugcount()==1){
				EhfCaseDocAttch asritCaseDocAttch=new EhfCaseDocAttch();
				asritCaseDocAttch.setCaseId(drugVO.getCaseId());
				asritCaseDocAttch.setActualName(drugVO.getFileName());
				asritCaseDocAttch.setSavedName(drugVO.getFilepath());
				asritCaseDocAttch.setCrtUsr(drugVO.getUserId());
				asritCaseDocAttch.setCrtDt(new java.sql.Timestamp(date.getTime()));
				asritCaseDocAttch.setAttachType("CD1264");
			...
			if(ehfPatientBreakupdrugsNabh !=null){
				StringBuffer query = new StringBuffer();
				query.append(" select e.id.caseId as caseId,e.id.drugId as drugId,m.drugName as drugName,e.id.unitPrice as unitPrice,e.drugQuantity as drugQuantity,e.drugAmount as drugAmt ");
				query.append(" from EhfPatientBreakupdrugsNabh e, EhfmBreakupDrugMst m where e.id.drugId = m.drugId and e.activeYN = 'Y' and m.activeYn = 'Y' and e.id.caseId = '").append(drugVO.getCaseId()).append("' ");
				resList = genericDao.executeHQLQueryList(LabelBean.class, query.toString());
			}
		}
	}else{
		//for manual data saving
		EhfPatientBreakupdrugsNabh ehfPatientBreakupdrugsNabh = genericDao.findById(EhfPatientBreakupdrugsNabh.class, EhfPatientBreakupdrugsNabhPK.class,new EhfPatientBreakupdrugsNabhPK(drugVO.getCaseId(),drugVO.getDrugId(),drugVO.getUnitPrice()));
		Date date = new Date();
		if(ehfPatientBreakupdrugsNabh!=null){
			if(ehfPatientBreakupdrugsNabh.getActiveYN()!=null && "N".equalsIgnoreCase(ehfPatientBreakupdrugsNabh.getActiveYN())){
		...
		public List<LabelBean> deleteDrugDetails(LabelBean drugVO) {
			...
			Date date = new Date();
			if(ehfPatientBreakupdrugsNabh!=null){
				ehfPatientBreakupdrugsNabh.setActiveYN("N")	;
				ehfPatientBreakupdrugsNabh.setLstUpdDt(new java.sql.Timestamp(date.getTime()));
				ehfPatientBreakupdrugsNabh.setLstUpdUsr(drugVO.getUserId());
		    }
			ehfPatientBreakupdrugsNabh = genericDao.save(ehfPatientBreakupdrugsNabh);
		...
		StringBuffer query = new StringBuffer();
		query.append(" select e.id.caseId as caseId,e.id.drugId as drugId,m.drugName as drugName,e.id.unitPrice as unitPrice,e.drugQuantity as drugQuantity,e.drugAmount as drugAmt ");
		query.append(" from EhfPatientBreakupdrugsNabh e, EhfmBreakupDrugMst m where e.id.drugId = m.drugId and e.activeYN = 'Y' and m.activeYn = 'Y' and e.id.caseId = '"+caseId+ "'" );
		resList = genericDao.executeHQLQueryList(LabelBean.class, query.toString());
		return resList;
		}
		...
		List<LabelBean> resList = new ArrayList<LabelBean>();
		StringBuffer query = new StringBuffer();
		query.append(" select sum(e.drugAmount) as COUNT ");
		query.append(" from EhfPatientBreakupdrugsNabh e where  e.activeYN = 'Y' and e.id.caseId = '"+caseId+ "'" );
		resList = genericDao.executeHQLQueryList(LabelBean.class, query.toString());
		if(resList!=null && !resList.isEmpty() && resList.size()>0 && resList.get(0).getCOUNT()!=null)
			drugAmt = resList.get(0).getCOUNT().toString();
		return drugAmt;
		}
		...
		public Map<String, Object> getLuceneSearch(Map<String, Object> paramMap){
			...
			try{
				resList=preauthDtlsDao.getLuceneSearch(paramMap);
			}catch(Exception e){
				e.printStackTrace();
		...
		public String checkAmountExceed(List<LabelBean> drugvoList) {
			...
			StringBuffer	queryBuff = new StringBuffer();
		    double        oldDrugDetails=0d;
			double          totaldrugAmt=0d;
			String  drugFlag="";
			List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>();
			...
			drugDetails = genericDao.executeSQLQueryList(LabelBean.class,queryBuff.toString(),new String[]{drugvoList.get(0).getCaseId()} );
						
		    if(drugDetails != null && drugDetails.size() > 0 ){
						 oldDrugDetails=Double.parseDouble(drugDetails.get(0).getTOTALDRUGAMT().toString());
				}
			for (int i = 0; i < drugvoList.size(); i++) {
					EhfPatientBreakupdrugsNabh ehfPatientBreakupdrugsNabh = genericDao.findById(EhfPatientBreakupdrugsNabh.class,EhfPatientBreakupdrugsNabhPK.class,new EhfPatientBreakupdrugsNabhPK(drugvoList.get(i).getCaseId(),drugvoList.get(i).getDrugId(), drugvoList.get(i).getUnitPrice()));
					EhfmHospitals ehfmHos = genericDao.findById(EhfmHospitals.class,String.class,drugvoList.get(0).getHospId());
					criteriaList.add(new GenericDAOQueryCriteria("drugId",drugvoList.get(i).getDrugId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
					criteriaList.add(new GenericDAOQueryCriteria("hospId",ehfmHos.getHospGroup(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
					List<EhfmBreakupDrugMst> ehfmdrugs= genericDao.findAllByCriteria( EhfmBreakupDrugMst.class, criteriaList) ;	
					criteriaList.removeAll(criteriaList);
					if(ehfmdrugs.size()>0&&ehfmdrugs.get(0).getDrugId()!=null&&!"".equalsIgnoreCase(ehfmdrugs.get(0).getDrugId())){
					        if (ehfPatientBreakupdrugsNabh != null) {
						totaldrugAmt += ehfPatientBreakupdrugsNabh.getId().getUnitPrice()* (drugvoList.get(i).getDrugQuantity() + Float.parseFloat(ehfPatientBreakupdrugsNabh.getDrugQuantity().toString()));

					     } else {
						totaldrugAmt += Float.parseFloat(ehfmdrugs.get(0).getPrice())* drugvoList.get(i).getDrugQuantity();

					       }
				  }
					else{
						drugFlag="N";
						msg="The "+drugvoList.get(i).getDrugId()+" Drug Id is not available in "+ehfmHos.getHospName()+"";
						break;
					
					}
			   }
				if(totaldrugAmt>oldDrugDetails||(drugFlag!=null&&!drugFlag.equals("")&&drugFlag.equals("N"))){
					if(totaldrugAmt>oldDrugDetails)
					msg="New Drug amount can not be greater than old drug amount";
				}
				else{
					msg="Y";
				}
		...
		public String getOldDrugsAmount(String caseId) {
			String drugAmt = "0";
			List<PreauthVO> resList = new ArrayList<PreauthVO>();
			StringBuffer query = new StringBuffer();
			query.append(" select sum(e.amount)||'' as COUNT ");
			query.append(" from EhfPatientDrugsNabh e where  e.activeYN = 'Y' and e.id.caseId = '"+caseId+ "'" );
			resList = genericDao.executeHQLQueryList(PreauthVO.class, query.toString());
			if(resList!=null && !resList.isEmpty() && resList.size()>0 && resList.get(0).getCOUNT()!=null)
				drugAmt = resList.get(0).getCOUNT().toString();
			return drugAmt;
			}
		public String getEnhancementSum(String pStrcaseId,String patientType)
		{
			 String enhancementAmt = "0";
			 StringBuffer query = new StringBuffer();
			List<PreauthVO> lstEnhancement = new ArrayList<PreauthVO>();

			 try
			 {  
				 query.append("  select sum(ee.amount)||'' as COUNT ");	
				 query.append(" from EhfCaseEnhancementDtls ee  ");
				 query.append("  where ee.caseId = '"+pStrcaseId+"' and ee.activeYN ='Y' and ee.newConsumableFlg is null ");	
				 lstEnhancement = genericDao.executeHQLQueryList(PreauthVO.class, query.toString());
				 if(lstEnhancement!=null && !lstEnhancement.isEmpty() && lstEnhancement.size()>0 && lstEnhancement.get(0).getCOUNT()!=null)
					 enhancementAmt = lstEnhancement.get(0).getCOUNT().toString();
			 }catch(Exception e)
			 {
			e.printStackTrace();	 
			 }
			 return enhancementAmt;
		}
		@Override
		public String saveMedicalCaseDetails(PreauthVO preauthVO) {
			Date date = new Date();
			/*SimpleDateFormat sdfw=new SimpleDateFormat("dd/MM/yyyy HH:mm:ss a");
			Date lCurrentDate=sdfw.parse(preauthVO.getC());*/
			EhfCaseTherapy ehfCaseTherapy = new EhfCaseTherapy();
			List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>(); 
    		criteriaList.add(new GenericDAOQueryCriteria("caseId",preauthVO.getCaseId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
			criteriaList.add(new GenericDAOQueryCriteria("activeyn","Y",GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
			  ...
			  Long therapyId=0L;
				...
				{	
					if(ehfCaseThrpyLstLst.get(0).getCaseTherapyId()!=null)
					{
						EhfCaseTherapy ehfCaseTherapy1 = genericDao.findById(EhfCaseTherapy.class, Long.class, ehfCaseThrpyLstLst.get(0).getCaseTherapyId());
						/*List<GenericDAOQueryCriteria> criteriaList3 = new ArrayList<GenericDAOQueryCriteria>(); 
			    		criteriaList3.add(new GenericDAOQueryCriteria("caseTherapyId",ehfCaseThrpyLstLst.get(0).getCaseTherapyId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
			    		List<EhfCaseTherapy> ehfCaseThrpyLstLst4 = genericDao.findById(EhfCaseTherapy.class, String.class, ehfCaseThrpyLstLst.get(0).getCaseTherapyId());*/
						ehfCaseTherapy1.setActiveyn("N");
						ehfCaseTherapy1.setLstUpdDt(new java.sql.Timestamp(date.getTime()));
						genericDao.save(ehfCaseTherapy1);
					}
				...
				List<GenericDAOQueryCriteria> criteriaList1 = new ArrayList<GenericDAOQueryCriteria>(); 
	    		criteriaList1.add(new GenericDAOQueryCriteria("caseId",preauthVO.getCaseId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				criteriaList1.add(new GenericDAOQueryCriteria("activeyn","Y",GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				  List<EhfCaseTherapy> ehfCaseThrpyLstLst1 = genericDao.findAllByCriteria(EhfCaseTherapy.class, criteriaList1);
				  if(ehfCaseThrpyLstLst1.size() == 0)
					{
						ehfCaseTherapy = new EhfCaseTherapy();
						ehfCaseTherapy.setCaseTherapyId(preauthVO.getMedSeq());
					}
			ehfCaseTherapy.setAsriCatCode(preauthVO.getMedAsriCode());
			ehfCaseTherapy.setIcdCatCode(preauthVO.getMedCatCode());
			ehfCaseTherapy.setIcdProcCode(preauthVO.getIcdProcName());
			ehfCaseTherapy.setCaseId(preauthVO.getCaseId());
			ehfCaseTherapy.setCrtDt(new java.sql.Timestamp(date.getTime()));
			ehfCaseTherapy.setCrtUsr(preauthVO.getCruUsr());
			ehfCaseTherapy.setActiveyn("Y");
			ehfCaseTherapy.setIpOtherMed(preauthVO.getIpOtherRemark());
			
			/*if(patientVO.getCritical()!=null && !"".equalsIgnoreCase(patientVO.getCritical()))
			{
				ehfCaseTherapy.setCriticalFlg(patientVO.getCritical());
			}*/
			//ehfCase.setCriticalFlg(patientVO.getCritical());
		...
		public String updateEnhAttch(PreauthVO preauthVO) {
		    ...
		    Date lCurrentDate1=new java.sql.Timestamp(new Date().getTime()); 
		     try{
		    	...
		    	// EhfCaseEnhancementDtls ehfCaseEnhDtls = new EhfCaseEnhancementDtls();
		    	 EhfCaseEnhancementDtls ehfCaseEnhDtls = genericDao.findById(EhfCaseEnhancementDtls.class, Long.class, Long.parseLong(preauthVO.getCaseStatusId()));		
		      /*  ehfCaseEnhDtls.setAttachPath(preauthVO.getAttachPath());
		        ehfCaseEnhDtls.setAttachName(preauthVO.geta());
		        ehfCaseEnhDtls.setLstUpdUser(preauthVO.getCruUsr());
		        ehfCaseEnhDtls.setLstUpdDt(lCurrentDate1);
		        ehfCaseEnhDtls.setActiveYN("N");*/
		    	 if(preauthVO.getAttachmentsList()!=null && preauthVO.getAttachmentsList().size() > 0)
					{
		 	
					AttachmentVO attachVO = preauthVO.getAttachmentsList().get(0);
					ehfCaseEnhDtls.setNewAttachpath(attachVO.getAttachPath());
					ehfCaseEnhDtls.setNewPathName(attachVO.getFileName());
				...
				genericDao.save(ehfCaseEnhDtls);
