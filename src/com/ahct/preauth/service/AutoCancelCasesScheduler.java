...
public class AutoCancelCasesScheduler {
	...
	{
		logger.info("Scheduler to Auto Cancel Inactive Preauth Cases started in TS");
...
//		System.out.println("Scheduler :started");
		...
		validateSurgeryUpdatedCases("preauthAprvDate","csSurgUpdDt",60); //If not updated case from PTD approved to surg upd//Validate Surgery Updated before and  feb 01 16
...
//		System.out.println("Start:validateClaimSubmitPending(CD21) method");
		...
		query.append(" AND A.SCHEME_ID='"+schemeId+"' AND (SYSDATE-A.LST_UPD_DT)>60");
...
//		System.out.println("Start:inside validatePendingPreauthNew(CD10,CD210,CD217,CD208) after feb01 2016 ");
		...
		query.append(" WHERE A.CASE_PATIENT_NO=B.PATIENT_ID AND A.CASE_STATUS='"+status+"'");
		query.append(" AND A.SCHEME_ID='"+schemeId+"' AND (SYSDATE-A.LST_UPD_DT)>30");
		query.append(" and a.case_id not in (select distinct ect.case_id from ehf_case_therapy ect where ect.icd_proc_code  in ('20.96','N18.6.A','92.23.2','92.23.3','92.24.2','92.20.1','92.20.2','92.27.1','92.27.2','92.29.1','99.23.2','99.23.1','95.49.7','95.49.8','95.49.3','95.49.4','95.49.5','92.24.3','64.3.1','92.23.1','92.24.1','92.29.2','92.29.3') AND ect.activeyn = 'Y')");
		query.append(" AND TRUNC(B.REG_HOSP_DATE)>=TO_DATE('01/02/2016','DD/MM/YYYY')");
		//Added for exemption of NABH Cases
		query.append(" AND A.CASE_ID NOT IN (select CASE_ID from EHF_NABH_CANCEL_EXEMP ) ");
...
//		System.out.println("Start: validatePendingPreauth(CD10,CD210,CD217,CD208) before feb01 2016 ");
		...
		query.append("SELECT A.CASE_ID AS CASEID ,A.CASE_NO AS CASENO,A.CASE_STATUS AS CASESTATUS,A.CASE_REGN_DATE AS REGNDATE FROM EHF_CASE A , EHF_PATIENT B");
		query.append(" WHERE A.CASE_PATIENT_NO=B.PATIENT_ID AND A.CASE_STATUS='"+status+"'");
		query.append(" AND A.SCHEME_ID='"+schemeId+"' AND (SYSDATE-A.LST_UPD_DT)>45");
		query.append(" and a.case_id not in (select distinct ect.case_id from ehf_case_therapy ect where ect.icd_proc_code  in ('20.96','N18.6.A','92.23.2','92.23.3','92.24.2','92.20.1','92.20.2','92.27.1','92.27.2','92.29.1','99.23.2','99.23.1','95.49.7','95.49.8','95.49.3','95.49.4','95.49.5','92.24.3','64.3.1','92.23.1','92.24.1','92.29.2','92.29.3') AND ect.activeyn = 'Y')");
		query.append(" AND TRUNC(B.REG_HOSP_DATE)<TO_DATE('01/02/2016','DD/MM/YYYY')");
		//Added for exemption of NABH Cases
		query.append(" AND A.CASE_ID NOT IN (select CASE_ID from EHF_NABH_CANCEL_EXEMP ) ");
...
//		System.out.println("Start: validateSurgeryUpdatedCases(preauthApprvDt) before feb01 2016 ");
		...
		query.append(" AND A.SCHEME_ID='CD202' AND A.PREAUTH_CANCEL_DT IS NULL AND "); 
		query.append("  (case when trunc(b.reg_hosp_date) >= to_date('01/02/2016', 'DD/MM/YYYY') and (sysdate - (a.preauth_aprv_dt)) > 30 then 1 ");
		query.append(" when trunc(b.reg_hosp_date) < to_date('01/02/2016', 'DD/MM/YYYY') and (sysdate - (a.preauth_aprv_dt)) > 60 then 1 else 0  end) = 1 ");
		//Added for exemption of NABH Cases
		query.append(" AND A.CASE_ID NOT IN (select CASE_ID from EHF_NABH_CANCEL_EXEMP ) ");
						    ...
						    for(EhfCaseTherapy locTherapy: ehfCaseTherapyList)
						    	{
						    		if(locTherapy.getIcdProcCode()!=null && !"".equalsIgnoreCase(locTherapy.getIcdProcCode()))
						    			{
						    			if("'20.96','N18.6.A','92.23.2','92.23.3','92.24.2','92.20.1','92.20.2','92.27.1','92.27.2','92.29.1','99.23.2','99.23.1','95.49.7','95.49.8','95.49.3','95.49.4','95.49.5','92.24.3','64.3.1','92.23.1','92.24.1','92.29.2','92.29.3'".contains(locTherapy.getIcdProcCode()))
						    			{
						    				return "false";
						    			}
