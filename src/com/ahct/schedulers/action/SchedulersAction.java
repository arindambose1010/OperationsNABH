	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,HttpServletRequest request, 
			HttpServletResponse response) throws Exception 
			{
				...
				if(lStrActionVal!=null && "openSchedulers".equalsIgnoreCase(lStrActionVal))
				 ...
				 if(lStrActionVal!= null && "generateDrugLucene".equals(lStrActionVal)){
					 PreauthDtlsDao preauthdtlsdao=(PreauthDtlsDao)ServiceFinder.getContext(request).getBean("preauthDtlsDao");
					 String hospId=request.getParameter("hospId");
					 preauthdtlsdao.insertDetailsLuceneEHS(hospId);
						schedulersForm.setMsg("Lucene file genertion done");
				 }
				...
				if(lStrActionVal!=null && "mhcScheduler".equalsIgnoreCase(lStrActionVal))
				{

					if(schedulerType!=null && "updateEnrollmentFamily".equalsIgnoreCase(schedulerType))
						{
							AhcScheduler ahcScheduler=(AhcScheduler)ServiceFinder.getContext(request).getBean("mhcCron");
							ahcScheduler.fetchMhcData();
							schedulersForm.setMsg("MHC Scheduler to Update the Enrollment Family Table ran");
						}
