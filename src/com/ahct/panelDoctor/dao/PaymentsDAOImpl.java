...
public class PaymentsDAOImpl implements PaymentsDAO{
	...
	public ArrayList getDoctorDetails(HashMap lparamMap) {
		...
		query.append(" where u.user_id=pam.user_id and bm.bank_id=pam.bank_id");
...
//					+ e.getMessage());
	...
	public ArrayList getRejDoctorDetails(HashMap lparamMap) {
        ...
        args[1]=scheme;
        query.append(" select distinct pd.cases_set_id CASE_ID,pd.doctor_id DOC_ID,u.first_name || '-' || u.last_name EMPNAME,pd.pnldoc_amt||'' AMOUNT,");
		query.append(" pam.name_as_per_acc ACCNAME,bm.bank_branch BANKBRANCH,bm.bank_category BANKCATEGORY,bm.ifc_code BANKIFSC,");
		query.append(" bm.bank_id BANKID,pam.account_no ACCOUNTNO,bm.bank_name BANKNAME");
		query.append(" from ehfm_users u,ehfm_bank_master bm,ehfm_pnldoc_acct_dtls pam,ehf_pnldoc_payments pd,ehf_pnldoc_workflow epw,ehf_pnldoc_case_dtls epc ");
		query.append(" where u.user_id=pam.user_id and u.user_id=pd.doctor_id and bm.bank_id=pam.bank_id and epw.doc_id=u.user_id");
		query.append(" and pd.payment_status=? and pd.scheme=? and epw.w_id=epc.wrkflw_id and epc.pmnt_id=pd.cases_set_id ");
		query.append(selectedCondition);
		query.append(selectedWorkflowCondition);
		...
		for(int i=0;i<panelDocAccListDtls.size();i++)
		{
			...
			lFileMap.put("TRANSACTION_AMOUNT",allDocDetails.getPanelDocAmt());
...
public String SetPanelDocStatus(HashMap lparamMap) {
	...
	try {
		...
		{
			lFlag = 0;
			lstramount=(String) lDataList.get(j + 3);
...
private void setCaseDtls(HashMap lparamMap) throws Exception {
	...
	for(int i=0;i<wrkFlwIdList.size();i++){
		...
		String currWrkFlwId="";
				...
				else if(transStatus.equals("E"))
					{
						currWrkFlwId=config.getString("FIN.WorkFlowIdPaid");
						if(lparamMap.get("FileNameNew")!=null)
							FileNameNew=(String)lparamMap.get("FileNameNew");
					}
				else if(transStatus.equals("R"))
					{
	...
	try {
		...
		if (ehfPnldocTdsPayment.size() > 0) {
			...
			if ((String) lparamMap.get("TRANS_DT") != null)
				ehfPnldocTdsPay.setTransDate(df.parse((String) lparamMap
						.get("TRANS_DT")));
			ehfPnldocTdsPay
...
public int updatePanelDocPaymentDetails(HashMap lparamMap) throws Exception {
	...
	try {
		...
		if (ehfPanelDocPayments.size() > 0) {
			...
			if ((String) lparamMap.get("TRANS_DT") != null)
				ehfPanelDocAcc.setTransDate(df.parse((String) lparamMap
						.get("TRANS_DT")));
