...
public class ClaimsFlowServiceImpl implements ClaimsFlowService {
	...
	public ClaimsFlowVO getCaseDtls(ClaimsFlowVO claimFlowVO) {
		...
		if (ehfCaseClaim != null) {
			claimFlowVO.setBillNum(ehfCaseClaim.getBillNum());
			claimFlowVO.setMedcoRemark(ehfCaseClaim.getMedcoRemrk());
			if (ehfCaseClaim.getClaimInitAmt() != null)
			{
				claimFlowVO.setClaimInitAmt(ehfCaseClaim.getClaimInitAmt().toString());
				claimFlowVO.setClaimInitiatedAmt(ehfCaseClaim.getClaimBillAmt());
			}
			...
			{
				Double d = ehfCaseClaim.getChAprAmt();
			    Long lng = Double.valueOf(d).longValue();
				claimFlowVO.setChAprAmt(lng.toString());
			}
			if (ehfCaseClaim.getChEntAprAmt() != null)
			{
				Double s = ehfCaseClaim.getChEntAprAmt();
			    Long png = Double.valueOf(s).longValue();
				claimFlowVO.setChEntAprAmt(png.toString());
		...
		try{
			 EhfPatient ehfPatientDtls = getPatDtls(ehfCase.getCasePatientNo());
			 if(ehfPatientDtls.getPatientIpop() != null && !"".equalsIgnoreCase(ehfPatientDtls.getPatientIpop()))
					claimFlowVO.setIpOpFlag(ehfPatientDtls.getPatientIpop());
			claimFlowVO = claimsFlowDAO.getInvestigationDetails(claimFlowVO);
			claimFlowVO = claimsFlowDAO.getCEXVerifiedunits(claimFlowVO);
			claimFlowVO = claimsFlowDAO.getDrugDetails(claimFlowVO);
			if((ehfPatientDtls != null && !"".equalsIgnoreCase(ehfPatientDtls.getPatientIpopNo()) && "IPM".equalsIgnoreCase(ehfPatientDtls.getPatientIpop())))
			{
				//Long totalConsumAmt= Long.parseLong(claimFlowVO.getTotalConsumableAmount().toString())+Long.parseLong(claimFlowVO.getTotalDrugAmt().toString());
			//	claimFlowVO.setPackAmt(totalConsumAmt.toString());
				if(ehfCase.getEnhAppvAmt() != null && ehfCase.getEnhAppvAmt()>0)
					claimFlowVO.setEnhAppvAmt(ehfCase.getEnhAppvAmt());
				else
					claimFlowVO.setEnhAppvAmt(0L);
...
//					+ e.getMessage());
	...
	public List<LabelBean> getCasesForPayments(ClaimsFlowVO claimsFlowVO) {
		...
		try {
			StringBuffer query = new StringBuffer();
			query.append(" select EC.caseNo as ID,EC.caseId as SUBID,to_char(EC.lstUpdDt,'dd-mm-yyyy hh24:mi:ss') as LSTUPDDT, EH.hospName as LVL,ECC.totClaimAmt||'' as WFTYPE,EP.name as VALUE,EP.cardNo as UNITID, EL.locName as EMPNAME,EHB.hospAccName as SUBNAME, EC.clmSubDt as CLMSUBDT ");
...
//					+ e.getMessage());
	...
	public List<LabelBean> getChronicCasesForPayments(ClaimsFlowVO claimsFlowVO) {
		...
		try {
			StringBuffer query = new StringBuffer();
			query.append(" select EC.id.chronicId as SUBID,EC.id.chronicNo as ID,EH.hospName as LVL,EC.claimAmount as WFTYPE,EP.name as VALUE,EP.cardNo as UNITID, EL.locName as EMPNAME,EHB.hospAccName as SUBNAME , EC.clmSubDt as CLMSUBDT");
...
//					+ e.getMessage());
	...
	public List<LabelBean> getErrCasesForPayments(ClaimsFlowVO claimsFlowVO) {
		...
		try {
			StringBuffer query = new StringBuffer();
			query.append(" select ECC.errClaimId as ID,EC.caseNo as DSGNID,EC.caseId as SUBID,EH.hospName as LVL,ECC.totClaimAmt||'' as WFTYPE,EP.name as VALUE,EP.cardNo as UNITID, EL.locName as EMPNAME,EHB.hospAccName as SUBNAME, EC.clmSubDt as CLMSUBDT ");
	...
	public List<LabelBean> getTDSCasesForPayments(ClaimsFlowVO claimsFlowVO) {
		...
		try {
			...
			query.append(" select ECTP.tdsPaymentId as ID,EC.caseNo as DSGNID,EH.hospName as LVL,ECTP.claimAmt||'' as WFTYPE,EP.name as VALUE,EP.cardNo as UNITID, EC.clmSubDt as CLMSUBDT");
...
//					+ e.getMessage());
	...
	public String checkDissCaseFlag(ClaimsFlowVO claimFlowVO) {
		...
		try {
			StringBuffer query = new StringBuffer();
			query.append(" select (sysdate - a.lstUpdDt) * 24||'' as VALUE from EhfCase a  where a.caseStatus =? and  a.nabhHosp='Y' ");
...
//					+ e.getMessage());
	...
	public List<LabelBean> getRfCasesForPayments(ClaimsFlowVO claimsFlowVO) {
		...
		try {
			StringBuffer query = new StringBuffer();
			query.append(" select ECTP.trustPaymentId as ID,EC.caseNo as DSGNID,EH.hospName as LVL,ECTP.claimAmount||'' as WFTYPE,EP.name as VALUE,EP.cardNo as UNITID, EC.clmSubDt as CLMSUBDT");
            // for followup Payment
			if (claimsFlowVO.getPaymentType() != null
					&& !claimsFlowVO.getPaymentType().equalsIgnoreCase(
							ClaimsConstants.EMPTY)
...
@Override
	public int getErroneousClaimOrNot(String caseId) 
	{
		int count = 0;
		...
		String args[] = new String[1];
		args[0] = "S18";
		query.append("select count(*) as COUNT from EhfCaseTherapy where asriCatCode=? and caseId='"+caseId+"' ");

		List<LabelBean> orderList = genericDao.executeHQLQueryList(LabelBean.class, query.toString(),args);
...
@Override
public ClaimsFlowVO getCmsDedDtyls(String caseId){
	ClaimsFlowVO cmsDtls=null;
	try{ cmsDtls=new ClaimsFlowVO();
		List<GenericDAOQueryCriteria> criteriaList = new ArrayList<GenericDAOQueryCriteria>();
	    criteriaList.add(new GenericDAOQueryCriteria("caseId",caseId, GenericDAOQueryCriteria.CriteriaOperator.EQUALS ));
	    criteriaList.add(new GenericDAOQueryCriteria("crWorkflowId","CONTEST_AMT", GenericDAOQueryCriteria.CriteriaOperator.EQUALS ));		    
	   List<SgvaCRMgmtDtls> cmsList=genericDao.findAllByCriteria(SgvaCRMgmtDtls.class,criteriaList);
	    criteriaList.removeAll(criteriaList);
	    if(cmsList!=null&&cmsList.size()>0){
	    	cmsDtls.setCmsRiseFlag("Y");
	    	cmsDtls.setCmsId(cmsList.get(0).getCrId());
	    	cmsDtls.setCrDesc(cmsList.get(0).getCrDesc());
	    	criteriaList.add(new GenericDAOQueryCriteria("id.remarksId",cmsList.get(0).getCrId(), GenericDAOQueryCriteria.CriteriaOperator.EQUALS ));
		   List<SgvaCRMgmtRemarks> cmsRemList=genericDao.findAllByCriteria(SgvaCRMgmtRemarks.class,criteriaList);
		    criteriaList.removeAll(criteriaList);

		   if(cmsRemList!=null&&cmsRemList.size()>0){
			   cmsDtls.setCmsRemarks(cmsRemList.get(0).getRemarks()); 
			   //cmsDtls.setCmsEORemarks(cmsRemList.get(1).getRemarks());
		   }
		        List<SgvaCRMgmtAttach> allAttachments=null;
		        List<ClaimsFlowVO> attachVOList=null;
		        List<GenericDAOQueryCriteria> criteriaList3=new ArrayList<GenericDAOQueryCriteria>();
		        criteriaList3.add(new GenericDAOQueryCriteria("id.crId",cmsList.get(0).getCrId(),GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
		        criteriaList3.add(new GenericDAOQueryCriteria("crtDate","",GenericDAOQueryCriteria.CriteriaOperator.DESC));
		        allAttachments =genericDao.findAllByCriteria(SgvaCRMgmtAttach.class,criteriaList3);
		         if(allAttachments!=null && !allAttachments.isEmpty())
		          {                
		             attachVOList=new ArrayList<ClaimsFlowVO>(allAttachments.size());
		               for( SgvaCRMgmtAttach sgvaCRMgmtAttach:allAttachments)
		                {
		                	ClaimsFlowVO changeMgmtAttachVO=new ClaimsFlowVO();
		                    changeMgmtAttachVO.setLINEITEMNO(sgvaCRMgmtAttach.getId().getLineItemNo());
		                    changeMgmtAttachVO.setAttachFileName(sgvaCRMgmtAttach.getAttachFileName());
		                    changeMgmtAttachVO.setAttachFilePath(sgvaCRMgmtAttach.getAttachFilePath());
		                    attachVOList.add(changeMgmtAttachVO);
		                }
		            }
		            cmsDtls.setAttachlist(attachVOList);
...
@Override
public List<ClaimsFlowVO> getCmsRemarks(String caseId) 

{
	StringBuffer query= new StringBuffer();
	List<ClaimsFlowVO> cmsRemarks = null;
	String args[] = new String[2];
	args[0] = caseId;
	args[1] ="CONTEST_AMT";
	...
	try
	{
		query.append("  select case when cr.lineitem_no ='1' then dt.cr_desc  else cr.remarks end as CR_REMARKS,  ");
		query.append("  cr.action_taken CR_ACTION_TAKEN,  ");
		query.append("  em.first_name || ' ' || em.middle_name || ' ' || ");
		query.append("  em.last_name EMP_FULL_NAME, ");
		query.append("  to_char(cr.crt_dt, 'dd/mm/yyyy HH12:MI:SS am') REMARKS_ENTRYTIME,  ");
		query.append("  d.dsgn_name DSGN_NAME,   ");
		query.append("  w.user_code EMPCODE,  ");
		query.append("  nvl(em.mobile_no, '--NA--') PHONE_NUMBER,  ");
		query.append("  cr.attach_exists ATTACH_EXISTS,   ");
		query.append("  cr.lineitem_no LINEITEMNO   ");
		query.append("   from sgva_crmgmt_remarks  cr, ");
		query.append("  ehfm_users  em, ");
		query.append("  ehfm_designation  d,  ");
		query.append("  ehfm_work_allocation w,  ");
		query.append("  sgva_crmgmt_dtls dt   ");
		query.append("  where em.user_id = cr.emp_id   ");
		query.append(" and d.dsgn_id = em.dsgn_id   ");
		query.append(" and w.user_code = em.login_name ");
		query.append(" and w.primary_flag = 'Y'  ");
		query.append(" and dt.cr_id =cr.remarks_id  ");
		query.append(" and dt.case_id =?   ");
		query.append(" and dt.cr_workflow_id =?   ");
		query.append("   order by cr.lineitem_no desc  ");
		
       cmsRemarks = genericDao.executeSQLQueryList(ClaimsFlowVO.class, query.toString(),args);
	}
   catch(Exception e)
   {
	e.printStackTrace();
...
@Override
public List<LabelBean> getCheckBill(ClaimsFlowVO claimFlowVO) {

	List<LabelBean> lstHospStayQuant = new ArrayList<LabelBean>();
	StringBuffer query = new StringBuffer();
	try
	{
		query.append(" select a.billNum as ID,b.caseId as VALUE   from EhfCaseClaim a,EhfCase b where a.caseId=b.caseId and  a.billNum='"+claimFlowVO.getCaseId()+"' and b.caseHospCode='"+claimFlowVO.getHospId()+"'  ");
...
@Override
public String checksurgFlag(String caseId) {
	String surgFlag = "";
	try{
		EhfCase ehfCase = genericDao.findById(EhfCase.class, String.class, caseId);
	if(ehfCase != null)
	{
		if(ehfCase.getCasePatientNo()!=null && !"".equalsIgnoreCase(ehfCase.getCasePatientNo()))
		{
			surgFlag=ehfCase.getCasePatientNo();
		}
	}
	}catch(Exception e)
	{
...
@Override
public String checksurgFlagNew(String patientId) {
	String surgFlag = "N";
	try{
		EhfPatient ehfPatient = genericDao.findById(EhfPatient.class, String.class, patientId);
	if(ehfPatient != null)
	{
		if(ehfPatient.getPatientIpop()!=null && "IP".equalsIgnoreCase(ehfPatient.getPatientIpop()))
		{
			surgFlag="Y";
		}
	}
