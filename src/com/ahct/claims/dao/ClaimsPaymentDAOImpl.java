...
*/				
				...
				{
					actType = ClaimsConstants.TGTDSJHS;// Journalist AP
		 		}
		    	else
				{
					if(lStrSchemeId!=null && lStrSchemeId.equalsIgnoreCase("CD201"))
				...
				if(patientScheme!=null && patientScheme.equalsIgnoreCase(config.getString("Scheme.JHS")))
				{
					actType = ClaimsConstants.JHSTGTRUST;// Journalist AP
				}
			  else
				{
					if(lStrSchemeId!=null && lStrSchemeId.equalsIgnoreCase("CD201"))
...
//								logger.error("No record present in EhfSurplusTdsClaim with case id "+caseid);	
	...
	public void setTrustAuditDetails(HashMap lparamMap) throws Exception {
		...
		ehfRfAuditId.setRfPaymentId((lStrCaseNo.substring(0, lStrCaseNo.indexOf("/G"))+"/SD/G"));
	...
	public String SetClaimStatus(HashMap lparamMap) {
		...
		try {
			...
			for (int j = 0; j < DataList; j = j + 17) 
			{
				...
				lStrCaseid = (String) lDataList.get(j + 6);
			...
			if (ClaimsConstants.TRUE.equalsIgnoreCase(lResult)) {
				...
				ehfClaimUploadFile = genericDao.findById(
						EhfClaimUploadFile.class, String.class,
...
//			logger.error("Error:SetClaimStatus method in ClaimsPaymentDAOImpl with message "+e);
		...
		try {
			...
			if (ehfClaimAccount.size() > 0) {
				...
				if ((String) lparamMap.get("TRANS_DT") != null)
					ehfClaimAcc.setTransactionDt(df.parse((String) lparamMap
							.get("TRANS_DT")));
				...
				if (ehfClaimAcc != null) {
					...
					if ((String) lparamMap.get("TRANS_DT") != null)
						ehfCase.setCsTransDt(df.parse((String) lparamMap
								.get("TRANS_DT")));
					if ((String) lparamMap.get("SBH_PAID_DATE") != null)
				...
				if (ehfErrClaim != null) {
					...
					if (ehfClaimAccounts.getTimeMilSec() < timeMillSec) {
						ehfClaimAccounts.setTransactionId((String) lparamMap
								.get("TRANS_ID"));
						if ((String) lparamMap.get("TRANS_DT") != null)
							ehfClaimAccounts.setTransactionDt(df
	...
	public String SetFollowupClaimStatus(HashMap lparamMap) {
		...
		try {
			...
			int DataList = lDataList.size();
			for (int j = 0; j < DataList; j = j + 17) // 020
			{
				lFlag = 0;
				lStrClaimAnt = (String) lDataList.get(j + 3);
				lStrTransDt = (String) lDataList.get(j + 5);
				lStrCaseid = (String) lDataList.get(j + 6);
				lStrTransId = (String) lDataList.get(j + 15);
				lStrPaymentUid = (String) lDataList.get(j + 10);
		...
		try {
			...
			if (ehfClaimTrustPayment.size() > 0) {
				...
				ehfClaimTrustPay.setTransactionId((String) lparamMap
						.get("TRANS_ID"));
				if ((String) lparamMap.get("TRANS_DT") != null)
					ehfClaimTrustPay.setTransactionDt(df
							.parse((String) lparamMap.get("TRANS_DT")));
				ehfClaimTrustPay
	...
	public int updateTDSPaymentDetails(HashMap lparamMap) throws Exception {
		...
		try {
			...
			if (ehfClaimTdsPayment.size() > 0) {
				...
				if ((String) lparamMap.get("TRANS_DT") != null)
					ehfClaimTdsPay.setTransDate(df.parse((String) lparamMap
							.get("TRANS_DT")));
				ehfClaimTdsPay
		...
		try {
			...
			if (ehfFPClaimAccounts.size() > 0) {
				...
				ehfFollowUpClaimAcc.setTransactionId((String) lparamMap
						.get("TRANS_ID"));
				if ((String) lparamMap.get("TRANS_DT") != null)
					ehfFollowUpClaimAcc.setTransactionDt(df
							.parse((String) lparamMap.get("TRANS_DT")));
...
catch(Exception e)
{
	...
	public TransactionVO submitClaimPaymentsInAccounts(
		...
		if(transactionVO.getHospitalId()!=null && (transactionVO.getHospitalId().equalsIgnoreCase("EHS69") ||  transactionVO.getHospitalId().equalsIgnoreCase("EHS34") || transactionVO.getHospitalId().equalsIgnoreCase("EHS13"))){
	...
	public TransactionVO submitChronicClaimPaymentsInAccounts(
		...
		if(transactionVO.getDrugAmount()!=null && !("").equalsIgnoreCase(transactionVO.getDrugAmount()))
			totalAmt=Long.valueOf(transactionVO.getGrossAmount())+Long.valueOf(transactionVO.getDrugAmount());
		else
			totalAmt=Long.valueOf(transactionVO.getGrossAmount());
			
		transactionVO.setAmount(totalAmt+"");
		transactionVO.setFlag(config.getString("ACC.JV"));
		...
		if (config.getString("ACC.GOVT").equals(transactionVO.getHospitalType()))
			transactionVO.setCreditAccount(config.getString("ACC.CHRONICMEDICALHOSP"));
		transactionVO.setVoucherType(config.getString("ACC.Journal"));
		...
		/* PAYMENT ENTRY TOWARDS MEDCINIES
		medicalEntry=true;
		transactionVO.setDebtAccount(config.getString("ACC.CHRONICMEDICALHOSP"));		
		if(transactionVO.getScheme()!=null && transactionVO.getScheme().equalsIgnoreCase("CD201"))
			transactionVO.setCreditAccount(config.getString("ACC.CHRONICMEDICAL.AP"));
		else if(transactionVO.getScheme()!=null && transactionVO.getScheme().equalsIgnoreCase("CD202"))
			transactionVO.setCreditAccount(config.getString("ACC.CHRONICMEDICAL.TG"));		
		transactionVO.setVoucherType(config.getString("ACC.Payment"));
		transactionVO.setSection("");
		transactionVO.setAmount(transactionVO.getDrugAmount());
		transactionVO.setNarration(" Drug  amount payment");
		transactionVO.setFlag(config.getString("ACC.P"));
