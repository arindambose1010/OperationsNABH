...
import com.ahct.model.EhfClaimDeductionDtls;
		...
		SimpleDateFormat sqlf = new SimpleDateFormat("yyyy-MM-dd");
		 SessionFactory factory= hibernateTemplate.getSessionFactory();
		  ...
		  if(moduleType.contains("preauth"))
			...
			if(moduleType.contains("drugbreakup"))
			{
				moduleType="drugbreakup";
			}
		  ...
		  try{
			 ...
			 for(LabelBean labelBean:casesSearchVO.getGrpList())
 		    {
				 ...
				 if(casesSearchVO.getCaseForDissFlg()!=null && casesSearchVO.getCaseForDissFlg().equalsIgnoreCase("Y")){
					...
					/* if(!"".equalsIgnoreCase(casesSearchVO.getExceedFlg()) && "Y".equalsIgnoreCase(casesSearchVO.getExceedFlg()) )
					 {
						 caseSts =  config.getString("preauth_caseStatusExceed");
					 }
					 else*/ if(!"".equalsIgnoreCase(casesSearchVO.getExceedFlg()) && "N".equalsIgnoreCase(casesSearchVO.getExceedFlg()) )
					 {
						 //caseSts =  config.getString("preauth_caseStatus_HOLD");
						 caseSts =  config.getString(moduleType+"_holdcaseStatus_"+labelBean.getID());
					 }
					 else
             ...
             if(caseSts != null && labelBean.getID() != null && labelBean.getID().equals(config.getString("preauth_hold_role")) && !holdFlag)
		     {
            	 holdFlag = true; 
		     }
 		 ...
 		 if (caseSts != null && labelBean.getID() != null && labelBean.getID().equals(config.getString("EHF.Claims.CEX"))
		 	...
		 	for(EhfCase ehfCase:blockedCaseList)
			{
			  ...
			  */
			  query.append("  select distinct ac.caseId as caseId, ac.enhAmounts as enhAmounts, ac.caseNo as caseNo ,ac.flagged as flagged,ap.name as patientName ,  ap.cardNo as wapNo , acb.cmbDtlName as claimStatus ,ah.hospName as hospName,ah.hospId as hospId,ac.schemeId as schemeId,ac.casePatientNo as patientId  ");
			  query.append(" , ac.caseStatus as caseStatusId , ac.lstUpdDt  as lstUpdDt,ac.errClaimStatus as errStatusId, ap.patientId as patientId, ac.pendingFlag as pendingFlag,ac.preauthPckgAmt as preauthPckgAmt,ac.enhFlag as enhFlag, nvl(ac.enhReqAmt,0) as enhReqAmt,nvl(ac.preauthTotalPackageAmt,0) as totPckgAmt,ac.clmSubDt as SUBMITTEDDATE,case when ((ac.clmSubDt-ac.csDisDt)<=1  and ap.patientIpop='IP' and trunc(ac.actualClmSubDt)>=TO_DATE('11-11-2019','DD-MM-YYYY')) then ac.clmSubDt end as FORWRDDT ");
			  ...
			  if(casesSearchVO.getExcelFlag()!=null && !casesSearchVO.getExcelFlag().equalsIgnoreCase("")){
				  query.append("  ,to_char(ac.csPreauthDt, 'DD/MM/YYYY HH:MI:SS AM') as csPreauthDt, to_char(ac.preauthAprvDate, 'DD/MM/YYYY HH:MI:SS AM') as preauthAprvDate,to_char(ac.csDisDt, 'DD/MM/YYYY HH:MI:SS AM') as csDisDt,to_char(ac.csDeathDt, 'DD/MM/YYYY HH:MI:SS AM') as csDeathDt,to_char(ac.clmSubDt, 'DD/MM/YYYY HH:MI:SS AM') as clmSubDt,ac.csTransId as csTransId, to_char(ac.csTransDt, 'DD/MM/YYYY HH:MI:SS AM') as csTransDt,ac.csRemarks as csRemarks,to_char(ac.csSbhDt, 'DD/MM/YYYY HH:MI:SS AM') as csSbhDt ,");
				  query.append(" case when ap.patientIpop ='IPM' then nvl(ac.pckAppvAmt,0)  when ap.patientIpop ='IP' then nvl((nvl(ac.pckAppvAmt,0)+nvl(ac.comorbidAppvAmt,0)+nvl(ac.enhAppvAmt,0)),0) end as estimAmt,  cast(eccm.claimBillAmt as string)  as billedAmt, ");
				  query.append(" case when ap.patientIpop = 'IPM' then  'IP Medical' when ap.patientIpop = 'IP' then  'IP Surgical' end as patientType ");
			 ...
			 /* if(casesSearchVO!=null && ("Y").equalsIgnoreCase(casesSearchVO.getCeoFlag()))
			  ...
			  if(casesSearchVO.getCasesForApprovalFlag() != null && casesSearchVO.getCasesForApprovalFlag().equalsIgnoreCase("Y")&& !("Y").equalsIgnoreCase(pendingFlag))
			      if(casesSearchVO.getErrCaseApprovalFlag()!=null && casesSearchVO.getErrCaseApprovalFlag().equalsIgnoreCase("Y"))
				    	  {
					  	if(casesSearchVO.getUserRole()!=null && (mappedGroups.contains("~"+config.getString("preauth_medco_role")+"~")))
			    		  		query1.append(",    EhfCaseClaim eced ");
				    	  }
			...
			if((ppdFlag || cpdFlag || holdFlag) && !ppdComiteFlag )
			{
			...
			/*if(cpdFlag){
			  ...
			  */
			  ...
			  query1.append("  where ap.patientId = ac.casePatientNo   and acb.id.cmbDtlId = ac.caseStatus  and ah.hospId = ac.caseHospCode ");
			  if(casesSearchVO.getNimsFlag() != null && !"".equalsIgnoreCase(casesSearchVO.getNimsFlag()) && "Y".equalsIgnoreCase(casesSearchVO.getNimsFlag()))
				  query1.append(" and ac.nabhHosp is null and ac.caseHospCode in ('"+ config.getString("HOSP_NIMS") +"','"+config.getString("HOSP_MNJ")+"') ");
			  else
				  query1.append(" and ac.nabhHosp='Y' ");
			  /*if(casesSearchVO.getCaseSearchType()==null || casesSearchVO.getCaseSearchType().equalsIgnoreCase("") || !casesSearchVO.getCaseSearchType().equalsIgnoreCase("ChronicOp")){
			  ...
			  if(casesSearchVO.getCasesForApprovalFlag() != null && casesSearchVO.getCasesForApprovalFlag().equalsIgnoreCase("Y")&& !("Y").equalsIgnoreCase(pendingFlag))
			      if(casesSearchVO.getErrCaseApprovalFlag()!=null && casesSearchVO.getErrCaseApprovalFlag().equalsIgnoreCase("Y"))
				    	  {
					  	if(casesSearchVO.getUserRole()!=null && (mappedGroups.contains("~"+config.getString("preauth_medco_role")+"~")))
			    		  		query1.append(" and ac.caseId=eced.caseId ");
				    	  }
			  if(casesSearchVO.getErrDentalCaseApprovalFlag()!=null && casesSearchVO.getErrDentalCaseApprovalFlag().equalsIgnoreCase("Y")){
				  if(casesSearchVO.getRoleId()!=null && (mappedGroups.contains("~"+config.getString("EHF.Claims.CH")+"~"))){
			  ...
			  //vijay
			  if(casesSearchVO.getSearchCaseType() != null && casesSearchVO.getSearchCaseType().equals("IPM"))
			  {
				  query1.append(" and ap.patientIpop ='IPM' ");
				  
			   }
			  if(casesSearchVO.getSearchCaseType() != null && casesSearchVO.getSearchCaseType().equals("IP"))
			  {
				  query1.append(" and ap.patientIpop ='IP' ");
				  
			   }
			  if(casesSearchVO.getSearchCaseType() != null && casesSearchVO.getSearchCaseType().equals("IPD"))
			  {
				  query1.append(" and ap.patientIpop ='IPD' ");
			 ...
			 */
			  ...
			  if(casesSearchVO.getCasesForApprovalFlag() != null && casesSearchVO.getCasesForApprovalFlag().equalsIgnoreCase("Y")&& !("Y").equalsIgnoreCase(pendingFlag))
				 {
			      ...
			      if(casesSearchVO.getErrCaseApprovalFlag()!=null && casesSearchVO.getErrCaseApprovalFlag().equalsIgnoreCase("Y")){
			    	  //query1.append(" and nvl(ac.enhAmounts,0)+nvl(ac.comorbidAppvAmt,0)+nvl(ac.enhAppvAmt,0)-nvl(ac.csClAmount,0) > 0 ");
					  //if(casesSearchVO.getUserRole()!=null && !casesSearchVO.getUserRole().equalsIgnoreCase("") && (casesSearchVO.getUserRole().equalsIgnoreCase(config.getString("preauth_medco_role"))))
					  	if(casesSearchVO.getUserRole()!=null && (mappedGroups.contains("~"+config.getString("preauth_medco_role")+"~")))
					  	{  query1.append(" and nvl(eced.claimBillAmt,0)-nvl(ac.csClAmount,0)- nvl(ac.penaltyAmount,0)> 0 "); 
			      ...
			      /* added by satish kola for erroneous dental claim cases*/
					  ...
					  if(casesSearchVO.getDisallowanceFlag()==null||"".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag())||!"Y".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag()))
					  {
					  if(!moduleType.equalsIgnoreCase("claim") && casesSearchVO.getUserId()!=null && !casesSearchVO.getUserId().equalsIgnoreCase("") && (mappedGroups.contains(config.getString("userGroup_eo")))&&((casesSearchVO.getExceedFlg()==null ||"".equals(casesSearchVO.getExceedFlg()))&& !"N".equalsIgnoreCase(casesSearchVO.getExceedFlg())))
					  {
					  ...
					  else if(moduleType.equalsIgnoreCase("claim") && casesSearchVO.getUserRole()!=null && !casesSearchVO.getUserRole().equalsIgnoreCase("") && (casesSearchVO.getUserRole().equalsIgnoreCase(config.getString("preauth_medco_role"))))
					  {
						 ...
						 if(caseStatus != null && caseStatus.size() >0 && !caseStatus.contains("CD21"))
						  query1.append(" ac.revertFlag is NULL and ");
					  }
					  else if(moduleType.equalsIgnoreCase("drugbreakup"))
					  {
						  query1.append(" and ap.patientIpop ='IPM' ");
					  }
					  else 
					  ...
					  int statusCount=0;
					  if(moduleType.equalsIgnoreCase("drugbreakup"))
					  {
						 
						  query1.append(" and  ac.preauthCancelDt is NULL ");
						  query1.append(" and  ac.revertFlag='Y' ");
						  query1.append(" and ac.preauthRejDate is NULL "); 
						  query1.append(" and ac.caseId in (select dn.caseId from EhfPatientDrugsNabh dn  where dn.activeYN='Y') "); 
						  /*query1.append(" and ac.clmSubDt is NOT NULL "); */
					 ...
					 if(moduleType!=null && ((ptdFlag && moduleType.equalsIgnoreCase("preauth"))
					  ...
					  else if(!"".equalsIgnoreCase(casesSearchVO.getExceedFlg()) && "Y".equalsIgnoreCase(casesSearchVO.getExceedFlg()) )
					  {
						  query1.append("  ac.exceedFlg='Y' ");
					  }
					  else if (moduleType.equalsIgnoreCase("drugbreakup"))
					  {
						  query1.append(" and ac.caseStatus in ('CD2589','CD21','CD49','CD427','CD44','CD47','CD1332','CD515')");
					  }
					  else
						  query1.append("  ac.caseStatus in ('') ");
				  }
			       ...
			       */
			  if(moduleType.equalsIgnoreCase("claim") && casesSearchVO.getUserRole()!=null && !casesSearchVO.getUserRole().equalsIgnoreCase("") && (casesSearchVO.getUserRole().equalsIgnoreCase(config.getString("preauth_medco_role")))&&casesSearchVO.getDisallowanceFlag()!=null&&!"".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag())&&"Y".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag()))
			  {
				  query1.append(" and ac.caseId  in ( select distinct id.caseId from EhfClaimDeductionDtls where  id.caseId not in ( select caseId from  SgvaCRMgmtDtls where crWorkflowId='CONTEST_AMT' and caseId is not null ) )   ");
					String disallunsedStatus=config.getString("Disallowance_Unused_status");
				  List<String>   DisalnceUnused= new ArrayList<String>();
				     if(disallunsedStatus != null && !disallunsedStatus.equalsIgnoreCase(""))
		 		     {
		 		     StringTokenizer str = new StringTokenizer(disallunsedStatus,"~");
		 			  while(str.hasMoreTokens())
		 			  {
		 				 DisalnceUnused.add(str.nextElement().toString());  
		 			  }
		 			   
		 		     }

				  if(DisalnceUnused != null && DisalnceUnused.size() >0)
					  query1.append("  and ac.caseStatus not in ( ");  
				  
				 for(int k=0; k<DisalnceUnused.size() ;k++)
				 {
				   if(k!=0 && k!=DisalnceUnused.size())
				     {
					   query1.append(" , ");  
				     }
				   query1.append(" '"+DisalnceUnused.get(k)+"' ");	
				 }
				 if(DisalnceUnused != null && DisalnceUnused.size() >0)
				  query1.append(" ) ");  
			      }
			  ...
			  if((ppdFlag||cpdFlag||holdFlag) && !ppdComiteFlag)
		  		{
			  		...
			  		if(ppdFlag && cpdFlag && holdFlag)
			  			query1.append(" and spm.id.grpId in('"+config.getString("preauth_ppd_role")+"','"+config.getString("claim_cpd_role")+"','"+config.getString("preauth_hold_role")+"') ");
			  		else if(cpdFlag)
			  			...
			  			query1.append(" and spm.id.grpId in('"+config.getString("preauth_hold_role")+"') " );
		      ...
		      if(ppdComiteFlag || ppdFlag || cpdFlag || holdFlag)
		      {
			...
			/*  
			  ...
			  if(pendingFlag!=null && ("Y").equalsIgnoreCase(pendingFlag))
			  {
				  ...
				  else if(("hold").equalsIgnoreCase(moduleType) && !("Y").equalsIgnoreCase(dentalFlag))
					  claimStatus =  ClaimsConstants.holdSentBackStatuses;
			 ...
			 if(!"".equalsIgnoreCase(casesSearchVO.getExceedFlg()) && "Y".equalsIgnoreCase(casesSearchVO.getExceedFlg()) )
				 	query1.append(" and ac.exceedFlg='Y' ");
			 			 if(casesSearchVO.getNimsFlag() != null && !"".equalsIgnoreCase(casesSearchVO.getNimsFlag()) && !"Y".equalsIgnoreCase(casesSearchVO.getNimsFlag()))
			 	query1.append(" and ac.skipFlag='N' ");
			  if(!casesSearchVO.getCasesForApprovalFlag().equalsIgnoreCase("Y")){
			  ...
			  {
				  query1.append(" order by case when ((ac.clmSubDt-ac.csDisDt)<=1  and ap.patientIpop='IP' and trunc(ac.actualClmSubDt)>=TO_DATE('11-11-2019','DD-MM-YYYY')) then ac.clmSubDt end asc, ac.clmSubDt asc ");   
			  }
			  else
			  {
...
//							System.out.println(userRole);
			  ...
			  if((!"".equalsIgnoreCase(lStCasesSearchVO.getCaseId()) && lStCasesSearchVO.getCaseId() != null) && ("Y").equalsIgnoreCase(casesSearchVO.getColorCodeView()))
			  {
						  List<EhfCaseTherapy> spcltyList= new ArrayList<EhfCaseTherapy>();
							Map<String, Object> thraphyMap=new HashMap<String,Object>();
							thraphyMap.put("caseId", lStCasesSearchVO.getCaseId());
							spcltyList = genericDao.findAllByPropertyMatch(EhfCaseTherapy.class, thraphyMap);
							
					if(spcltyList.size()>1)
					{
						lStCasesSearchVO.setColorCode("Multi");
					}
					else if(spcltyList.size()>0)
					{
						if(spcltyList != null)
						{	
							 if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S7"))
							{
								lStCasesSearchVO.setColorCode("Violet");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("M5"))
							{
								lStCasesSearchVO.setColorCode("Indigo");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S18"))
							{
								lStCasesSearchVO.setColorCode("Blue");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S12"))
							{
								lStCasesSearchVO.setColorCode("Green");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("M7"))
							{
								lStCasesSearchVO.setColorCode("Yellow");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S10"))
							{
								lStCasesSearchVO.setColorCode("Orange");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S3"))
							{
								lStCasesSearchVO.setColorCode("Red");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S13"))
							{
								lStCasesSearchVO.setColorCode("Black");
							}
							else if(!"".equalsIgnoreCase(spcltyList.get(0).getAsriCatCode()) && spcltyList.get(0).getAsriCatCode().equalsIgnoreCase("S11"))
							{
								lStCasesSearchVO.setColorCode("Pink");
							}
							else
							{
								lStCasesSearchVO.setColorCode("");
							}
						  }
					}
					else
		  ...
		  try{
			 ...
			 for(LabelBean labelBean:casesSearchVO.getGrpList())
				 ...
				 */
				  ...
				  * end of getting status for cases for approval
				  */
				  query.append("  select distinct ac.CASE_ID ,ac.CASE_NO  ,ac.flagged ,ap.name ,  ap.card_no , acb.cmb_dtl_name  ,ah.hosp_name ,ah.hosp_id ,ac.SCHEME_ID ,ac.CASE_PATIENT_NO,ac.CLM_SUB_DT as SUBMITTEDDATE   ");
				...
				if(casesSearchVO.getSearchFlag() != null && casesSearchVO.getSearchFlag().equalsIgnoreCase("Y"))
				 	{
						if((casesSearchVO.getMainCatName() != null && !casesSearchVO.getMainCatName().equals("") && !casesSearchVO.getMainCatName().equals("-1")) || (casesSearchVO.getCatName() != null && !casesSearchVO.getCatName().equals("") && !casesSearchVO.getCatName().equals("-1")))
						    {
							  	query1.append("  , EHF_CASE_THERAPY acs ,  EHFM_MAIN_CATEGORY_NABH emc ");  
								if(casesSearchVO.getProcName() != null && !casesSearchVO.getProcName().equals("") && !casesSearchVO.getProcName().equals("-1"))
								    {
									  	query1.append("   ,  EHFM_MAIN_THERAPY_NABH etp ");  
								    }
						    }
				 	}
				/*if(cpdFlag){
			  ...
			  */
			  ...
			  /*if(casesSearchVO.getCaseSearchType()==null || casesSearchVO.getCaseSearchType().equalsIgnoreCase("") || !casesSearchVO.getCaseSearchType().equalsIgnoreCase("ChronicOp")){
			    ...
			    if(casesSearchVO.getNimsFlag() != null && !"".equalsIgnoreCase(casesSearchVO.getNimsFlag()) && "Y".equalsIgnoreCase(casesSearchVO.getNimsFlag()))
				  query1.append(" and ac.NABH_HOSP is null and ac.CASE_HOSP_CODE in ('"+ config.getString("HOSP_NIMS") +"','"+ config.getString("HOSP_MNJ")+"') ");
			  else
				  query1.append(" and ac.NABH_HOSP='Y' ");
			  if(casesSearchVO.getSurgyId() != null && !casesSearchVO.getSurgyId().equals(""))
				  {
...
//	         System.out.println(count);
	...
	public List<CasesSearchVO> getDeathCases(CasesSearchVO casesSearchVO)
	{
		...
		try
		{
			...
			if(casesSearchVO.getSchemeVal()!=null && !"".equalsIgnoreCase(casesSearchVO.getSchemeVal()))
			{
				if(casesSearchVO.getSchemeVal().equalsIgnoreCase("CD203"))
					query.append(" and ec.schemeId in ('CD201','CD202','1') ");
				else 
					query.append(" and ec.schemeId ='"+casesSearchVO.getSchemeVal()+"' ");
			}
			query.append("and ec.nabhHosp='Y' ");
	...
	public CasesSearchVO getListCasesDirect(CasesSearchVO casesSearchVO)
	{
		...
		try {
			 ...
			 * end of getting status for cases for approval
			 */
			query.append(" select distinct ac.caseId as caseId,ac.caseRegnDate as InpatientCaseSubDt, ac.caseStatus as caseStatusId,ac.schemeId as schemeId,ac.clmSubDt as SUBMITTEDDATE,case when ((ac.clmSubDt-ac.csDisDt)<=1  and ap.patientIpop='IP' and trunc(ac.actualClmSubDt)>=TO_DATE('11-11-2019','DD-MM-YYYY')) then ac.clmSubDt end as FORWRDDT ");
			...
			query.append("  where ap.patientId = ac.casePatientNo   and acb.id.cmbDtlId = ac.caseStatus  and ah.hospId = ac.caseHospCode and ac.nabhHosp='Y'  ");
			...
			/*
			 * Added for separation of Medical & Surgical Cases
			 */
			if(!"".equalsIgnoreCase(casesSearchVO.getMedFlg()) && "Y".equalsIgnoreCase(casesSearchVO.getMedFlg()))
			{
				query.append(" and ap.patientIpop in ('IPM') ");
			}
			else if(!"".equalsIgnoreCase(casesSearchVO.getMedFlg()) && "N".equalsIgnoreCase(casesSearchVO.getMedFlg()))
			{
				query.append(" and ap.patientIpop in ('IP','IPD') ");
			}
			else
			{
				query.append(" and ap.patientIpop in ('IP','IPD','IPM','DOP') ");
			}
			if (casesSearchVO.getUserRole() != null
					&& !casesSearchVO.getUserRole().equalsIgnoreCase("")
			...
			if((casesSearchVO.getOrganFlagNew()!=null && "O".equalsIgnoreCase(casesSearchVO.getOrganFlagNew())) && (pexFlag || chFlag || cexFlag || eoFlag || eocommiteeFlag))
			{
		...
		/*	if((casesSearchVO.getOrganFlagNew()!=null && "S".equalsIgnoreCase(casesSearchVO.getOrganFlagNew())) && (ppdFlag || cpdFlag) )
			{
				query.append("  and ac.lstUpdUsr not in ('"+casesSearchVO.getUserId()+"') ");
			}*/
				...
				if (casesSearchVO.getErrCaseApprovalFlag() != null
					...
					if (!moduleType.equalsIgnoreCase("claim")
							...
							&& (mappedGroups.contains(config
									.getString("userGroup_eo")))&&(casesSearchVO.getExceedFlg()==null&& !"N".equalsIgnoreCase(casesSearchVO.getExceedFlg()))) {
...
if((chFlag || cexFlag) && !("Y").equalsIgnoreCase(dentalFlag))
{
	...
	query.append(" and ect.asriCatCode  not in ('S18')");
			...
			if (casesSearchVO.getSchemeVal() != null
					&& !casesSearchVO.getSchemeVal().equalsIgnoreCase("")) {
				if (casesSearchVO.getSchemeVal().equalsIgnoreCase("CD202"))
					query.append(" and ac.schemeId in ('CD202','1') ");
				else if (casesSearchVO.getSchemeVal().equalsIgnoreCase("CD203"))
					query.append(" and ac.schemeId in ('CD201','CD202','1') ");
				else
					query.append(" and ac.schemeId in ('"
							+ casesSearchVO.getSchemeVal() + "') ");
			}
			...
			/*if(moduleType.equalsIgnoreCase("claim"))
			  ...
			  if(moduleType!=null && ("claim").equalsIgnoreCase(moduleType) && !"".equalsIgnoreCase(casesSearchVO.getMedFlg()) && "N".equalsIgnoreCase(casesSearchVO.getMedFlg()))
			  {
				  query.append("   order by case when ((ac.clmSubDt-ac.csDisDt)<=1  and ap.patientIpop='IP' and trunc(ac.actualClmSubDt)>=TO_DATE('11-11-2019','DD-MM-YYYY')) then ac.clmSubDt end asc, ac.clmSubDt asc ");   
			  }
			  else if(moduleType!=null && ("claim").equalsIgnoreCase(moduleType) && !"".equalsIgnoreCase(casesSearchVO.getMedFlg()) && "Y".equalsIgnoreCase(casesSearchVO.getMedFlg()))
			  {
			...
			lstCases = genericDao.executeHQLQueryList(CasesSearchVO.class,query.toString());
	...
	public CasesSearchVO getListCasesNabh(CasesSearchVO casesSearchVO) {
		List<CasesSearchVO> nabhreports=new ArrayList<CasesSearchVO>();
		SessionFactory factory=hibernateTemplate.getSessionFactory();
		Session session=factory.openSession();
		CasesSearchVO casesVo = new CasesSearchVO();
	  ...
	  try
	  {
				...
				query1.append("select CASEID,CASENO,PATIENTNAME,CARDNO,EMPTYPE,AGE,GENDER,PASTILLNESS,CONSUMAMT,CATEGORYCODE,CATEGORYNAME,PROCCODE,PROCNAME,CLIAMSTATUS,HOSPNAME,HOSPID,STATUSDATE,CLAIMSUBDATE,");
				query1.append("TDSAMT,RFAMT,HOSPAMT,IPCASEREGDATE,CLAIMAMOUNT,CLAIMNUM,PREAUTHDATE,PREAUTHAPPVDATE,DISCHDATE,DEATHDATE,TRANSID,TRANSDATE,REMARKS,SBHPAIDDATE,ESTIMATEDAMT,BILLEDAMT,PATIENTTYPE from " );
				 query1.append(" (select distinct ec.CASE_ID as CASEID, ec.CASE_NO as CASENO, ep.name as PATIENTNAME, ep.card_no as CARDNO, ");
				 query1.append("  ep.card_type as EMPTYPE, ");
				 query1.append("   to_char(ep.age) as AGE, ep.gender as GENDER, eph.history_illness as PASTILLNESS, ");
				 query1.append(" to_char(nvl(sum(ece.amount + epd.amount), 0)) as CONSUMAMT, T.CATEGORYCODE as CATEGORYCODE,T.CATEGORYNAME as CATEGORYNAME, T.PROCCODE as PROCCODE, T.PROCNAME as PROCNAME,  ");
				 query1.append(" ecm.cmb_dtl_name as CLIAMSTATUS,  eh.hosp_name as HOSPNAME, eh.hosp_id as HOSPID, to_char(ec.LST_UPD_DT, 'DD/MM/YYYY HH:MI:SS AM') as STATUSDATE, to_char(ec.CLM_SUB_DT, 'DD/MM/YYYY HH:MI:SS AM') as CLAIMSUBDATE, ");
				 query1.append(" to_char(ecc.TDS_PCT_AMT) as TDSAMT, to_char(ecc.TRUST_PCT_AMT) as RFAMT, to_char(ecc.HOSP_PCT_AMT) as HOSPAMT,to_char(ec.CASE_REGN_DATE, 'DD/MM/YYYY HH:MI:SS AM') as IPCASEREGDATE, ");
				 query1.append(" to_char(nvl(ec.CS_CL_AMOUNT, 0)) as CLAIMAMOUNT, ec.CLAIM_NO as CLAIMNUM, to_char(ec.cs_preauth_dt, 'DD/MM/YYYY HH:MI:SS AM') as PREAUTHDATE, ");
				 query1.append(" to_char(ec.preauth_aprv_dt, 'DD/MM/YYYY HH:MI:SS AM') as PREAUTHAPPVDATE, to_char(ec.CS_DIS_DT, 'DD/MM/YYYY HH:MI:SS AM') as DISCHDATE, to_char(ec.CS_DEATH_DT, 'DD/MM/YYYY HH:MI:SS AM') as DEATHDATE, ec.cs_trans_id as TRANSID, ");
				 query1.append(" to_char(ec.cs_trans_dt, 'DD/MM/YYYY HH:MI:SS AM') as TRANSDATE,  ec.cs_remarks as REMARKS, to_char(ec.cs_sbh_dt, 'DD/MM/YYYY HH:MI:SS AM') as SBHPAIDDATE,   ");
				 query1.append("  to_char(nvl(nvl(ec.PCK_APPV_AMT, 0) + nvl(ec.comorbid_appv_amt, 0) +  nvl(ec.enh_appv_amt, 0),0)) as ESTIMATEDAMT, ");
				 query1.append(" cast(ecc.claim_bill_amt as varchar2(255 char)) as BILLEDAMT, ep.patient_ipop  as PATIENTTYPE ");
				 
				 query.append(" from ehfm_cmb_dtls   ecm, EHFM_HOSPITALS eh, EHF_PATIENT ep, ehf_case_enhancement_dtls  ece, ehf_patient_drugs_nabh  epd, ");
				 query.append(" EHF_PATIENT_HOSP_DIAGNOSIS eph, EHF_CASE  ec, EHF_CASE_CLAIM ecc,");
				 query.append (" (select ECT.CASE_ID CASE_ID, WM_CONCAT(distinct ES.DIS_MAIN_ID) as CATEGORYCODE, WM_CONCAT(distinct ES.DIS_MAIN_NAME) as CATEGORYNAME, ");
				 query.append (" WM_CONCAT(distinct EMT.ICD_PROC_CODE) as PROCCODE, WM_CONCAT(distinct EMT.PROC_NAME) as PROCNAME ");
				 query.append (" from EHF_CASE_THERAPY ECT, EHFM_SPECIALITIES ES, EHFM_MAIN_CATEGORY_NABH EMC, EHFM_MAIN_THERAPY_NABH EMT ");
				 query.append (" WHERE ect.asri_cat_code = emt.asri_code and ect.icd_proc_code = emt.icd_proc_code and es.dis_main_id = emt.asri_code ");
				 query.append (" and emc.icd_cat_code = ect.icd_cat_code and emt.state = 'CD202' GROUP BY ECT.CASE_ID ) T  ");
		 
				 query.append(" where ep.PATIENT_ID = ec.CASE_PATIENT_NO  and ecm.cmb_dtl_id = ec.CASE_STATUS and ec.CASE_ID = ecc.case_id(+) and eph.patient_id = ep.patient_id ");
				 query.append(" and ece.active_y_n(+) = 'Y'  and ec.case_id = ece.case_id(+) and ec.case_id = epd.case_id(+) and epd.active_yn(+) = 'Y' ");
				 query.append(" AND ec.case_id = T.case_id(+)  and eh.hosp_id = ec.CASE_HOSP_CODE and ec.NABH_Hosp = 'Y'  and (ec.CASE_STATUS not in ('CD4', 'CD5')) ");
				 query.append(" and ec.SCHEME_ID in ('CD202')   and ec.skip_flag = 'N' ");
				   
				  
		  if(casesSearchVO.getFromDate() != null && casesSearchVO.getToDate() != null && !"".equalsIgnoreCase(casesSearchVO.getFromDate()) && !"".equalsIgnoreCase(casesSearchVO.getToDate()))
		  {
			   query.append(" and ec.CRT_DT between to_date('"+casesSearchVO.getFromDate()+"', 'dd/MM/yyyy') and to_date('"+casesSearchVO.getToDate()+"', 'dd/MM/yyyy')  ");
			 
		  }
		  if(casesSearchVO.getSchemeId() != null && !"-1".equalsIgnoreCase(casesSearchVO.getSchemeId()))
		  {
			   query.append(" and ec.patient_scheme in ( '"+casesSearchVO.getSchemeId()+"' ) ");
			 
		  }
			 query.append(" group by ec.CASE_ID, ec.CASE_NO,  ep.name, ep.card_no, ep.card_type,  ep.age, ep.gender, eph.history_illness, ");
			 query.append(" ecm.cmb_dtl_name, eh.hosp_name, eh.hosp_id,  ec.LST_UPD_DT,  ");
			 query.append(" ec.CLM_SUB_DT, ecc.TDS_PCT_AMT, ecc.TRUST_PCT_AMT, ecc.HOSP_PCT_AMT,   T.CATEGORYCODE, T.CATEGORYNAME, T.PROCCODE,T.PROCNAME, ");
			 query.append("  ec.CASE_REGN_DATE,  ec.CS_CL_AMOUNT, ec.CLAIM_NO,  ec.cs_preauth_dt,  ");
			 query.append(" ec.preauth_aprv_dt,  ec.CS_DIS_DT, ec.CS_DEATH_DT, ec.CLM_SUB_DT,  ec.cs_trans_id, ec.cs_trans_dt, ec.cs_remarks, ec.cs_sbh_dt,  ");
			 query.append(" ec.PCK_APPV_AMT, ec.enh_appv_amt, ec.comorbid_appv_amt, ecc.claim_bill_amt,ep.patient_ipop,ecc.claim_bill_amt  ");
		  ...
		  if(casesSearchVO.getCsvMsg()!=null && "Y".equalsIgnoreCase(casesSearchVO.getCsvMsg()))
			{
			
			  nabhreports=session.createSQLQuery((query1.append(newquery)).toString()).setResultTransformer(Transformers.aliasToBean(CasesSearchVO.class)).list();
			  casesVo.setNabhReportList(nabhreports);
			 	return casesVo;
			}
		  nabhreports=session.createSQLQuery((query2.append(query)).toString()).setResultTransformer(Transformers.aliasToBean(CasesSearchVO.class)).list();
		  
		  
		  
		  if(nabhreports!=null)
			{
				if(nabhreports.size()>0)
					{
						if(nabhreports.get(0)!=null)
							{
								if(nabhreports.get(0).getCOUNT().intValue()!=0)
									{
									
										totalRecords=nabhreports.size();
										if(casesSearchVO.getMaxresults()==0)
											maxResults=10;
										else
											maxResults=casesSearchVO.getMaxresults();
										
										if(casesSearchVO.getPageId()==0)
											{
												pageId=1;
												startIndex=0;
											}
											...
											{
												pageId=casesSearchVO.getPageId();
										...
										nabhreports=new ArrayList<CasesSearchVO>();
										
												
					nabhreports=session.createSQLQuery((query1.append(newquery).toString())).setFirstResult(startIndex).setMaxResults(maxResults).setResultTransformer(Transformers.aliasToBean(CasesSearchVO.class)).list();
	  ...
	  casesVo.setTotalPages(totalPages);
	  casesVo.setTotalRecords(totalRecords);
	  casesVo.setPageId(pageId);
	  casesVo.setMaxresults(maxResults);
	  casesVo.setStrtIndex(startIndex);
	  casesVo.setNabhReportList(nabhreports);
	 		 ...
	 		 if (caseSts != null && labelBean.getID() != null && labelBean.getID().equals(config.getString("EHF.Claims.CEX"))
			 	...
			 	for(EhfCase ehfCase:blockedCaseList)
				{
				  ...
				  */
				  ...
				  query.append(" , ac.case_Status as CASESTATUS , ac.lst_Upd_Dt  as LSTUPDDT,ac.err_clm_status as ERRSTATUSID, ap.patient_Id as PATIENTID, ac.pending_Flag as PENDINGFLAG,ac.preauth_Pckg_Amt as PREAUTHPCKGAMT,ac.enh_Flag as ENHFLAG,ac.clm_Sub_Dt as SUBMITTEDDATE  ");
				  if(casesSearchVO.getExcelFlag()!=null && !casesSearchVO.getExcelFlag().equalsIgnoreCase(""))
				  	{
				...
				/*if(cpdFlag){
				  ...
				  */
				  ...
				  query1.append("  where ap.patient_Id = ac.case_Patient_No   and acb.cmb_Dtl_Id = ac.case_Status  and ah.hosp_Id = ac.case_Hosp_Code  and ac.nabh_Hosp='Y' ");
				  /*if(casesSearchVO.getCaseSearchType()==null || casesSearchVO.getCaseSearchType().equalsIgnoreCase("") || !casesSearchVO.getCaseSearchType().equalsIgnoreCase("ChronicOp")){
					  query1.append(" and ap1.patientId = ac.casePatientNo and ap.patientId= ap1.patientId ");
				  }*/
				  if(casesSearchVO.getErrDentalCaseApprovalFlag()!=null && casesSearchVO.getErrDentalCaseApprovalFlag().equalsIgnoreCase("Y")){
				  ...
				  if(casesSearchVO.getCasesForApprovalFlag() != null && casesSearchVO.getCasesForApprovalFlag().equalsIgnoreCase("Y")&& !("Y").equalsIgnoreCase(pendingFlag))
					 {
				      ...
				      if(moduleType.equalsIgnoreCase("claim") && casesSearchVO.getUserRole()!=null && !casesSearchVO.getUserRole().equalsIgnoreCase("") && (casesSearchVO.getUserRole().equalsIgnoreCase(config.getString("preauth_medco_role")))&&casesSearchVO.getDisallowanceFlag()!=null&&!"".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag())&&"Y".equalsIgnoreCase(casesSearchVO.getDisallowanceFlag()))
					  {
				    	  query1.append(" and ac.case_Id  in ( select distinct case_Id from Ehf_Claim_Deduction_Dtls where  case_Id not in ( select case_Id from  Sgva_CRMgmt_Dtls where cr_Workflow_Id='CONTEST_AMT' and case_Id is not null ) )   ");
							String disallunsedStatus=config.getString("Disallowance_Unused_status");
						  List<String>   DisalnceUnused= new ArrayList<String>();
						     if(disallunsedStatus != null && !disallunsedStatus.equalsIgnoreCase(""))
				 		     {
						  ...
						  if(DisalnceUnused != null && DisalnceUnused.size() >0)
							  query1.append("  and ac.case_Status not in ( ");  
