...
import com.ahct.CEO.service.CeoWorkListServiceImpl;
...
//	    	System.out.println(request.getParameter("username"));
	    	...
	    	if(loLoginFlag)
	    	{
				...
				session.setAttribute("lowStockAlert", lstEmpDetails.getLowStockAlert());
				System.out.println("lowstockY"+lstEmpDetails.getLowStockAlert());
				...
				if(lstEmpDetails.getDSGNNAME().equalsIgnoreCase("REFERRED CENTER DOCTOR"))
				{
				session.setAttribute("isReferredDoc", "Y");
			    if(lstEmpDetails.getGrpList()!=null && lstEmpDetails.getGrpList().get(0)!=null && lstEmpDetails.getGrpList().get(0).getID()!=null)
			    {
			    	String grp=lstEmpDetails.getGrpList().get(0).getID();
				...
				/*Added for NABH Medcos and Mithra Logins By Namratha   */
				for(LabelBean grpList:lstEmpDetails.getGrpList())
				{
						if(grpList.getID()!=null && (grpList.getID().equalsIgnoreCase(ClaimsConstants.MEDCO_GRP) || grpList.getID().equalsIgnoreCase(ClaimsConstants.MITHRA_GRP)))
						{
							nabhFlag=true;													
						}																	
				}
				...
				/*End of NABH Medcos and Mithra Logins By Namratha   */
				...
				**/
				...
				if(lstEmpDetails.getUSERID()!=null && !"".equalsIgnoreCase(lstEmpDetails.getDSGNNAME()) && lstEmpDetails.getDSGNNAME()!=null && !"".equalsIgnoreCase(lstEmpDetails.getDSGNNAME()) && ("MEDCO").equalsIgnoreCase(lstEmpDetails.getDSGNNAME()))
				{
					...
					List<LabelBean> expireCasesList= new ArrayList<LabelBean>();
					if (casesFlag != null && !"".equalsIgnoreCase(casesFlag) && casesFlag.equalsIgnoreCase("true"))
					{
						expireCasesList= loginService.getCasesAbtToExpire(lstEmpDetails.getUSERID());
						if(expireCasesList != null && expireCasesList.size()>0)
						{
							if(expireCasesList != null)
							session.setAttribute("expireCasesList", expireCasesList);
						}
						else
							session.setAttribute("expireCasesList", "");
					}
					else
						session.setAttribute("expireCasesList", "");
					...
					if(nimsHosp!=null && nimsHosp.equalsIgnoreCase("EHS34")) {
						List<LabelBean> cancledCasesList= flaggingService.getCancelledCases();
				...
				if(lstEmpDetails.getUSERID()!=null && !"".equalsIgnoreCase(lstEmpDetails.getUSERID())){
					...
					for(int i=0;i<lstEmpDetails.getGrpList().size();i++){
						...
						if(lstEmpDetails.getGrpList().get(i).getID().equalsIgnoreCase(config.getString("Group.DATAOP"))||lstEmpDetails.getGrpList().get(i).getID().equalsIgnoreCase(config.getString("Group.REFDOC")))
						{
							hospId=loginService.getHospitslId(lstEmpDetails.getUSERID(), lstEmpDetails.getGrpList().get(i).getID());
						break;
						}
				...
				request.setAttribute("groupList", grp);
				}
				request.setAttribute("showDashboard", showDashboard);
	    	      ...
	    	      if(lstEmpDetails.getDIGILOGIN()==null || "".equalsIgnoreCase(lstEmpDetails.getDIGILOGIN()) || lstEmpDetails.getDIGILOGIN().equalsIgnoreCase("N"))
	    	      {
	    	    	  String caseEndTime = sds.format(new Date().getTime());
			            loggingService.logTime("LoginLoadingTime", lstEmpDetails.getUSERNO(), caseStartTime, caseEndTime,lstEmpDetails.getUSERID(), "EHS_OperationsNABH");
	    	      }
...
//					System.out.println(OTP);
		...
		if(lStrFlagStatus!= null && lStrFlagStatus.equalsIgnoreCase("incrementTokenNo"))
		{
			String dispensaryId="";
			...
			int flag=0;
			  String lStrUserId=  (String) session.getAttribute("UserID");
			   List<LabelBean> roleIdList= (List<LabelBean>) session.getAttribute("groupList");
			   
			   
			   if(roleIdList!=null && roleIdList.size()>0){
//				   System.out.println("roleIdList.get(0).getID()"+roleIdList.get(0).getID());
			   if(roleIdList.get(0).getID().contains("GP790"))
			   {
				   roleId="GP790";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP791"))
			   {
				   roleId="GP791";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP792"))
			   {
				   roleId="GP792";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP793"))
			   {
				   roleId="GP793";
				   flag=1;
			   }
			  /* else if(roleIdList.get(0).getID().contains("GP794"))
			   {
				   roleId="GP794";
				   flag=1;
			   }*/
			   else if(roleIdList.get(0).getID().contains("GP795"))
			   {
				   roleId="GP795";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP796"))
			   {
				   roleId="GP796";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP798"))
			   {
				   roleId="GP798";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP799"))
			   {
				   roleId="GP799";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP800"))
			   {
				   roleId="GP800";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP802"))
			   {
				   roleId="GP802";
				   flag=1;
			   }
			   else if(roleIdList.get(0).getID().contains("GP803"))
			   {
				   roleId="GP803";
				   flag=1;
			   }
//			   System.out.println("roleId:"+roleId+"-flag:"+flag);
			   if(flag==1)
			   {
				hospIds=patientService.getReferredCenterDtls(lStrUserId, roleId);
				dispensaryId=hospIds.get(0).getID();
//				System.out.println(dispensaryId);
				hospName=hospIds.get(0).getVALUE();
				String paramValue=request.getParameter("arg");
					
				String tokenVal="";
				if(paramValue!=null && !"".equalsIgnoreCase(paramValue) && "inc".equalsIgnoreCase(paramValue))
				 tokenNo=patientService.updateRoomTokenNo(dispensaryId, roleId, lStrUserId);
				if(paramValue!=null && !"".equalsIgnoreCase(paramValue) && "onload".equalsIgnoreCase(paramValue))
					tokenVal=patientService.getRoomTokenNo(dispensaryId, roleId, lStrUserId);
					
				if(!"".equalsIgnoreCase(tokenVal))
						{
					tokenNo=Integer.parseInt(tokenVal);
						}
//				System.out.println("Token No:"+tokenNo);
				
				session.setAttribute("tokenNo", tokenNo);
		 ...
		 if(lStrFlagStatus!=null && lStrFlagStatus.equalsIgnoreCase("logout"))
         {
			...
			if(seqId == null && "".equalsIgnoreCase(seqId))
				seqId = (String) session.getAttribute("sessionSeqId");
			vo.setSeqId(seqId);
		    ...
		    if ( lStrFlagStatus != null && lStrFlagStatus.equalsIgnoreCase ( "viewAttachment" ) )
		      {
		    	 ...
		    	 if(newAttachFlg!=null && newAttachFlg.length()>0 && newAttachFlg.equalsIgnoreCase("Y"))
	       		 {
       	 ...
       	 */
       	    ...
       	    File file = null ;
		          ...
		          String attachMode="attachment";
		              ...
		              if(newAttachFlg.equalsIgnoreCase("Y"))
		              {
