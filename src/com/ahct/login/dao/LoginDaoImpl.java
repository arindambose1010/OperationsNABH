...
public class LoginDaoImpl implements LoginDao{
	...
	public List<MenuVo> getActionBarModulesForEMP(String pStrParentModId,String pStrUsrID,String pStrLangId)
	{
		...
		args[0] = pStrUsrID;
		args[1]="('MD1210')";
		args[2] = pStrParentModId;
		args[3]=pStrLangId;
		args[4]="Y";
		args[5]="OperationsNABH";
		...
		sql_query.append(" and SUG.id.usergrpId=? and AM.modId=AMR.id.modId and AMR.id.modId not in (?) ");
	...
	public List<EmployeeDetailsVO> checkCasesForClinicalNotes(String userId)
    {
    	...
    	query.append(" ,EHFM_MAIN_THERAPY_NABH emt,Ehf_Case_Therapy ect ");
  		...
  		query.append(" and cc.scheme_id = 'CD202' and cc.case_Id=ect.case_Id and ect.icd_Proc_Code=emt.icd_Proc_Code");
  		query.append(" and cc.scheme_Id=emt.state and emt.process not in (?) ");
	...
	public String getHospitslId(String userId, String grpId) {
		  String hospId=null;
			...
			if(ClaimsConstants.DATAOP.equalsIgnoreCase(grpId)){
				critList.add(new GenericDAOQueryCriteria("id.userId",userId,GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				critList.add(new GenericDAOQueryCriteria("id.activeyn","Y",GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				critList.add(new GenericDAOQueryCriteria("id.grpId",ClaimsConstants.DATAOP,GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				List<EhfmDispUsrMpg> dataOprt=genericDao.findAllByCriteria(EhfmDispUsrMpg.class,critList );
				if(dataOprt!=null && dataOprt.size()>0){
					
					hospId=	dataOprt.get(0).getId().getDispId();
				}
				
			}
			
			if(ClaimsConstants.REFDOC.equalsIgnoreCase(grpId)){
				critList.add(new GenericDAOQueryCriteria("id.userId",userId,GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				critList.add(new GenericDAOQueryCriteria("id.activeyn","Y",GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				critList.add(new GenericDAOQueryCriteria("id.grpId",ClaimsConstants.REFDOC,GenericDAOQueryCriteria.CriteriaOperator.EQUALS));
				List<EhfmDispUsrMpg> refDoc=genericDao.findAllByCriteria(EhfmDispUsrMpg.class,critList );
				if(refDoc!=null && refDoc.size()>0){
					
					hospId=	refDoc.get(0).getId().getDispId();
				}
