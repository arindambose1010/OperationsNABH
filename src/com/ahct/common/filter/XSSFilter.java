...
public class XSSFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
       ...
       httpResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate"); // HTTP 1.1.
        ...
        httpResponse.setHeader("Content-Security-Policy", "default-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; font-src 'self' data:; frame-ancestors 'self' https://tdmobile.bhishak.net/api/Reports/GetReport https://tdiagnostics.telangana.gov.in/ data:;img-src 'self' data:;object-src 'self' data:; connect-src 'self' http: https:;child-src 'self' https://www.google.com/maps/embed https://tdmobile.bhishak.net/api/Reports/GetReport  https://tdiagnostics.telangana.gov.in/ data:;");
