...
<script SRC="Preauth/maximizeScreen.js"></script> 
...
function fn_viewAttachments(){
	...
	var url="ahcClaimsAction.do?actionFlag=viewAhcAttachments&ahcId="+ahcId+"&winOpen=N&headerShow=N";
...
function fn_claim()
{
	var url="ahcClaimsAction.do?actionFlag=claimsPage&ahcId=${ahcId}";	 
 ...
 var errMsg='';
	...
	if(roleId=='GP2')
		...
		if (!errMsg == '') {
			...
			url = 'ahcClaimsAction.do?actionFlag=checkMandatoryAttch&ahcId='+ahcId+'&attachType=ahcTestAttach&callType=Ajax';
...
function claimSubmitMedco(buttonId,displayMsg)
{
	  ...
	  var url ="ahcClaimsAction.do?actionFlag=saveClaimDtls&actionDone="+buttonId;
		  document.forms[0].action=url;		    
		  document.forms[0].submit(); 
