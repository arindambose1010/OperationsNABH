...
function fn_loadcategories(type){
	...
	var typeOfSpeciality=document.getElementById(type).value;
	 ...
	 var xmlhttp;
	  ...
	  var url='medicalAudit.do?actionFlag=getCategory&specialityType='+typeOfSpeciality;
...
function fn_loadProcedures(type){
	...
	var typeOfCategory=document.getElementById(type).value;
	 var xmlhttp;
	  ...
	  var url='medicalAudit.do?actionFlag=getProcedure&categoryType='+typeOfCategory;
	 xmlhttp.open("Post",url,true);
	 	xmlhttp.send(null);
		...
		var hospType=document.getElementById('hospType').value;
		 ...
		 if(distName!=-1 && distName !='' && hospType!=-1 && hospType!=''){
			 ...
			 var xmlhttp;
			  ...
			  var url='medicalAudit.do?actionFlag=gethospitals&districtCode='+distName+'&hospType='+hospType;
			 xmlhttp.open("Post",url,true);
			 	xmlhttp.send(null);
 ...
 document.forms[0].procedure.options.length=0;
	...
	document.getElementById('auditedBy').value="";
 	 ...
 	 <%-- if(flag == "worklist")
	 {
 	 document.forms[0].action = 'medicalAudit.do?actionFlag=getMAworklist&backFlag=no';
	 }
 	if(flag == "auditedList"){
 		document.forms[0].action = 'medicalAudit.do?actionFlag=getAuditedCaseslist&backFlag=no';
 	}
 	 document.forms[0].submit(); --%>
 }
 ...
 function showCase(caseId){
		...
		document.forms[0].action ='casesApprovalAction.do?actionFlag=getCaseDtls&CaseId='+caseId+'&caseSearchType=caseIp&casesForApproval=N&ipOpType=IP&errSearchType=N&disSearchType=N&module=medicalAuditworklist&redirectedFrom='+fromPage;
		document.forms[0].submit();
	}
</script>
