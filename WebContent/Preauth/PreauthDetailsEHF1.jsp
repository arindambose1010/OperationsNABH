...
<script type="text/javascript" src="js/PreauthScripts.js"></script> 
<script type="text/javascript" src="js/JqueryScrollbar.js"></script> 
<script src="js/jquery.msgBox.js" type="text/javascript"></script>
<script src="bootstrap/js/bootbox.min.js"></script>
<link rel="stylesheet" type="text/css" href="bootstrap/css/bootstrap.min.css"> 
    <link href="bootstrap/bootstrap-fileupload.css" rel="stylesheet">
...
var enhAmo = "${preauthDetailsForm.enhAmt}";
var ipFlag = "${ipFlag}";
var nimsHospId = "${nimsHospId}";
var testAmt = null;
var testSurg = null;
var count=0;
var xxx = null;
...
$(document).ready(function()
	    {
	        $("#ipOtherRemarks").keypress(function(event) {
	            if (event.keyCode == 13) {
...
function fn_addAttch(input,arg,arg1)
{
	...
	document.forms[0].action="preauthDetails.do?actionFlag=updateAttach&caseId="+caseId+"&input="+input+"&arg1="+arg1;
	document.forms[0].submit();
		}
	else
		{
		alert("Please select attachment to proceed");
		}
	}
function capturePatBiometrics()
{
 ...
 var fingerPrint="CVERFDGVNERJIOFNDIOHNERIOJGIODFNBUIONGIFMIOFGBETGEGRTGHDBTFG";
	 document.forms[0].fingerPrint.value=fingerPrint;  
		document.forms[0].action="preauthDetails.do?actionFlag=enrollPatDtls&cardNo="+cardNo+"&fingerPrint="+fingerPrint;
		document.forms[0].submit();
}
function uploadDocuments()
{
	var cardNo = document.forms[0].cardNo.value;
	 var caseId = '<bean:write name="preauthDetailsForm" property="caseId" />';
	if( document.getElementById('patientInTime') != null &&   document.getElementById("patientInTime").value <1)
	{
		alert("Please upload patient photo");
		return false;
	}
	if( document.getElementById('patientOutTime') != null &&   document.getElementById("patientOutTime").value <1)
	{
		alert("Please upload consent documents");
		return false;
	}
	document.forms[0].action="preauthDetails.do?actionFlag=uploadDialyDocs&cardNo="+cardNo+"&caseId="+caseId;
	document.forms[0].submit();
}
...
function fn_savePreauthdtls()
{
	...
	else
	{
	 ...
	 url = 'preauthDetails.do?actionFlag=checkMandatoryAttch&caseId='+caseId+'&attachType=ehfPreauthNabh&drugbrkFlag=Y&callType=Ajax&ipFlag='+ipFlag;
		    ...
		    {	
		    	 var resultArray=xmlhttp.responseText;
			        ...
			        if(resultArray[0]!=null)
			        {
			        	...
			        	if(document.getElementById("drugAmount")!=null){
			           
			        		if(resultArray[0].trim() =='success'||document.getElementById("drugAmount").value==0)
			        	   {
			        	   ...
			        	   if(document.getElementById("treatmntDt").value==null || document.getElementById("treatmntDt").value=='')
			        		...
			        		if(document.getElementById("ipNum").value==null || document.getElementById("ipNum").value=='')
			        		{
			        		...
			        		var jq = jQuery.noConflict();
			        		if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' ){
			        			var x = jq("#EnhancementDiv").find("table").length;
			        			if(x>0){
			        			   str = "Please click on Save before Submitting.";
			        			   alert(str);
			        			   return false;
			        			}
			
			        		}
			        		var existingInputs = jq("#medConsumablesList").find('tr').length;
			        		if(existingInputs>0){
			        			var str='';
		        			   for(var j=2;j<existingInputs;j++){
		        				if(jq("#medConsumablesList").find("tr:eq("+j+")").is(":visible") == true && jq("#medConsumablesList").find("tr:eq("+j+")").find("td:eq(6)").text()=='No Records'){
		        					str += "\n"+jq("#medConsumablesList").find("tr:eq("+j+")").find("td:eq(2)").text().trim();
		        				}
		        			   }
		        				if(str!=''){
		        				   str = "All attachments are mandatory. Please upload below mentioned attachments by readding them" + str;
		        				   alert(str);
		        					return false;
		        			   }
			        		 }
			        	 ...
			        	 if(document.getElementById("treatmntDt").value==null || document.getElementById("treatmntDt").value=='')
			        		...
			        		var jq = jQuery.noConflict();
			        		if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true'){
			        			var x = jq("#EnhancementDiv").find("[type='file']").length;
			        			if(x>0){
			        			   var str='';
			        			   for(var j=0;j<x;j++){
			        				var qid = jq("#EnhancementDiv").find("[type='file']:eq("+j+")").attr("id");
			        				if(document.getElementById(qid)!=null && document.getElementById(qid).value==""){
			        					qid = qid.split(".").join('');
			        					str += "\n"+jq("#Code"+qid).text().trim();
			        				}
			        			  }
			        			   if(str!=''){
			        				   str = "All attachments are mandatory. Please upload below mentioned attachments." + str;
			        				   alert(str);
			        					return false;
			        			   }
...
function fn_saveDrugPreauthdtls()
{
    var newDrAmt=parseFloat(document.getElementById("drugAmount").value);
    var oldrgAmt=parseFloat("${oldDrugAmt}");
    if('${consumableFlg}'=='Y'){
	       var oldTotEnhAmt='${preauthDetailsForm.oldTotalAmt}'
		   var latestEnhAmt=parseInt(document.forms[0].enhAmt.value);
		  
		       if(latestEnhAmt>oldTotEnhAmt){
		    	   alert('Enhancement Amount cannot exceed Previous Total Enhancement Amount');
		   		return false;
				}else{
		    		fn_testDrugAtachmmnt();	
		    	}
	   }
    else{
if(newDrAmt>oldrgAmt){
	
	 alert("Drug amount Should not be greater than previous drug amount");
	 return false;
}else{
	fn_testDrugAtachmmnt();	
}
...
function fn_testDrugAtachmmnt(){
	...
	else
	{
	 ...
	 url = 'preauthDetails.do?actionFlag=checkMandatoryAttch&caseId='+caseId+'&attachType=ehfPreauthNabh&callType=Ajax&ipFlag='+ipFlag;
	 xmlhttp.onreadystatechange=function()
		{
		    if(xmlhttp.readyState==4)
		    {	
		    	 var resultArray=xmlhttp.responseText;
			        ...
			        {	
			           if(resultArray[0].trim() =='success')
			        	   {
			        	       var jq = jQuery.noConflict();
			        		     if('${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' || ('${consumableFlg}'=='Y' && '${status}' == 'CD2589')){
			        			var x = jq("#EnhancementDiv").find("table").length;
			        			if(x>0){
			        			   str = "Please click on Save before Submitting.";
			        			   alert(str);
			        			   return false;
									...
									bootbox.confirm("Please check whether you have added all the medical expenses and attachments, Once submitted you will not be able to modify ", function(result) {
										if(result)
											{
												var caseId= '<bean:write name="preauthDetailsForm" property="caseId" />';
												document.forms[0].action="preauthDetails.do?actionFlag=saveDrugBreakupPreauthDtls&caseId="+caseId+"&ipFlag="+ipFlag;
												document.forms[0].submit();
											}
											});
...
{   var status='${status}';
	...
	bootbox.confirm("Do you want to submit reamrks ", function(result) {
		if(result)
			{
				var caseId= '<bean:write name="preauthDetailsForm" property="caseId" />';
				document.forms[0].action="preauthDetails.do?actionFlag=updateRemarks&caseId="+caseId+"&caseStatus="+status;
				document.forms[0].submit();
			}
			});
...
function fn_getICDProcedures()
{
	...
	else
	{
	 ...
	 url = 'preauthDetails.do?actionFlag=getProcList&IcdcatId='+document.forms[0].icdCatCode.value+'&procId='+document.forms[0].procCode.value+'&callType=Ajax';
	 xmlhttp.onreadystatechange=function()
		{
		    if(xmlhttp.readyState==4)
		    {	
...
if(catId=='S16' && pending=='Y')
{
	...
	function fn_hideTableEnhc(value,sno,amount,quantId)
	{
		  ...
		  if(confirm('Do you want to delete ? '))
		  {
			...
			if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' && '${consumableFlg}'!='Y'){

				fn_checkamountlocked(sno,amount);
				}
		  }
	}
	...
	{   
	    var drugAmount='${drugAmount}';
	    if(nimsHospId!=null && nimsHospId=='EHS34')
	    	{
	    	document.forms[0].enhAmt.value = enhAmo;
	    	}
	    else
	    	{
		if(document.forms[0].enhAmt != null)
		document.forms[0].enhAmt.value = enhAmt;
	if(document.forms[0].enhAmt !=null && document.forms[0].drugAmount !=null)
			{
document.forms[0].enhAmt.value = parseInt(enhAmt) + parseInt(document.forms[0].drugAmount.value);
			}
		else if(document.forms[0].enhAmt !=null && drugAmount !=null){
			document.forms[0].enhAmt.value = parseInt(enhAmt) + parseInt(drugAmount);
		}
	    	}
	}
	...
	{
        var drugAmt=document.forms[0].drugAmount.value;
	    var drugAmtOld = document.forms[0].drugAmountOld.value;
		if(enhAmt !='')
			{
			if(parseInt(enhAmt) != null && parseInt(enhAmt) !='' )
			{   
				if(parseInt(drugAmtOld) != parseInt(drugAmt))
					enhAmt = parseInt(enhAmt)+parseInt(amount)+parseInt(drugAmt)-parseInt(drugAmtOld);	
				else
					enhAmt = parseInt(enhAmt)+parseInt(amount);
			}	
			else
			{
				enhAmt = parseInt(amount)+parseInt(drugAmt);	
			}
			}
		else
			{
	...
	document.forms[0].enhAmt.value = enhAmt;
		document.forms[0].drugAmountOld.value = drugAmt;
	}
	...
	function fn_addAttachments()
	{
		...
		parent.parent.parent.attachmentWin= window.open("attachmentAction.do?actionFlag=onload&UpdType=ehfPreauthNabh&caseId="+caseNo+"&caseAttachmentFlag=Y&openWin=Y&caseApprovalFlag=Y", 'window1',
			'toolbar=no,resizable=yes,scrollbars=yes,width=800, height=600, top=100,left=50');
	}
	function fn_addDrugAttachments()
	{
		var caseNo='<bean:write name="preauthDetailsForm" property="caseId" />';
		parent.parent.parent.attachmentWin= window.open("attachmentAction.do?actionFlag=onload&UpdType=ehfPreauthNabh&caseId="+caseNo+"&caseAttachmentFlag=Y&drugsBreakFlag=Y&openWin=Y&caseApprovalFlag=Y", 'window1',
			'toolbar=no,resizable=yes,scrollbars=yes,width=800, height=600, top=100,left=50');
	}
	/* function fn_addDrugAttachmentsNew()
	{
		var caseNo='<bean:write name="preauthDetailsForm" property="caseId" />';
		parent.parent.parent.attachmentWin= window.open("attachmentAction.do?actionFlag=onload&UpdType=ehfPreauthNabh&caseId="+caseNo+"&caseAttachmentFlag=Y&drugsBreakFlag=Y&openWin=Y&caseApprovalFlag=Y", 'window1',
			'toolbar=no,resizable=yes,scrollbars=yes,width=800, height=600, top=100,left=50');
			    	 ...
			    	 var resultArray=xmlhttp.responseText;
				        ...
				        {	
				           if(resultArray[0].trim() =='success')
				       		...
				       		var criticalFlag="${preauthDetailsForm.critical}";
							var totPackAmt=document.forms[0].preauthTotalPackageAmt.value;
							if(('${viewType}' != null && '${viewType}' =='medco') && ('${status}' != null && '${status}' =='CD2' && '${ipFlag}' != null && '${ipFlag}' =='IPM') && (criticalFlag!=null && ( criticalFlag=='N' && totPackAmt>50000)||(criticalFlag=='Y'&&totPackAmt>100000))){
								alert("As per the trust guidelines, if the estimated amount is more than Rs.50,000/- under Non Critical and Rs.1,00,000/- under Critical category, the case goes to the Aarogyasri Health Care Trust for Preauth approval.");
							}
	...
	{   
		...
		if(document.getElementById('drugAmount')!=null)
		document.getElementById('drugAmount').disabled=true;
		...
		if('${ipFlag}' != null && ('${ipFlag}' =='IPM' || '${ipFlag}' =='IPD'))
		{
			...
			jqL(function() {
				jqL( "#treatmntDt,#deathDt,#dischrgeDt" ).datepicker({
					minDate: ipDate,
					changeMonth: true,
					changeYear: true,
					showOn: "both", 
	...
	if(criticalFlag!=null&&criticalFlag!=''){
			//document.forms[0].comorBidAppvAmt.innerHTML='0';
			jqL('#comorBidAppvAmt').val(parseInt(0));
			jqL('#comorBidAmt').val(parseInt(0));
			} 
		...
		if('${fn:length(preauthDetailsForm.dailysisList)}' >0)
		{
		document.getElementById("dailysisTable").style.display="";
		}
		...
		if('${viewType}' =='saveRemarks'&&'${ipFlag}' != null && ('${ipFlag}' =='IP' ||'${ipFlag}' =='IPD')){
			if(document.getElementById('CD71')!=null)
			document.getElementById('CD71').style.display="none";
		}
		if('${viewType}' =='medco11')
		{
		jqL(function() {
			jqL( "#admissionDate" ).datepicker({
				 changeMonth: true, 
		         changeYear: true, 
		   		 showOn: "both", 
		         buttonImage: "images/calend.gif",
		...
		if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' && '${consumableFlg}'!='Y')
		{    
			 var nextAmountLocked= '<bean:write name="preauthDetailsForm" property="nextAmountLocked" />';
			 var drugAmt=document.forms[0].drugAmount.value;
			 var totEnhAmt=parseInt(enhAmt)+parseInt(drugAmt);
		      	if(totEnhAmt>=nextAmountLocked && nextAmountLocked!=0){
		      		document.getElementById('lockNote').style.display="";
		      		document.getElementById('nextLock').style.display="none";
		      		 jqL("#submitPreauthMedical").prop('disabled', true);
			       jqL("[name='addHospStay']").prop('disabled', true);
			       jqL("[name='addimplantCode']").prop('disabled', true);
			       jqL("#drugImplant").prop('disabled', true);
			       jqL("[name='lucneSearch']").prop('disabled', true);

		      	}
		      	else{
		      		document.getElementById('lockNote').style.display="none";
		      		document.getElementById('nextLock').style.display="";
		      		 jqL("#submitPreauthMedical").prop('disabled', false);
		      		jqL("[name='addHospStay']").prop('disabled', false);
		    		jqL("[name='addimplantCode']").prop('disabled', false);
		 	       jqL("[name='drugImplant']").prop('disabled', false);
			       jqL("[name='lucneSearch']").prop('disabled', false);

		      	}
		}
		if('${preauthDetailsForm.specialityCode}'!=null&&'${preauthDetailsForm.specialityCode}'!=''&&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true'){
			if(document.getElementById('medicalSpclty')!=null){
			document.getElementById('medicalSpclty').value='${preauthDetailsForm.specialityCode}';
			//alert('${preauthDetailsForm.specialityCode}');
			 jqL("#medicalSpclty").prop('disabled', true);
			 getMedicalCategoryList2('${preauthDetailsForm.specialityCode}');
			}
		}
		...
		if('${ipFlag}' != null && '${ipFlag}' =='IPM')
	{
		...
		if( '${status}' != null && ( '${status}' !='CD651' && '${status}' !='CD8' && '${status}' !='CD207') &&  '${ipFlag}' != null && '${ipFlag}' =='IPM')
		{
			if(document.getElementById('remarksTr') !=null)
		...
		if('${ipFlag}' != null && '${ipFlag}' =='IPM' && '${viewType}' =='others'&& ('${status}' =='CD3241'||'${status}' =='CD3243')&& document.forms[0].enhAmt.value>1000000){
			document.getElementById('CD333').style.display="none";
			document.getElementById('CD73').style.display="";
		}else{
			document.getElementById('CD333').style.display="";
	...
	function fn_addHospStay(codeId,quanId)
	{
		...
		var jqL = jQuery.noConflict();
		jqL("[name='addHospStay']").each(function(){jqL(this).hide();});
		jqL("[name='addimplantCode']").each(function(){jqL(this).hide();});
		coverScreen();
    ...
    if (window.XMLHttpRequest)
		...
		var z = ele.split(".").join('');
		
		hospData=hospData+'<table border="0"  width="100%"  cellpadding="1" cellspacing="1" align="center"  id="'+tableId+'" name="'+tableId+'" ><tr align="center"><td width="11%" class="tbcellBorder" ><b>'+divId+'</b></td><td width="16%" class="tbcellBorder"><b>'+Code+'</b></td><td width="16%" class="tbcellBorder" id="Code'+z+'"><b>'+Quantity+'</b></td><td width="12%" class="tbcellBorder"><b>'+units+'</b></td><td width="10%" class="tbcellBorder"><b>'+amount+'</b></td><td class="tbcellBorder" width="10%"><input type="file"  id="'+ele+'" name="medclExpnsAttch['+medclExpnsId+']" onchange="checkBrowser(this)"/></td><td width="5%" class="tbcellBorder"><a href="javascript:fn_deleteHospStay('+enhancementCnt+','+amount+')" ><b>Delete</b></a></td></tr></table>';
		...
		if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' && '${consumableFlg}'!='Y'){
		fn_checkamountlocked(enhancementCnt-1,amount);
		}
		...
		document.getElementById(codeId+"Unit").value ='';
	     var temp="#"+codeId;
	     jqL(temp).select2().val='-1';
	     var temp1="#"+quanId;
	     jqL(temp1).select2().val='-1';
		...
		jqL("[name='addHospStay']").each(function(){jqL(this).show();});
		jqL("[name='addimplantCode']").each(function(){jqL(this).show();});
		removeScreenCover();
	...
	function fn_deleteHospStay(tableId,amount)
	{
		 var fr = partial(fn_deleteHospStayOnCnfm,tableId,amount);
		  if(confirm('Do you want to delete ? '))	
		  {
			...
			fn_checkamountlocked(tableId,amount);
			}
		  }
	}
	function fn_deleteHospStayOnCnfm(tableId,amount)
	{
		...
		//	var divValue = hospCodeArray[tableId][0];
		 ...
		 {
				if(document.getElementById('enhancemetTitlesId')!=null)
				document.getElementById('enhancemetTitlesId').style.display="none";
				if(document.getElementById('enhancemetTitlesId1')!=null)
	...
	function fn_checkamountlocked(id, amount){
		var jqL = jQuery.noConflict();
		var drugAmt=document.forms[0].drugAmount.value;
        var enhAmt=document.forms[0].enhAmt.value;
	      var nextAmountLocked='${preauthDetailsForm.nextAmountLocked}';
	      var maxAmountalloewd='${preauthDetailsForm.maxAmountAllowed}';
        var totEnhAmt=parseInt(enhAmt);
	      	if(totEnhAmt>=nextAmountLocked && nextAmountLocked!=0){
		       ...
		       jqL("[name='addHospStay']").prop('disabled', true);
		       jqL("[name='addimplantCode']").prop('disabled', true);
		       jqL("[name='drugImplant']").prop('disabled', true);
		       jqL("[name='lucneSearch']").prop('disabled', true);
	      	}
	      	else{
	      		document.getElementById('lockNote').style.display="none";
		...
		if(('${viewType}' != null && '${viewType}' =='medco' || '${viewType}' =='saveRemarks') || ('${viewType}' != null && '${viewType}' =='enhMedco'))
			{
				...
				if('${viewType}' =='saveRemarks' && buttonVal !='Save' )
					{
		   ...
		   if('${viewType}' =='coc' ||'${viewType}' =='ptd' || '${viewType}' =='ppd' || '${viewType}' =='ceo' || '${viewType}' =='eo' || '${viewType}' =='orgcomm')
		  {
		    ...
		    if(buttonVal != null && buttonVal=='Approve' || ('${viewType}' =='eo' && buttonVal=='Recommended Approve')|| ('${viewType}' =='ptd' && buttonVal=='Recommended Approve')
			  ...
			  // comorbid amount
			  	var criticalFlag="${preauthDetailsForm.critical}";
                if(criticalFlag==null||criticalFlag==''){
			   if(document.forms[0].comorBidAppvAmt.value == null || document.forms[0].comorBidAppvAmt.value=='' )
			  {
				   if(errMsg=='')
		...
		if(x>0){
		   ...
		   for(var j=0;j<x;j++){
			...
			if(document.getElementById(qid)!=null){
				 var elems = document.querySelectorAll("[id='"+qid+"']");
				 for(var al=0;al<elems.length;al++){
					   if(elems[al].value==null || elems[al].value==""){
					 	qid = qid.split(".").join('');
					 	if(str.indexOf("\n"+jq("#Code"+qid).text().trim())>=0){
...
function addCycle()
{
	var procedureCode= '${preauthDetailsForm.diaProcCode}';
	var cycles='${preauthDetailsForm.cycles}';
	
	var date1 = new Date();
	var date2 = '${preauthDetailsForm.patientInTime}';
	var dt= date2.split(/\-|\s/);
	date2 = new Date(dt.slice(0,3).reverse().join('-') + ' ' + dt[3]);
	var timeDiff = Math.abs(date2.getTime() - date1.getTime());
	var diffDays = timeDiff / (1000 * 3600 * 24); 
		if(diffDays <1)
			{
				bootbox.alert("Dialysis has been done for the day ");
				return false;
			}
		else if( ((procedureCode == 'N18.1' || procedureCode == 'N19.1') && (cycles <8))
			|| ((procedureCode == 'N18.2' || procedureCode == 'N19.2') && (cycles <12)) )
	{
		 var dailysisTable=document.getElementById("dailysisTr");    
			var newRow=dailysisTable.insertRow(-1);
		
			var newcell=newRow.insertCell(0);
			newcell.innerHTML='<td width="15%" class="tbcellBorder"><input type="file"   id="patientInTime"  name="patientPhoto[0]" onchange="checkBrowser(this)" /></td>';
			
			 newcell=newRow.insertCell(1);
			 newcell.innerHTML='<td width="15%" class="tbcellBorder"><input  type="file"   id="patientOutTime"  name="consentDoc[0]" onchange="checkBrowser(this)" /></td>';
			 
			medclDrugsId=medclDrugsId+1;
			medclDrugsIdSeq=medclDrugsIdSeq+1;
			medclExpnsId=medclExpnsId+1;
			document.getElementById("addCycleDia").style.display="none";
			document.getElementById("captureInTime").style.display=""; 
	}
	else
		{
			bootbox.alert("Maximum number of cycles reached for this procedure,Please raise new preauth");
			return false;
		}
}
...
else
{
 ...
 url = "preauthDetails.do?actionFlag=updateSentBackCases&caseId="+caseId+"&caseStatus="+caseStatus+"&remarks="+remarks+"&sendBackFlag="+sendBackFlag+"&moduleType=preauth&module="+module;
 xmlhttp.onreadystatechange=function()
	{
	    if(xmlhttp.readyState==4)
	    {	
...
{    var module=parent.parent.module;
	if(module=='hold')
	parent.parent.parent.fn_holdPendingCases(); 
	else
...
if (window.XMLHttpRequest)
	...
	var z = ele.split(".").join('');
	hospData=hospData.replace("</TBODY>","");
	hospData=hospData+'<table border="0"  width="100%"  cellpadding="1" cellspacing="1" align="center"  id="'+tableId+'" name="'+tableId+'" ><tr align="center"><td width="15%" class="tbcellBorder" ><b>'+divId+'</b></td><td width="16%" class="tbcellBorder"><b>'+Code+'</b></td><td width="16%" class="tbcellBorder" id="Code'+z+'""><b>'+Quantity+'</b></td><td width="6%" class="tbcellBorder"><b>'+units+'</b></td><td width="8%" class="tbcellBorder"><b>'+amount+'</b></td><td class="tbcellBorder" width="10%"><input type="file"  id="'+ele+'" name="medclExpnsAttch['+medclExpnsId+']" onchange="checkBrowser(this)"/></td><td width="5%" class="tbcellBorder"><a href="javascript:fn_deleteHospStayNew('+enhancementCnt+','+amount+')" ><b>Delete</b></a></td></tr></table>';
	hospStaytable.innerHTML=hospData;
	enhancementCnt++;
	// reset values
	fn_addEnhAmt(amount);
...
function fn_saveSurgicalImplants(){
 ...
 function holdCase(id)
 {
	 ...
	 if(id=="HD1")
		 ...
		 msg="Recommended to Discharge";
	 else
		 msg="Recommended Release";
 ...
 function fn_checkamountlockedDrug(amount,arg){
		var jqL = jQuery.noConflict();
		var drugAmt=document.forms[0].drugAmount.value;
	    var enhAmtt=0;
		if(('${status}' == 'CD651' || '${status}' == 'CD207'  || '${status}' == 'CD8') &&  '${ipFlag}' == 'IPM'  &&  '${isMedco}' == 'true' && '${consumableFlg}'!='Y')
	    ...
	    if(arg=='A'){
		    fn_addTotalBreakUp();
	    enhAmtt=parseInt(document.forms[0].enhAmt.value);

	    }
	    else
	    	{
	    enhAmtt=parseInt(document.forms[0].enhAmt.value)-parseInt(amount);
	    fn_addTotalBreakUp()
	    	}

	      var nextAmountLocked= '<bean:write name="preauthDetailsForm" property="nextAmountLocked" />';
	      var maxAmountalloewd= '<bean:write name="preauthDetailsForm" property="maxAmountAllowed" />';
	    var totEnhAmt=parseInt(enhAmtt);
	      	if(totEnhAmt>=nextAmountLocked && nextAmountLocked!=0 ){
	      		document.getElementById('lockNote').style.display="";
	      		document.getElementById('nextLock').style.display="none";
...
<c:forEach var="list" items="${groupList}" varStatus="loop" >
    ...
    <c:if test="${(ipFlag eq 'IPM' or ipFlag eq 'IPD') }">
    	...
    	<table width="100%"   id="addSurgdiv" >
    		...
    		<td  class="tbcellBorder" style="word-wrap:break-word;width:25%">${preauthDetailsForm.icdProcName1} </td>
    		
    		</tr>
    		<c:if test="${preauthDetailsForm.critical ne null && preauthDetailsForm.critical ne '' }">
    		<tr>
    		<td  style="word-wrap:break-word;width:25%" class="labelheading1 tbcellCss"><b>Procedure Name</b></td>
    		...
    		<c:if test="${preauthDetailsForm.critical eq 'Y'}">
    		<td  class="tbcellBorder" style="word-wrap:break-word;width:25%">Critical </td>
    		</c:if>
    		<c:if test="${preauthDetailsForm.critical eq 'N'}">
    		...
    		<td  style="word-wrap:break-word;width:25%" class="labelheading1 tbcellCss"><b>Reason for selecting any other procedure</b></td>
...
<br><br>
<!-- vijay -->
 <c:if test="${(ipFlag eq 'IPM') && (viewType eq 'medco')}"> 
...
<tr>
<td width="25%"  class="labelheading1 tbcellCss"><b> Category Name </b> <font color="red" class="onlyAp">*</font></td>
<td width="25%"  class="labelheading1 tbcellCss"><b> Category Code </b> <font color="red" class="onlyAp">*</font></td>
<td width="25%"  class="labelheading1 tbcellCss"><b>ICD Category</b> <font color="red" class="onlyAp">*</font></td>
<td width="25%"  class="labelheading1 tbcellCss"><b>ICD Category Code</b> <font color="red" class="onlyAp">*</font></td>
</tr>
<tr>
<td class="tbcellBorder">
<html:select name="preauthDetailsForm" property="medicalSpclty" styleId="medicalSpclty" style="width:17em;" title="Select Speciality" onchange="getMedicalCategoryList2(this.id)"  onmouseover="addTooltip('medicalSpclty')">
		<html:option value="-1">--------------------Select-----------------</html:option>
		 <html:options property="ID" collection="medicalSpltyList" labelProperty="VALUE"/> 
</html:select>
</td>
<td class="tbcellBorder"><html:text name="preauthDetailsForm"  property="asriCatCode1" styleId="asriCatCode1" title="Category Code" maxlength="10" style="WIDTH:17em" onkeydown="validateBackSpace(event)" readonly="true"/></td>
<td class="tbcellBorder"><html:select name="preauthDetailsForm" property="medicalCat" styleId="medicalCat" style="WIDTH: 17em" title="Select ICD Category " onchange="javascript:getTherapyICDProcedureListMed1(1);" onmouseover="addTooltip('medicalCat')">
<html:option value="-1">-------------------Select----------------</html:option>
 <logic:notEmpty name="ICDcategoryList">
<html:options property="ID" collection="ICDcategoryList" labelProperty="VALUE"/>
</logic:notEmpty> 
</html:select></td>
<td class="tbcellBorder"><html:text name="preauthDetailsForm"  property="ICDCatCode1" styleId="ICDCatCode1" title="ICD Category Code" maxlength="10" style="WIDTH:17em" onkeydown="validateBackSpace(event)" readonly="true"/></td>
</tr>
<tr>
<td  class="labelheading1 tbcellCss"><b>ICD Procedure</b> <font color="red" class="onlyAp">*</font></td>
<%-- <td><b><fmt:message key="EHF.ProcCode"/></b> <font color="red">*</font></td> --%>
<td  class="labelheading1 tbcellCss"><b>ICD Procedure Code</b> <font color="red" class="onlyAp">*</font></td>
<td  class="labelheading1 tbcellCss" id="otherId" style="display:none"><b>Reason for selecting any Other procedure</b><font color="red">*</font></td>

<td>&nbsp;</td>
</tr>
<tr>
<td class="tbcellBorder"><html:select name="preauthDetailsForm" property="ICDProcName1" styleId="ICDProcName1" style="WIDTH:17em" title="Select ICD Procedure" onchange="getProcedureCodesMed1();" onmouseover="addTooltip('ICDProcName1')">                        
            <html:option value="-1">----------------Select-----------------</html:option>
<logic:notEmpty name="ICDprocedureList">
<html:options property="ID" collection="ICDprocedureList" labelProperty="VALUE"/>
</logic:notEmpty>           
    </html:select></td>
<td class="tbcellBorder"><html:text name="preauthDetailsForm"  property="ICDProcCode1" styleId="ICDProcCode1" title="ICD Procedure Code" maxlength="10" style="WIDTH:17em" onkeydown="validateBackSpace(event)" readonly="true"/></td>
<td class="tbcellBorder" id="unitsTd" style="display:none">
<html:select name="preauthDetailsForm" property="procUnits" styleId="procUnits" style="WIDTH:17em" title="Select No of Units" onchange="javascript:getDentalConditions('','','','unitsCond')"onmouseover="addTooltip('procUnits')">
...
<td class="tbcellBorder" id="otherRemarks" style="display:none" >
<html:textarea name="preauthDetailsForm" property="ipOtherRemarks" styleId="ipOtherRemarks" title="Enter Medical Remarks" style="width:18em;;height:3em" onkeydown="check_maxLength('ipOtherRemarks',3000)" onchange="checkRemarks(this.value,3000,'ipOtherRemarks')"></html:textarea>
</td>
<td colspan="4"><button class="but"   type="button" name="addSurgicalDtls" id="addSurgicalDtls" value="Add Medical Details"  onclick="javascript:fn_addMedicalDtls('<bean:write name="preauthDetailsForm" property="caseId" />')"  >Change Therapy Details</button></td>
<%-- <td class="tbcellBorder"><html:select name="preauthDetailsForm" property="docSpecReg" styleId="docSpecReg" style="WIDTH:17em" title="Select Treating Doctor" onmouseover="addTooltip('docSpecReg')" onchange="fn_getIPDoctorsDetails()">                        
            <html:option value="-1"><fmt:message key="EHF.Select"/></html:option>     
</html:select></td> --%>
...
<html:text property="preauthTotalPackageAmt" name="preauthDetailsForm"  title="Total package amount"></html:text></td></tr>
</c:when>
<c:otherwise>
...
<c:if test="${preauthDetailsForm.critical eq null || preauthDetailsForm.critical eq '' }">
...
<button class="but" type="button" name="addAttachments" id="addAttachments" value="Add Attachments"  onclick="javascript:fn_addAttachments()" >View Attachments</button>
	...
	<tr><td>&nbsp;</td></tr>
	<tr><td>
	<c:if test="${ ipFlag eq 'IPM' && (viewType eq 'medco' || viewType eq 'saveRemarks') && status eq 'CD2' }">
	<b><center>Note: Please click on Save button to generate PRF Form </center></b>
	</c:if>
	</td></tr>
	<tr><td>&nbsp;</td></tr>
...
<div>
<table  cellpadding="1" cellspacing="1" align="center" width="100%" border="0" id="medConsumablesList">
...
<c:if test="${ipFlag eq 'IPM' && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && preauthDetailsForm.holdRejectFlag ne 'Y' || (consumableFlg eq 'Y' && status eq 'CD2589') }">
...
<c:if test="${(ipFlag eq 'IPM' && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && isMedco eq true ) && preauthDetailsForm.holdRejectFlag ne 'Y' || (consumableFlg eq 'Y' && status eq 'CD2589') }">
<table width="99%" align="center">
<tr  class="tbheader"  align="center"><td colspan="5">&nbsp;<b>Inputs</b></td></tr>
<tr   align="center"><td width="20%" class="labelheading1 tbcellCss">&nbsp;<b>Consumable Type</b></td>
<td width="25%" class="labelheading1 tbcellCss" >&nbsp;<b>Consumable Sub Type</b></td>
<td width="25%" class="labelheading1 tbcellCss">&nbsp;<b>Consumable Name</b></td>
<td width="15%" class="labelheading1 tbcellCss">&nbsp;<b>Units</b></td>
<td width="10%" class="labelheading1 tbcellCss"></td>
</tr>
<tr>
...
<!--   Sravan for Dialysis Start -->
<c:if test="${dialysisProc eq 'Y' && status ne 'CD2' }">
<table  width="100%"   border="0" id="dailysisTr">
<tr  class="tbheader"  align="center"><td colspan="6">&nbsp;<b>Dialysis Documents</b></td></tr>

<c:if test="${ status eq 'CD651'}">
		<tr>
				<td  class="labelheading1 tbcellCss"><b>Patient Photo taken during each cycle with timestamp</b></td>
				<td  class="labelheading1 tbcellCss"><b>Dialysis Chart</b></td>
				<td align="center" rowspan="2">
					<button class="but" id="addCycleDia"  type="button" name="addDrug" value="Add Cycle" onclick="addCycle();">Add Cycle</button>
					<button class="but" id="captureInTime"  type="button" name="capturetls"  onclick="uploadDocuments();" style="display: none;">Upload</button>
				
</c:if>
</table> 
</c:if>
<table  width="100%"   border="0"  id="dailysisTable" style="display: none;" >
							<tr  class="tbheader"  align="center"  >
								<td colspan="6">&nbsp;<b>Cycles Done</b></td>
							</tr>
							<tr>  
								<td width="15%" class="labelheading1 tbcellCss"><b>Cycle</b></td>
								<td width="15%" class="labelheading1 tbcellCss"><b>Patient Photo</b></td>
								<td width="15%" class="labelheading1 tbcellCss"><b>Consent Document</b></td>
								<td width="15%" class="labelheading1 tbcellCss"><b>Date</b></td>
							</tr>
			<logic:present name="preauthDetailsForm" property="dailysisList">
				<bean:size  id="dailysisSize" name="preauthDetailsForm" property="dailysisList"/>
				<logic:greaterThan value="0" name="dailysisSize">
					<logic:iterate id="rowCnt" name="preauthDetailsForm" property="dailysisList" indexId="cnt" >
						<tr>  
							<td width="13%" class="tbcellBorder"><bean:write name="rowCnt" property="COUNT" /></td>
						   <td width="13%" class="tbcellBorder">
						  <a href="javascript:fn_openAtachmentDialysis('<bean:write name="rowCnt" property="PATIENTPHOTOPATH" />');" >
						   <bean:write name="rowCnt" property="PATIENTPHOTONAME" /> </a></td>
							<td width="13%" class="tbcellBorder">
							  <a href="javascript:fn_openAtachmentDialysis('<bean:write name="rowCnt" property="PATIENTCONSENTPATH" />');" >
							<bean:write name="rowCnt" property="PATIENTCONSENTNAME" /></a></td>
							<td width="13%" class="tbcellBorder"><bean:write name="rowCnt" property="TESTTIME" /></td>
						</tr>
					</logic:iterate>
				</logic:greaterThan>
			</logic:present>            
...
<logic:present name="preauthDetailsForm" property="enhamcementList">
		<bean:size id="enhList" name="preauthDetailsForm" property="enhamcementList"/>
		<c:set var="enhListSize" value="${enhList}"/>
		<logic:greaterThan value="0" name="enhList">
		<c:if test="${ viewType eq 'enhMedco'  }" >
	...
	<c:if test="${ viewType ne 'enhMedco' && status eq 'CD3242' }" >
		<tr  class="tbheader"  align="center" id="enhancemetTitlesId" >
			<td colspan="8">&nbsp;<b>Inputs Requested List</b></td>
		</tr>
	</c:if>
	  	<c:if test="${ viewType ne 'enhMedco' && status ne 'CD3242' }" >
		<tr  class="tbheader"  align="center" id="enhancemetTitlesId" >
		...
		<!--<td width="5%" class="labelheading1 tbcellCss"><b>Sno</b></td>-->
		<td width="11%" class="labelheading1 tbcellCss"><b>Consumable Type</b></td>
		<td width="14%" class="labelheading1 tbcellCss"><b>Consumable Sub Type</b></td>
		<td width="14%" class="labelheading1 tbcellCss"><b>Consumable Name</b></td>
		<td width="10%" class="labelheading1 tbcellCss"><b>Units</b></td>
		 <td width="10%" class="labelheading1 tbcellCss"><b>Amount</b></td> 
		 <td width="12%" class="labelheading1 tbcellCss"><b>Date</b></td> 
		  <td width="12%" class="labelheading1 tbcellCss"><b>Attachments</b></td> 
		<c:if test="${ viewType eq 'enhMedco' && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && isMedco eq true || (consumableFlg eq 'Y'  && status eq 'CD2589') }" >
			<td width="5%" class="labelheading1 tbcellCss"></td>
		</c:if>
		<c:if test="${status eq 'CD3242' }" >
		<td width="9%" class="labelheading1 tbcellCss"><b>Edit</b></td>
		</c:if>
	</tr> 
			<logic:iterate id="enhResults" name="preauthDetailsForm" property="enhamcementList" indexId="index" >
				<tr id="EnhancementDataTable<%=index+1%>" style="display:true" align="center" >
					...
					<logic:notEmpty  name="enhResults" property="attachPath">
						<td class="tbcellBorder"><a href="javascript:fn_openAtachmentMedical('<bean:write name="enhResults" property="attachPath" />');" >View</a><br><logic:notEmpty  name="enhResults" property="attachPathNew"><a href="javascript:fn_openAtachmentMedical('<bean:write name="enhResults" property="attachPathNew" />');" >View</a></logic:notEmpty></td>
					</logic:notEmpty>
					<logic:empty  name="enhResults" property="attachPath">
					 <logic:notEmpty  name="enhResults" property="attachPathNew">
					<td class="tbcellBorder"><a href="javascript:fn_openAtachmentMedical('<bean:write name="enhResults" property="attachPathNew" />');" >View</a></td>
					</logic:notEmpty>
					<logic:empty  name="enhResults" property="attachPathNew"> 
					<td class="tbcellBorder">No Records</td>
				 	</logic:empty> 
					</logic:empty>
                    <%--  <td class="tbcellBorder"><bean:write name="enhResults" property="enhAttach" /></td>  --%>
					<c:if test="${ viewType eq 'enhMedco' && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && isMedco eq true || (consumableFlg eq 'Y'  && status eq 'CD2589' && enhResults.newConsumableFlg eq 'Y') }" >
						<td class="tbcellBorder"><a href="javascript:fn_hideTableEnhc('EnhancementDataTable<%=index+1%>','<bean:write name="enhResults" property="enhSno" />','<bean:write name="enhResults" property="enhAmt" />','<bean:write name="enhResults" property="enhQuantCode" />')" >Delete</a></td>
					</c:if>
         <c:if test="${status eq 'CD3242'}" >
  <logic:notEmpty  name="enhResults" property="attachPathNew">
         
		 <td class="tbcellBorder"><input title="upload the Enhancement Attachment" type="file"  id="editId[<%=index+1%>]" name="editId[<%=index+1%>]" disabled="disabled" onchange="javascript:fn_addAttch('<bean:write name="enhResults" property="enhSno" />',this,<%=index+1%>)" /></td>
         </logic:notEmpty>
         <logic:empty  name="enhResults" property="attachPathNew">
          
		 <td class="tbcellBorder"><input title="upload the Enhancement Attachment" type="file"  id="editId[<%=index+1%>]" name="editId[<%=index+1%>]" onchange="javascript:fn_addAttch('<bean:write name="enhResults" property="enhSno" />',this,<%=index+1%>)" /></td>
...
<table width="100%" border="0">
	...
	<tr>
		<td>
			...
			<c:if test="${ ipFlag eq 'IPM' && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207' or status eq 'CD2589') && isMedco eq true}" >
						<table width="99%" align="center">
							<tr  class="tbheader"  align="center"><td colspan="4">&nbsp;<b>Pharmacy Bills</b></td></tr>
								...
								<tr>
<td width="20%" align="center">Drugs Amount</td>
<td width="20%" align="center">
<html:text property="drugAmount" name="preauthDetailsForm" style="width:100px" styleId="drugAmount" onchange="javascript:fn_addEnhAmt()" readonly="readonly" title="Please add amount"></html:text>
</td>
<c:if test="${preauthDetailsForm.holdRejectFlag ne 'Y'}" >
<td width="20%" align="center">
<!-- <button class="but" id="addDrug"  type="button" name="addDrug" value="Add Drugs" onclick="addDrugs()">Add</button> -->

<button class="but" id="addDrug"  type="button" name="addDrug" value="Add Drugs" onclick="fn_getDrugQuant();" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">Add/View Drugs</button>
</td>
</c:if>
<td>
<button class="but" type="button" name="addAttachments" id="addDrugAttachments" value="Add Attachments"  onclick="javascript:fn_addDrugAttachments()" >Add / View Attachments</button>
</td>
</tr>
<tr>
<td colspan="5">
					...
					<td colspan="4">
						<table  width="100%"  id="drugsTable"  border="0" style="display:none">
							<tr  class="tbheader"  align="center" id="drugsTableId" >
								<td colspan="6">&nbsp;<b>Old Pharmacy List</b></td>
							</tr>
							<tr>  
								<td width="12%" class="labelheading1 tbcellCss"><b>Input type</b></td>
								<td width="12%" class="labelheading1 tbcellCss"><b>Code</b></td>
								<td width="12%" class="labelheading1 tbcellCss"><b>Amount</b></td>
								<td width="12%" class="labelheading1 tbcellCss"><b>Date</b></td>
								<td width="10%" class="labelheading1 tbcellCss"><b>Attachment</b></td>
								
								<c:if test="${ viewType eq 'enhMedco'  && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && isMedco eq true }" >
								<td width="5%" style="display:none" class="labelheading1 tbcellCss">&nbsp;</td>
								</c:if>
							</tr>
								...
								<logic:greaterThan value="0" name="drugSize">
									...
									<logic:iterate id="drugLst" name="preauthDetailsForm" property="drugLt" indexId="cnt" >
										...
										<td width="13%" class="tbcellBorder">Drugs and Disposables</td>   
											...
											<c:if test="${ viewType eq 'enhMedco'  && (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && isMedco eq true }" >
<td width="5%" class="tbcellBorder" style="display:none"><input class="but" type="button" style="display:none" value="Delete" name=<bean:write name='drugLst' property='SEQID' /> id=<bean:write name='drugLst' property='SEQID' /> onclick="javascript:confirmDeleteRow(this,'editDrug','<bean:write name='drugLst' property='SEQID' />','<bean:write name='drugLst' property='AMOUNT' />');"/></td>
											</c:if>
											<!-- <script>
											if(enhAmt == null || enhAmt=='')
...
<table width="100%" align="center" >
	<tr>
	<td align="center">
	<c:if test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207') && ipFlag eq 'IPM'  && isMedco eq true && preauthDetailsForm.holdRejectFlag ne 'Y' || (consumableFlg eq 'Y' && status eq 'CD2589') }">
<button class="btn btn-primary"  type="button" value="Save" id="saveMultiple"   onclick="javascript:fn_saveEnhanceDtls();">Save</button>
   ...
   <br>
   <table id="lockNote" style="display:none" width="100%">
<tr>
<td>
 		<c:if test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207')  && ipFlag eq 'IPM'  && isMedco eq true  && (consumableFlg ne 'Y' && status ne 'CD2589') && preauthDetailsForm.holdRejectFlag ne 'Y' }">
	    <logic:notEmpty name="preauthDetailsForm"  property="nextAmountLocked">
		<b style="color:red"><center>Note: The case is locked due to exceeding the ceiling amount, Please Click on Save Button for further Processing  </center></b>
	...
	<br>
	   <table id="nextLock"  width="100%">
<tr>
<td>
 		<c:if test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207')  && ipFlag eq 'IPM'  && isMedco eq true  && (consumableFlg ne 'Y' && status ne 'CD2589')  && preauthDetailsForm.holdRejectFlag ne 'Y' && preauthDetailsForm.nextAmountLocked ne 0  }">
	    <logic:notEmpty name="preauthDetailsForm"  property="nextAmountLocked">
		<b style="color:red"><center>Note: Case will be  locked at <b style="color:blue"><bean:write name="preauthDetailsForm" property="nextAmountLocked"/>/- </b> or above </center></b>
	...
	<br>
	<c:if test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207' or status eq 'CD4830' or status eq 'CD4831')  && ipFlag eq 'IPM'  && isMedco eq true    && preauthDetailsForm.holdRejectFlag eq 'Y'  }">
		<b style="color:red"><center>Note: Case has been Recommend discharge by trust.Furthur drugs and consummables cannot be added </center></b>
	    
	</c:if>
	<br>
<c:if test="${ viewType eq 'ctd' ||viewType eq 'ch' ||viewType eq 'enhMedco' || enhancementFlag eq 'Y'  || (consumableFlg eq 'Y'  && status eq 'CD2589' )}" >
<table width="100%" border="0">
<c:if test="${ viewType eq 'enhMedco'  }" >
<tr><td colspan="2" align="right">
<!-- <button class="but"   type="button" name="enhAmtgen" id="enhAmtgen" value="enhAmtgen" onclick="javascript:fn_enableEnhAmt()">Generate Amount</button> -->
...
<td width="50%" class="tbcellBorder"><html:text property="enhAmt" name="preauthDetailsForm" readonly="true" styleId="enhAmt" title="Total enhancement amount"></html:text></td>
</tr>
<c:if test="${viewType == 'ctd'  || viewType =='ch'  || viewType =='enhCeo'  || viewType =='enhEo' || viewType =='cpd' }" >
<tr><td width="50%" class="labelheading1 tbcellCss">Medical Expenses Approved Amount  Rs. <font color="red">*</font></td>
...
<c:if test="${viewType eq 'others' && ipFlag eq 'IPM' && nimsHospId ne 'EHS34'}" >
<table width="100%">
<tr>
<%-- <td><fmt:message key="EHF.Claim.DrugAmt"/>:&nbsp;<b><bean:write name="claimsFlowForm" property="totalDrugAmount" /></b></td> --%>
<td align="center"><button class="but" id="addDrug"  type="button" name="addDrug" value="Add Drugs" onclick="fn_getDrugQuantNew();" data-toggle="modal" data-target="#myModalNew">View Drugs</button>
</td>
<!-- <td>
<button class="but" type="button" name="addAttachments1" id="addDrugAttachments1" value="Add Attachments"  onclick="javascript:fn_addDrugAttachmentsNew()" > View Attachments</button></td> -->
</tr>
</table>
</c:if>
<tr>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Pharmacy Breakup Details </h4>
        </div>
		...
		<tr>
		<td colspan="3" id="drugDivNew" style="display:none">
		<table style="width:100%;height:30%" id="drugTableNew">
		<thead><tr>
		<th class="labelheading1 tbcellCss" style="width:40%">Drug Name </th>
		<th class="labelheading1 tbcellCss" style="width:20%">Unit Price(in Rs.) </th>
		<th class="labelheading1 tbcellCss" style="width:20%">Drug Quantity </th>
		<th class="labelheading1 tbcellCss" style="width:20%">Total Amount(in Rs.)</th>
<!-- 		<th class="labelheading1 tbcellCss">Action</th></tr>
 -->		</thead>
		<tbody></tbody>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal" onclick="javascript:fn_addTotalBreakUp()">Close</button>
        </div>
    </div>
  </div>
</div>
...
<c:if  test="${ ipFlag eq 'IPM' && viewType eq 'others'}">
	<table width="100%">
		<tr>
			<td width="50%" class="labelheading1 tbcellCss">Total Medical Expenses Amount  Rs.</td>
			<td width="50%"><html:text property="enhAmt" name="preauthDetailsForm" readonly="true" styleId="enhAmt" title="Total enhancement amount"></html:text></td>
		</tr>
		<c:if  test="${  sentBack ne  true}">
		<tr>
		...
		<tr>
			...
			<c:if test="${status eq 'HD1' && nimsHospId ne 'EHS34'}">
			<button type="button" name="holdUnholdBut" class="but" value="CD651" onclick="holdCase(this.value)">Recommended Release</button>
			</c:if>
			<c:if test="${status eq 'HD1' && nimsHospId eq 'EHS34'}">
			<button type="button" name="holdUnholdBut" class="but" value="CD333" onclick="holdCase(this.value)">Release</button>
			</c:if>
			<c:if test="${status eq 'CD324'  && nimsHospId ne 'EHS34'}">
			<button type="button" name="holdUnholdBut" class="but" value="HD1" onclick="holdCase(this.value)">Pending</button>
			<button type="button" name="holdUnholdBut" class="but" value="CD651" onclick="holdCase(this.value)">Recommended Release</button>
			</c:if>
			<c:if test="${status eq 'CD324' && nimsHospId eq 'EHS34'}">
			<button type="button" name="holdUnholdBut" class="but" value="HD1" onclick="holdCase(this.value)">Pending</button>
			<button type="button" name="holdUnholdBut" class="but" value="CD333" onclick="holdCase(this.value)">Release</button>
			</c:if>
			<c:if test="${ status eq 'CD3241' || status eq 'CD3243' }">
  		<button type="button" class="btn btn-primary btn-sm" name="holdUnholdBut" class="but" id='CD333' value="CD333" onclick="holdCase(this.value)">Release</button>
              	<button class="btn btn-round btn-danger btn-sm"  id="holdUnholdBut" value="CD30" onclick="javascript:holdCase(this.value);" type="button">Recommended Discharge</button>
            </c:if>
			<c:if test="${ status eq 'CD3241' || status eq 'CD3243' }">
			<button type="button" class="btn btn-primary btn-sm" name="holdUnholdBut" class="but" id='CD73' value="CD73" onclick="holdCase(this.value)">Recommended Release</button>
			</c:if>
			 <c:if test="${status eq 'CD3241' || status eq 'CD3243' }">
			<button type="button" class="btn btn-warning btn-sm" name="holdUnholdBut" class="but" value="CD31" onclick="holdCase(this.value)">Pending</button>
			</c:if>  
         </td>
		</tr>
	</table>
...
<div>
<c:if test="${(ipFlag eq 'IPM' or ipFlag eq 'IPD') && sentBack ne true }">
<table width="100%" id="disDates">
<tr>
<td colspan="8" align="left" class="tbheader"><b>Discharge/Death</b></td>
</tr>
<tr>
<c:choose>
<c:when  test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207' or status eq 'CD4830' or status eq 'CD4831') && (ipFlag eq 'IPM' or ipFlag eq 'IPD')  && isMedco eq true || (consumableFlg eq 'Y'  && status eq 'CD2589')  }">
...
<td class="labelheading1 tbcellCss"><strong>Death Date</strong></td>
<td class="tbcellBorder">&nbsp;
 <html:text  name="preauthDetailsForm" property="deathDt"  styleId="deathDt" title="Please select Death date" ></html:text> 
</td>
<td class="labelheading1 tbcellCss"><strong>IP No.</strong></td>
<td class="tbcellBorder">&nbsp;
 <html:text  name="preauthDetailsForm" property="ipNum"  styleId="ipNum" title="Please enter IP No" onchange="javascript:chkAlphaNumeric('ipNum','IP No');" ></html:text> 
</td>
</c:when>
<c:otherwise>
...
<logic:notEmpty name="preauthDetailsForm"  property="dischrgeDt">
<td class="labelheading1 tbcellCss"><strong>Discharge Date</strong></td>
<td class="tbcellBorder">&nbsp;
...
<logic:notEmpty name="preauthDetailsForm"  property="deathDt">
<td class="labelheading1 tbcellCss"><strong>Death Date</strong></td>
<td class="tbcellBorder">&nbsp;
<html:text  name="preauthDetailsForm" property="deathDt"  styleId="deathDt" title="Please select Death date" readonly="readonly"></html:text>
</td>
</logic:notEmpty>
<logic:notEmpty name="preauthDetailsForm"  property="ipNum">
<td class="labelheading1 tbcellCss"><strong>IP No</strong></td>
<td class="tbcellBorder">&nbsp;
<html:text  name="preauthDetailsForm" property="ipNum"  styleId="ipNum"  readonly="true"></html:text>
</td>
</logic:notEmpty>
</c:otherwise>
</c:choose>
...
<tr>
<td align="center">
<c:if test="${ (status eq 'CD651' or status eq 'CD8' or  status eq 'CD207'  or status eq 'CD4830' or status eq 'CD4831') && (ipFlag eq 'IPM' or ipFlag eq 'IPD')  && isMedco eq true  }">
...
<td align="center">
<c:if test="${ (status eq 'CD2589') && (ipFlag eq 'IPM' or ipFlag eq 'IPD')  && isMedco eq true || (consumableFlg eq 'Y'  && status eq 'CD2589') }">
<button class="btn btn-primary has-spinner"  type="button"  value="Save" id="submitPreauthMedical"   onclick="javascript:fn_saveDrugPreauthdtls();">Submit</button>
</c:if>
</td>
</tr>
</table>
...
<c:if test="${ipFlag eq 'IPM' && (status eq 'HD1' || status eq 'CD3242' || status eq 'CD3244' ) && isMedco eq true  }">
<table width="100%">
<tr>
<td>
	<b style="color:red;">Note : The case is locked due to exceeding the ceiling amount and pending with AHCT for approval</b>
</td>
</tr>
<tr ><td width="100%" class="labelheading1 tbcellCss">Remarks :&nbsp;<font color="red">*</font>&nbsp;&nbsp;&nbsp;&nbsp;
...
<div class="modal fade" id="myModal" role="dialog">
    ...
    <!-- Modal content-->
      <div class="modal-content" style="width:700px;">
        <div class="modal-header">
          <!-- <button type="button" class="close" data-dismiss="modal">&times;</button> -->
          <h4 class="modal-title" align="center">Pharmacy Breakup Details </h4>
        </div>
        <div class="modal-body"  style="height:400px; width='100%';" >
         <table align="center">  
<td align="center">
		 <label>Drug Search</label><br>
              <input type="text" name="postcodeSearch" value="" id="postcodeSearch" class="form-control" width="100px" title="please enter charcters with out space" placeholder="Minimum 3 Charcters Required" >
		      </td>
		 <td style="width:22%">
		         <br>
        <button type="button" float="left" class="btn btn-primary" name="lucneSearch" onclick="javascript:fn_searchDrugBreakUp()" style="width: 81.97916599999999px;padding-bottom: 10px;margin-bottom: 0px; margin-top: 5px;"><span class="glyphicon glyphicon-search"></span></button>	
           <c:if test="${ (status eq 'CD2589')}">
		 ...
		 <td>
		  <br>
		 <label class="btn btn-success btn-file" id="uploadExcel" style="width: 101.97916599999999px;padding-bottom: 9px;margin-bottom: 0px;margin-top: 5px;height: 29.979166px;padding-right: 18px;">Excel Upload <input type="file" id="attachment" name=attachment onchange="javascript:uploadDrugExcel()" styleClass="form-control"/></label>
		  <a href="javascript:fn_openAtachment1();" onclick="javascript:this.style.color = 'brown';">(Sample Template)</a>
		  </td>
        </c:if>
		 </tr>
		 <tr>
		 <td><br></td></tr>
		 <c:if test="${ (status eq 'CD2589')}">
         <tr><td colspan="3"><font color="red">NOTE: 1.Please Upload xls or XLS File(Use  97/2000/XP .xls file for best performance),Should be in the format of Sample Template.</font></td></tr>	
         <tr><td colspan="3"><font color="red">      2.QUANTITY column can not be null or negitive values or fractions(Only Numbers Are  Allowed)..</font></td></tr>	
         <tr><td colspan="3"><font color="red">      3.Quantity column shoud be in number format only.</font></td></tr>	
         <tr><td colspan="3"><font color="red">      4.Drugs should not exceed 1000 per Excel sheet.</font></td></tr>
         </c:if>
<tr>
		       <td style="width:43%">
		<label>Drug Name </label><br>
		<html:select styleClass="form-control"  title="Please select drug name" name="preauthDetailsForm" property="drugId"  styleId="drugId" style="width:100%" onchange="javascript:fn_getPrice(preauthDetailsForm.drugId)">
		<html:option value="-1">-------------select------------</html:option>
		</html:select>
		</td>
		     <td>
		<label>Drug Price </label><br>
		<html:text styleClass="form-control" name="preauthDetailsForm" property="drugPrice" styleId="drugPrice" ></html:text> 
	</td>
		     <td>
		<label>Drug Quantity </label><br>
		<html:text styleClass="form-control" name="preauthDetailsForm" title="Please select drug Quantity" property="drugQuant" styleId="drugQuant" onchange="javascript:fn_numberValidation(this.value,'drugQuant')" ></html:text> 
	</td>
		</tr><br>
		<tr>
		 <td align="center" colspan="3">
		<button class="but" type="button" name="drugImplant" id="drugImplant" value="Add" onclick="javascript:fn_addDrugBreakUp(document.getElementById('drugId').value,document.getElementById('drugQuant').value,document.getElementById('drugPrice').value)">Add</button>
		</td>
		</tr>
		<tr>
		<td colspan="3" id="drugDiv" style="display:none">
		<table style="width:100%;height:30%" id="drugTable">
		<thead><tr>
		<th class="labelheading1 tbcellCss" style="width:30%">Drug Name </th>
		<th class="labelheading1 tbcellCss" style="width:15%">Unit Price(in Rs.) </th>
		<th class="labelheading1 tbcellCss" style="width:15%">Drug Quantity </th>
		<th class="labelheading1 tbcellCss" style="width:15%">Total Amount(in Rs.)</th>
		<th class="labelheading1 tbcellCss" style="width:15%">Action</th></tr>
		</thead>
		<tbody></tbody>
	</table>
		</td>
...
<html:hidden property="drugAmountOld" value="0"/>
...
<html:hidden name="preauthDetailsForm" property="drugType" />
...
<html:hidden name="preauthDetailsForm" property="diaProcCode" />
...
<html:hidden name="preauthDetailsForm" property="fingerPrint" />
<html:hidden name="preauthDetailsForm" property="cardNo" />
...
bootbox.alert('${ResultMsg}',function (){
		...
		{	
			if(autoAction!=null && autoAction=="OnloadCaseOpen")
			{
				...
				else if(module=="preauthSurg")
				{
					parent.parent.fn_casesForApprovalNew();
				}
				else if(module=="preauthMed")
				{
					parent.parent.parent.fn_casesForApprovalMed();
				}
				else if(module=="preauthOrg")
				{
				parent.parent.fn_casesForApprovalOrgNabh();
				}
				else if(module=="hold")
				{
					parent.parent.parent.fn_holdPendingCases();
