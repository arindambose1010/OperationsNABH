...
function fn_getICDCatList()
{
	...
	else
	{
	 ...
	 url = 'preauthDetails.do?actionFlag=getIcdCatList&catId='+document.forms[0].category.value+'&callType=Ajax&hospId='+document.forms[0].hospitalId.value;
...
function fn_getProcedures()
{
	...
	var caseId = '<bean:write name="preauthDetailsForm" property="caseId" />';
	 url = 'preauthDetails.do?actionFlag=getProcList&IcdcatId='+document.forms[0].icdCatCode.value+'&asriCode='+document.forms[0].category.value+'&callType=Ajax&hospId='+document.forms[0].hospitalId.value+'&caseId='+caseId;
	 xmlhttp.onreadystatechange=function()
		{
		    if(xmlhttp.readyState==4)
		    {	
...
function fn_getSpecialInvestigations_test()
{
	...
	//alert(document.forms[0].category.value);
	 url = 'preauthDetails.do?actionFlag=getSpecialInvestigation&icdProcCode='+document.forms[0].icdProcCode.value+'&callType=Ajax&catCode='+document.forms[0].category.value;
	 xmlhttp.onreadystatechange=function()
		{
		 if(xmlhttp.readyState==4)
		    {
...
if(catId=='S16' && pending=='Y')
{
	...
	function fn_testMandatoryAttach(varType,buttonVal,buttonName,remarksCheck)
	{
		...
		else
		{
		 ...
		 url = 'preauthDetails.do?actionFlag=checkMandatoryAttch&caseId='+caseId+'&attachType=ehfPreauth&callType=Ajax';
		 xmlhttp.onreadystatechange=function()
			{
			    if(xmlhttp.readyState==4)
			    {	
		...
		if(parent.parent.parent.attachmentWin != null)
    	   ...
    	   if(varType != null && varType=='submit')
    		   ...
    		   for(var j=0; j<items.length; j++)
				...
				document.forms[0].action="preauthDetails.do?actionFlag=submitPreauthEhf&ceoFlag=Y&buttonVal="+buttonVal;
   			   document.forms[0].submit();	
    		   }
...
<script>
	...
	{	
		document.getElementById('buttonBlock').disabled=true;	
		disableDiv(document.getElementById('buttonBlock'));
    	document.forms[0].action="preauthDetails.do?actionFlag=submitNextLvl&ceoFlag=Y&viewType=${viewType}&buttonVal="+buttonId;
	   document.forms[0].submit();
	}
...
function testEnhButtonSubmitConfirm(buttonId)
{
	document.getElementById('enhButtonBlock').disabled=true;	
	   disableDiv(document.getElementById('enhButtonBlock'));
    	document.forms[0].action="preauthDetails.do?actionFlag=submitPreauthEhf&ceoFlag=Y&buttonVal="+buttonId+"&enhFlag=Y";
	   document.forms[0].submit();		
	}
...
<td class="tbcellBorder"><a href="javascript:fn_pexQuestions();" onclick=""><bean:write name="results1" property="auditRemarks" /></a></td>
</c:when>
<c:otherwise  >
