...
function saveAdminSancDetails(buttonValue){
	...
	if(status==true)
		{
		  fn_loadImage();
	      document.forms[0].action = 'adminSanction.do?actionFlag=saveAdminSancDetails&buttonValue='+buttonValue;
...
function onSuccessSave(buttonValue){
	 fn_loadImage();
		 var sanctionId=document.getElementById("sanctionId").value;		
		 document.forms[0].action = 'adminSanction.do?actionFlag=submitAdminSancDetails&sanctionId='+sanctionId+'&buttonValue='+buttonValue;
			document.forms[0].submit();
...
function viewAttachmnt(arg)
{
	...
	document.forms[0].action = 'adminSanction.do?actionFlag=viewSelectAttach&TransId='+TransIdNum+'&fileName='+arg.id;
 document.forms[0].submit(); 
	}
</script>
</head>
...
<img id="eraImg"  title="Click here to erase" src="images/erase37.jpg" alt="click to clear"  onclick="javascript:eraser('date');" >
</td>
...
<img id="eraImg"  title="Click here to erase" src="images/erase37.jpg" alt="click to clear" onclick="javascript:eraser('sanctionOrderDate');" >
</td>
...
<img id="eraImg"  title="Click here to erase" src="images/erase37.jpg" alt="click to clear" onclick="javascript:eraser('purchaseDate');" >
</td>					
...
<tr id="addRowTrId">
 ...
 <a href="viewAttachmnt(this)" id='${fileList.VALUE}'><span title="Click to view attachment" class="labelheading1" style="font-size: 1.25em;">${fileList.VALUE}</span></a>
 </td>
 </tr>
  </c:forEach>  
  </c:if> 
...
<logic:greaterThan name="size" value="0">
   <table class="tbheader"  style="width:90%;margin:0 auto;">
            <tr >
            <td align="left">
<img id="menuImage" src="images/updownArrow.jpg" title="Maximize/Minimize" style=cursor:hand; width="25" height="25" alt="Hide Menu" align="top" onclick="javascript:expandOrCollapse()" ></img>
</td>
...
function fn_getDetailedHeads()
{
	...
	url= 'adminSanction.do?actionFlag=getDetailedHeads&scheme='+scheme;
	if (window.XMLHttpRequest){
	 xmlhttp=new XMLHttpRequest();
	}
	else if (window.ActiveXObject){		
...
function getVendorsSchemewise(scheme)
{
	...
	if(scheme=='SC13')
 	...
 	url= 'adminSanction.do?actionFlag=getVendorsSchemewise&state='+state;
...
function fn_getHeadDetails()
	...
	url= 'adminSanction.do?actionFlag=getHeadDetails&scheme='+scheme+'&detailedHead='+detailedHead;
	if (window.XMLHttpRequest){
	 xmlhttp=new XMLHttpRequest();
	}
	else if (window.ActiveXObject){		
...
function fn_getIssuingAuthNames()
{
	...
	url= 'adminSanction.do?actionFlag=getIssuingAuthName&issuingAuthority='+issuingAuthority;
	if (window.XMLHttpRequest){
	 xmlhttp=new XMLHttpRequest();
	}
	else if (window.ActiveXObject){		
