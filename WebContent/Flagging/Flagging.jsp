...
<script src="bootstrap/js/bootstrap.min.js"></script>
	...
	function getSize(id)
	{
							...
							if(con==true)
								{
									...
									document.forms[0].action='flaggingAction.do?actionFlag=saveFlagDtls&flagged&flaggedCasesForApproval=${flaggedCasesForApproval}&caseId=${caseId}&newFlag=${newFlag}';
		...
		function getAttachments(caseId,flagId)
			{
					url="flaggingAction.do?actionFlag=getAttachments&caseId="+caseId+"&flagId="+flagId;
						...
						<b>	Action</b>
							...
							<%-- <c:if test="${(flag.flagNature ne 'Money Collection')||((flag.flagNature eq 'Money Collection') && ((userGroup eq 'GP45') && (flag.dateDif > 15)) || ((userGroup ne 'GP45') && (flag.dateDif < 15) && (flag.currentStatus ne 'JEO Forwarded'))))}"> --%>
								...
								<bean:write name="flag" property="middleName"/>	<bean:write name="flag" property="lastName"/> </td>
										...
										<td class="tbcellBorder" align="center">		
		<a href="javascript:getAttachments('<bean:write name="flag" property="caseId"/>','<bean:write name="flag" property="flagId"/>')">View</a>		 </td>
