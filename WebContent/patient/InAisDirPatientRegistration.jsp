...
function retrieveDetails()
{
	...
	if((document.getElementById("ais_no").value==null || document.getElementById("ais_no").value=='') && (document.getElementById("fname").value==null || document.getElementById("fname").value==''))
	{
	 ...
	 {
 		document.forms[0].action="./patientDetails.do?actionFlag=retrieveCardDetails&cardNo="+wapFamilyNo+"&fromDisp="+fromDisp+"&appntdate="+appntdate+"&aisFlag="+aisFlag+"&aisDG="+aisDG+"&unitId="+unitId+"&serviceTyp="+serviceTyp+"&service="+service+"&aisTy="+aisTy+"&ayush="+ayush+"&mobileAppAis=N";
...
if (disabled != null && disabled === 'N') {
    ...
    var disID = document.getElementById("disID");
    var disNameText = document.getElementById("disNameText");
    var disIDText = document.getElementById("disIDText");

    if (disName) disName.style.display = '';
    if (disID) disID.style.display = '';
    if (disNameText) disNameText.style.display = '';
    if (disIDText) disIDText.style.display = '';
}
...
function validate()
{
	...
	/* 	else if (document.getElementById("eMailId").value==null || document.getElementById("eMailId").value=='') 
		...
		var ayushField = document.forms[0].ayush;
		var selected = false;
		var selectedValue = "";

		// Check if ayush has multiple radios or single
		if (ayushField.length) {
		    for (var i = 0; i < ayushField.length; i++) {
		        if (ayushField[i].checked) {
		            selected = true;
		            selectedValue = ayushField[i].value;
		            break;
		        }
		    }
		} else {
		    if (ayushField.checked) {
		        selected = true;
		        selectedValue = ayushField.value;
		    }
		}
		...
		if (!selected) {
		    if (errMsg == '')
		        errMsg = "Please Select Health Check Up <br>";
		    if (lField == '')
		        lField = 'ayush';
		} else {
		    // Case 1: Master Health Checkup
		    if (selectedValue == 'M') {
		        if (document.getElementById("dosage").value == -1) {
		            if (errMsg == '')
		                errMsg = "Please Select Package Type<br>";
		            if (lField == '')
		                lField = 'dosage';
		        }
		    }
		    // Case 2: Ayush Services
		    else if (selectedValue == 'F') {
		        if (!document.forms[0].ayushSubg.checked) {
		            if (errMsg == '')
		                errMsg = "Please Select Sub Category <br>";
		            if (lField == '')
		                lField = 'ayushSubg';
		        } else if (document.forms[0].ayushSubg.value == 'G') {
		            ...
		            if (!okay) {
		                if (errMsg == '')
		                    errMsg = "Please Select any one checkbox for General Therapy. <br>";
		                if (lField == '')
		                    lField = 'ayushSubg';
		            }
		        }
...
function registerPatientDetails()
{
	...
	/* added by sowmya for ayus separation */
	var nimsAyush='${nimsAyush}';
	...
	{	
	   ...
	   document.forms[0].action="./patientDetails.do?actionFlag=registerPatientDetails&aisSubmit=Y&aisSubmitDir=Y&fromDisp=${fromDisp}&aisflagNe=Y&tgGovPatCond="+tgGovPatCond+"&wapNo="+wapNo+"&familyNo="+familyNo+"&mobileApp="+mobileApp+"&appntdate="+appntdate+"&ayushID="+ayushID+"&unitId="+unitId+"&serviceTyp="+serviceTyp+"&aisTy="+aisTy+"&nimsAyush="+nimsAyush;
	   document.forms[0].method="post";
	   document.forms[0].submit(); 
		}	
	else
...
function validateSevice()
{
    ...
    var inServiceRadio = document.getElementById("servType");  
    var retiredRadio = document.getElementById("servType1");   

 
    inServiceRadio.disabled = false;
    retiredRadio.disabled = false;
    inServiceRadio.checked = false;
    retiredRadio.checked = false;

    if (serType === "EX-MLA" || serType === "EX-MLC") {
       
        inServiceRadio.disabled = true;
        retiredRadio.checked = true;
    } 
    else if (serType === "MLA" || serType === "MLC") {
       
        retiredRadio.disabled = true;
        inServiceRadio.checked = true;
    } 
...
function loadDependentDetails(name, gender, relation,dependentMobileNo,cardNo,maritalStatus) {
    
    document.getElementById("fname1").value = name;
    document.getElementById("contactno").value = dependentMobileNo;
   
        var genderRadios = document.getElementsByName("gender");
        for (var i = 0; i < genderRadios.length; i++) {
            if ((gender === "Male" && genderRadios[i].value === "M") ||
                (gender === "Female" && genderRadios[i].value === "F")) {
                genderRadios[i].checked = true;
            }
        }

        
        var relSelect = document.getElementById("relation");
        for (var i = 0; i < relSelect.options.length; i++) {
            if (relSelect.options[i].text === relation) {
                relSelect.selectedIndex = i;
                break;
            }
        }
        
        var marSelect = document.getElementById("maritalStatus");
        for (var i = 0; i < marSelect.options.length; i++) {
            if (marSelect.options[i].text === maritalStatus) {
            	marSelect.selectedIndex = i;
                break;
            }
        }
        
     
        var dependentWrapper = document.getElementById("dependentTableWrapper");
        if (dependentWrapper) {
            dependentWrapper.innerHTML = ""; 
            dependentWrapper.style.display = "none"; 
        }
}
...
function validateMobile(input)
{
	...
	if(!(a.trim().charAt(0)=="5" || a.trim().charAt(0)=="6" || a.trim().charAt(0)=="7" || a.trim().charAt(0)=="8" || a.trim().charAt(0)=="9"))
		{
		jqueryErrorMsg('Contact Number Validation','Contact No must start with 5/6/7/8/9',fr);
		input.value="";
		return false;
		}
}
...
<tr>
	...
	<td>
		...
		<td class="labelheading1 tbcellCss" width="20%" id="disName"><b>Employee Name&nbsp;</b><font color="red">*</font></td>
	 	<td class="labelheading1 tbcellCss" width="20%" id="disID"><b>Employee ID&nbsp;</b><font color="red">*</font></td>
		...
		<html:select name="patientForm" property="serType" styleId="serType" title="Select Service Type" style="WIDTH: 12em" onmouseover="addTooltip('serType')"  onkeydown="validateBackSpace(event)" onchange="validateSevice()">
					...
					<html:option value="EX-MLA">EX-MLA</html:option>
					<html:option value="EX-MLC">EX-MLC</html:option>
					<html:option value="FSOD">Family/Spouse of Deceased (MLA/MLC)</html:option>
					</html:select></td>
		<td class="tbcellBorder">
		...
		<html:radio name="patientForm"  property="servType" value="RS"  onclick="javascript:showFields();callFunction('N');" styleId="servType1"  title="Retired"/><b><label>&nbsp;Retired&nbsp;</label></b> 
		</td>
...
<c:if test="${not empty lhsDependentList}">
  <div id="dependentTableWrapper">
    <table class="tbheader">
        <tr>
            <td><b>LHS Dependent Details</b></td>
        </tr>
    </table>
    
    <table class="dependentContentTable" style="width:100%">
        <tr>
            <td class="labelheading1 tbcellCss" width="14%"><b>Dependent Name</b></td>
            <td class="labelheading1 tbcellCss" width="14%"><b>Card No</b></td>
            <td class="labelheading1 tbcellCss" width="15%"><b>Gender</b></td>
            <td class="labelheading1 tbcellCss" width="14%"><b>Contact No</b></td>
            <td class="labelheading1 tbcellCss" width="14%"><b>Marital Status</b></td>
            <td class="labelheading1 tbcellCss" width="14%"><b>Relationship</b></td>
            <td class="labelheading1 tbcellCss" width="15%"><b>Action</b></td>
        </tr>

       <c:forEach var="dependent" items="${lhsDependentList}" varStatus="status">
	    <tr>
	        <td class="tbcellBorder">${dependent.dependentName}</td>
	        <td class="tbcellBorder">${dependent.cardNo}</td>
	        <td class="tbcellBorder">${dependent.gender}</td>
	        <td class="tbcellBorder">${dependent.dependentMobileNo}</td>
	        <td class="tbcellBorder">${dependent.maritalStatus}</td>
	        <td class="tbcellBorder">${dependent.relation}</td>
	        <td class="tbcellBorder">
             <input class="but" type="button" value="Select" 
             onclick="loadDependentDetails(
 ...
 <table class="tbheader">
	...
	<!-- added by sowmya cr:8193 -->
		<c:choose>
				<c:when test="${nimsAyush ne 'Y'}">
				  <html:option value="NI">NIMS HOSPITAL</html:option>
				</c:when> 
				<c:otherwise>
				  <html:option value="ASN">AYUSH SERVICES NIMS</html:option>
				</c:otherwise>
		</c:choose>
		...
		<!-- added by sowmya cr:8193 -->
			<c:choose>
				<c:when test="${nimsAyush ne 'Y'}">
				  <html:radio name="patientForm" property="ayush" value="M" title="Master" styleId="ayush" onclick="javascript:fn_enablePackage()" /><b>&nbsp;Master Health Checkup</b> 
		        </c:when>
		        <c:otherwise>
		          <html:radio name="patientForm" property="ayush" value="F" title="Ayush"  styleId="ayush1" onclick="javascript:fn_disablePackage()"/><b>&nbsp;Ayush Services</b>
		        </c:otherwise>
	        </c:choose>
        </td>
		<c:if test="${nimsAyush ne 'Y'}">
