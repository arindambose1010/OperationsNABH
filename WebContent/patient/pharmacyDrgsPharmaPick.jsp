...
function fn_submitDrugs()
{
   ...
   else
   {
 	...
 	url = "./patientDetailsNew.do?actionFlag=saveDTOdrugsNimsPhr&patientId="+patientId+"&drugs="+drugs+"&fromDisp=Y";  
            ...
            else
            {
				...
				if(result[0] != null && result[0] == 'Y'){
					...
					//	partialFlg='Y';
								...
								document.forms[0].action="./patientDetailsNew.do?actionFlag=viewPharmacyPageNimsNew&patientId="+patientId+"&fromDisp=Y";
...
function getQuant(dbtr,k)
{
  ...
  xmlhttp.onreadystatechange=function()
	...
	url = "patientDetailsNew.do?actionFlag=getQuantAjax&callType=Ajax&drgType="+drgType+"&drgName="+drgName+"&mftr="+mftr+"&dbtr="+dbtr;
	coverScreen();
	xmlhttp.open("Post",url,true);
	xmlhttp.send(null);
	}	
...
function confirmRemoveRowNims(id)
{
	var patientId= document.getElementById("patientNo").value; 
   ...
   else
   {
 	...
 	url = "./patientDetailsNew.do?actionFlag=saveDrugDetailsNims&patientId="+patientId+"&id="+id;    
    xmlhttp.onreadystatechange=function()
    {
       if(xmlhttp.readyState==4)
       {
    	   ...
    	   if(resultArray.trim()=="SessionExpired*"){
            ...
            else
            {
				...
				if(result[0] != null && result[0] == 'Y'){
						...
						alert("Drugs deleted successfully");
						partialFlg='Y';
...
function checkDrug(){
	...
	if(x!=null && x!='' && (x=='OD6153' || x=='OD6154' || x=='OD6155' || x=='OD6156' || x=='OD6157' || x=='OD6158' || x=='OD6159' || x=='OD6160' || x=='OD6161' || x=='OD6162' || x=='OD6163' || x=='OD6164' || x=='OD6165'))
		{
		document.getElementById("newDrugName").value="";
...
function checkDrug1(){
	var jq = jQuery.noConflict();
	var drugs="${values}";
	var existdrugcode=document.getElementById("drugSubTypeCode").value;
	var drugValues=drugs.split("~");
	 if(drugValues.length>0){
		for(var c=0;c<drugValues.length;c++)
			{
			
			if(drugValues[c]!=null){
			var drugCode=drugValues[c];
			if(drugCode==existdrugcode)
				{
				jqueryErrorMsg('Unique Drug Validation',"Drug Set already exists.Please select another Drug set. If you want to edit the drug details, please delete them and add again.");
				document.getElementById("drugSubTypeCode").value="-1";
				document.getElementById("drugTypeCode").value="-1";
...
function fn_backDrugs()
{
	...
	var url="./patientDetailsNew.do?actionFlag=ViewRegisteredPatientsNims&fromDispnsry=Y&vieFlag=N";
...
//	}
	if(drugCount>0)
		...
		document.getElementById("SubmitNew").style.display="";
		}
	document.getElementById("drugTypeCode").value="-1";
	document.getElementById("drugSubTypeCode").value="-1";
	document.getElementById("newDrugName").value="";
...
function fn_saveDrgs()
{
	   if(document.getElementById("pickupName").value==null || document.getElementById("pickupName").value=='')
	   {
	      var fr=partial(focusBox,document.getElementById("pickupName"));
	      alert("Please give Person name");
	       return false;
	   }
		
	   if(document.getElementById("pickupNum").value==null || document.getElementById("pickupNum").value=='')
	   {
	      var fr=partial(focusBox,document.getElementById("pickupNum"));
	...
	var pickupNum = document.getElementById("pickupNum").value;
 ...
 */	document.getElementById("Back").disabled = true;
	...
	var url="./patientDetailsNew.do?actionFlag=savePharmacydrugsNimsPick&fromDisp=Y&saveFlg="+saveFlg+"&pickupName="+pickupName+"&pickupNum="+pickupNum+"";
...
function validateQuant(quant,k)
{
	if( quant!="" && k!="" && parseInt(quant)!=null && k!=null )
		{
		if (isNaN(quant)) 
		  {
		    alert("Dispatch Quantity should be numbers");
		    document.getElementById("prQuant"+k).value = "0";
		    return false;
		  }
		...
		var avlQuant=document.getElementById("avlQuant"+k).value;
...
<tr><td class="labelheading1 tbcellCss" width="40%"><b>Pickup Location</b></td><td class="tbcellBorder" width="60%"><b>&nbsp;<bean:write name="patientForm" property="pickLocation" /></b></td></tr>


<%-- <tr><td class="labelheading1 tbcellCss" width="40%"><b>Attachments</b></td>
...
<%--  <c:if test="${ deoViewFlg eq 'N' }" >

<table  width="80%"  class="tbheader" style="margin:0 auto">
<tr><td align="CENTER"><b>DRUGS ADDED</b></td></tr>
</table>

<table width="80%" id="drugInfo" style="margin:0 auto">
<tr>
<th width="20%" class="labelheading2 tbcellCss"><b>SNO</b></th>
...
<logic:present name="patientForm" property="drugLt">
        ...
        <input type="hidden" id="dbtr${index+1}"/> 
       	...
       	<td width="5%"><input type="text" id="prQuant${index+1}" value='<bean:write name="drugLst" property="avlbQuantity" />'  onchange="validateQuant(this.value,${index+1});"> </td>
   
  <td width="5%"><input class="but" type="button" value="Delete" name=<bean:write name='drugLst' property='DRUGSUBTYPECODE' /> id=<bean:write name='drugLst' property='DRUGSUBTYPECODE' /> onclick="javascript:confirmRemoveRowNims('<bean:write name='drugLst' property='DRUGSUBTYPECODE' />');" /></td>
...
<tr><td><font ><b>PickUp Details:</b></font></td></tr>
<tr>
<td width="50%" class="labelheading1 tbcellCss existDrugs"><b>Person Name</b><font color="red" >*</font></td>
 <td width="50%" class="labelheading1 tbcellCss existDrugs"><b>Phone Number</b><font color="red" >*</font> </td> 
</tr>
<tr>
...
<html:text name="patientForm" property="pickupName" styleId="pickupName" maxlength="50"  style="WIDTH:17em"  title="Enter Medication Period"/>
</td>
<td class="tbcellBorder"> 
 		<html:text name="patientForm" property="pickupNum" styleId="pickupNum"  maxlength="10" style="WIDTH:17em" onchange="validateNumber('Quantity',this);"  title="Enter Quantity" /> 
</td> 
...
<tr>
<td  >
<button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Submit"  onclick="javascript:fn_saveDrgs()">Issue Drugs&nbsp;</button>
</td>
<td  >
<%-- <button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Save" onclick="javascript:fn_saveDrgs(<c:out value = "${drgCount}"/>,'Save')">Save Drugs&nbsp;</button>
 --%></td>
<td  >
