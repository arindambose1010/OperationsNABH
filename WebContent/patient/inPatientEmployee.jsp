...
function retrieveDetails()
{
	...
	var appntdate='${appntdate}';
	 ...
	 {
 		document.forms[0].action="./patientDetails.do?actionFlag=retrieveCardDetails&aisFl=Y&aisFlag=Y&cardNo="+wapFamilyNo+"&fromDisp="+fromDisp+"&mobileApp="+mobileApp+"&appntdate="+appntdate+"&aisFlag="+aisFlag;
...
function validate()
{
	...
	{
		if(errMsg=='')
			errMsg=errMsg+"Please Select card type <br>";
		if(lField=='')
	        lField='card_type'; 
	}
	else if(document.forms[0].card_type[0].checked)
...
function registerPatientDetails()
{
	...
	if(document.getElementById("cardNo").value==cardFamilyNo)
	   ...
	   var tgGovPatCond=document.getElementById("tgGovPatCond").value;
	   document.forms[0].action="./patientDetails.do?actionFlag=registerPatientDetails&aisflagNe=N&fromDisp=${fromDisp}&tgGovPatCond="+tgGovPatCond+"&wapNo="+wapNo+"&familyNo="+familyNo+"&mobileApp="+mobileApp+"&appntdate="+appntdate+"&ayushID="+ayushID+"&empDirect=Y";
	   document.forms[0].method="post";
	   document.forms[0].submit(); 
		}
	else
...
//     document.forms[0].card_type[3].disabled=val2;
 //   document.forms[0].card_type[3].disabled=val2;
    ...
    //Enable for New Born Condition
     /*if(document.forms[0].card_type[3]!=null && document.forms[0].card_type[0][3].checked==true)
    	{
...
// 		document.forms[0].card_type[3].checked=false;
		...
		for(var i=0;i<11;i++)
			...
			document.getElementById("ECNo"+i).readOnly="";
...
function resetPatientData()
{
	...
	document.getElementById("village").options.length = 1;
	document.getElementById("comm_village").options.length = 1;
	$('input:radio').not('[name="card_type"]').removeAttr('checked');
	$('input:checkbox').not('[name="head"]').removeAttr('checked');
...
$('document').ready(function(){
	...
	if('${patientForm.errMsg}'!=null && '${patientForm.errMsg}'!='')
	{
	alert('${patientForm.errMsg}');
	resetData();
	}
...
<body onload="javascript:fn_chkTGGovCond();fn_load()">
...
<tr>
	...
	<td>
		...
		<td class="tbcellBorder"  width="70%">
			<html:radio name="patientForm" property="card_type" value="E" styleId="card_type" onclick="enable_cardType1(this.checked)" title="Employee Card No"/><b><fmt:message key="EHF.EmployeeCardNo"/></b> 
			</td>
		...
		<td class="tbcellBorder"><html:text name="patientForm" property="eMailId" styleId="eMailId" maxlength="100" title="Enter E-mail ID"/></td>
		</tr>
		<!-- <tr>
		<td class="labelheading1 tbcellCss"><b>E-Mail ID</b> <font color="red">*</font></td>
		<td>&nbsp;</td> 
		<td>&nbsp;</td>
		</tr>
		<tr>
