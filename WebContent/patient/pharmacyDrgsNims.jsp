...
var labGrp = "${labGrp}";
var x = "${enablePharma}";
var partialFlg = "${partialFlg}";

var deoViewFlg = "${deoViewFlg}";
var roleIdNims = "${roleIdNims}";
var viewFl="${viewFl}";
    	   ...
    	   if(resultArray.trim()=="SessionExpired*"){
            ...
            else
            {
				...
				if(result[0] != null && result[0] == 'Y'){
						...
						alert("Drugs added by DEO deleted successfully");
						partialFlg='Y';
						
							
								document.forms[0].action="./patientDetailsNew.do?actionFlag=viewPharmacyPageNims&patientId="+patientId+"&fromDisp=Y";
...
function fn_rejectDrugs()
{
	var patientId= document.getElementById("patientNo").value; 
	 bootbox.confirm("Do you Want to Reject ? ", function(result) {
		 	if(result)
			{
					 document.forms[0].action="./patientDetailsNew.do?actionFlag=rejectPatient&vieFlag=N&patientId="+patientId;
				   	 document.forms[0].method="post";
				   	 document.forms[0].submit();
			 }
		});
...
function fn_submitDrugs()
{
	...
	if(((document.getElementById("drugsTable")==null) && (document.getElementById("drugsTable")=="")) ||  ((document.getElementById("drugInfo")==null) && (document.getElementById("drugInfo")=="")))
	{
	...
	else
	{
		var x=0;
		var y=0;
		if(document.getElementById("drugsTable")!=null && document.getElementById("drugsTable")!="")
		x = document.getElementById("drugsTable").rows.length;
		if(document.getElementById("drugInfo")!=null && document.getElementById("drugInfo")!="")
		 y=  document.getElementById("drugInfo").rows.length;
    	...
    	return false;
		 ...
		 bootbox.confirm("Do you Want to Approve ? ", function(result) {
			 	if(result)
				{
			 		var url="./patientDetailsNew.do?actionFlag=saveDTOdrugsNims&fromDisp=Y";
...
//			 else{
    ...
    deleteButName=parseInt(drugCount+1);
	 ...
	 document.getElementById(deleteButName).onclick = function(){
		 ...
		 drug=drug+document.getElementById("drugTypeCode").value+"~"+document.getElementById("drugSubTypeCode").value+"~"+document.getElementById("dosage").value+"~"+
...
function fn_saveDrgs(drgCount,arg)
{
	...
	for(var k=1;k<=drgCount;k++)
		...
		document.getElementById("prQuant"+k).value+"@";
		}
	//alert(drgs);
...
<tr>
<td  >
        <c:if test="${partialFlg ne 'Y'}">

<button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Save" onclick="javascript:fn_submitDrugs()">Approve&nbsp;</button>
</c:if>
    <c:if test="${partialFlg eq 'Y'}">

<button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Save" onclick="javascript:fn_submitDrugs()">Partial Approve&nbsp;</button>
</c:if>
<button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Reject" onclick="javascript:fn_rejectDrugs()">Reject&nbsp;</button>

<%-- <button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Submit"  onclick="javascript:fn_saveDrgs(<c:out value = "${drgCount}"/>,'Submit')">Approve&nbsp;</button>
 --%></td>
<%-- <td  >
...
<!-- <button class="btn btn-primary has-spinner" style="text-align:center" type="button" id="Back" onclick="javascript:fn_backDrugs()">Reject&nbsp;</button>
 --></td>
</tr>
</table>
</c:if>
