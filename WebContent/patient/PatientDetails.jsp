...
<%--<script type="text/javascript" src="js/patientscripts.js?dummy="></script> --%>
<script>document.write("<script type='text/javascript' src='js/patientscripts.js?v=" + Math.random() + "'><\/script>");</script> 
...
var heightVar = null;
var weightVar = null;
var tempList='${procList}';
$(document).ready(function()
		
		{
	
	if(document.forms[0].medicalSpclty!=null){
		document.forms[0].medicalSpclty.value='${patientForm.medicalSpclty}';
		document.forms[0].asriCatCode1.value='${patientForm.medicalSpclty}';

	}
	if(document.forms[0].medicalCat!=null){
		getMedicalCategoryList('${patientForm.medicalCat}');
		document.forms[0].medicalCat.value='${patientForm.medicalCat}';
		document.forms[0].ICDCatCode1.value='${patientForm.medicalCat}';

	}
	if(document.forms[0].ICDProcName1!=null){
		getTherapyICDProcedureListMed(1);
		document.forms[0].ICDProcName1.value='${patientForm.ICDProcName1}';
		document.forms[0].ICDProcCode1.value='${patientForm.ICDProcName1}';

	}
	if(document.getElementById("ICDProcName1")!=null){
...
if(schemeId=='CD202')
{
	...
	if( (document.forms[0].patientType[1].checked || document.forms[0].patientType[2].checked) || hospType!='G')
	{
$('.onlyAp').css('display','');
		}
...
function fn_saveDetails(){
	...
	*/
	...
	if(treatType=='M' && document.forms[0].patientType.value!=null && document.forms[0].patientType.value!='IPM'){
		alert("Please select a valid treatment type and proceed");
		return false;
	}
	if(treatType=='S' && document.forms[0].patientType.value!=null && document.forms[0].patientType.value=='IPM'){
		alert("Please select a valid treatment type and proceed");
		return false;
	}
...
function fn_savePatientDetails(checkType)
{
	...
	var ipTestsCount=0;
	var updTestsCount=0;
	var ipOtherRemark=document.getElementById("ipOtherRemarks").value;
		for(var temp=1;temp<document.forms[0].elements.length;temp++)
    {
       if(document.forms[0].elements[temp].type=="file")
       {
...
else
{
	...
	if( opIP == 'IPM')
	{
		...
		{
		if(lErrorMsg==''){
	        lErrorMsg=lErrorMsg+"Select Procedure ";
			}
	        if(lField=='')
	        lField='medicalCat'; 
		}
		...
		{
		if(lErrorMsg==''){
	        lErrorMsg=lErrorMsg+"Please Enter reason for selecting any other procedure ";
			}
	        if(lField=='')
	        lField='ipOtherRemarks'; 
		}
		 if(!(document.getElementById("critical").checked || document.getElementById("nonCritical").checked))
		{
		if(lErrorMsg==''){
	        lErrorMsg=lErrorMsg+"Please select critical condition ";
			}
	        if(lField=='')
	        lField='ipOtherRemarks'; 
		} 
...
if(opIP == 'IP')
{
	...
	{
		if(document.getElementById("robotSurg").value=="-1"  || document.getElementById("robotSurg").value=="")
		{
			if(lErrorMsg==''){
				lErrorMsg=lErrorMsg+"Please select Whether Robotic Surgery or Not";
				}
			if(lField=='')
			lField='roboticId'; 
		}
...
function resetMedicalDetails()
{
	...
	if(document.forms[0].patientType[1].checked==true || document.forms[0].patientType[2].checked == true)
	{
...
function fn_ipop()
{
	...
	if(treatType!=null && treatType=='B'){
		$(".medTD, .surgTD").css("display","");
		
	}
	else if(treatType!=null && treatType=='S'){
		$(".medTD").css("display","none");
		$(".surgTD").css("display","");
		document.getElementById("patientTypeM").disabled=true;
	}
	else if(treatType!=null && treatType=='M'){
		$(".medTD").css("display","");
		$(".surgTD").css("display","none");
		document.getElementById("patientTypeD").disabled=true;
	}
...
var opActiveMsg='${opActiveMsg}';
	...
	else  if(document.forms[0].patientType[1].checked || document.forms[0].patientType[2].checked)
	{
	...
	else
	{
		...
		if(caseStatus!=null && caseStatus!='' && caseStatus=='CD2')
		{
			if(pastHistory!=null && pastHistory!='' && pastHistory=='Y')
				url="preauthDetails.do?actionFlag=getOnlineCaseSheet&caseId="+caseId+"&patientId="+patientId+"&caseSheetFlag=Y&closeBut=Y";     
			else
				url="clinicalNotesAction.do?actionFlag=printDischargeForm&caseId="+caseId+"&decFlag=N";	
		}
		else
			url="patientDetails.do?actionFlag=casePrintForm&patientId="+patientId+"&caseId="+caseNo+"&decFlag=N";
...
if(chemicalCode=="-1")
{
	...
	url = "./patientDetailsNew.do?actionFlag=getOpDrugDtlsAuto&callType=Ajax&chemicalCode="+chemicalCode; 
	xmlhttp.open("Post",url,true);
	xmlhttp.send(null);
}
