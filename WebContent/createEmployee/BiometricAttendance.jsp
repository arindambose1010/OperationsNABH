...
<%-- <c:if test="${macId eq 'N' }">
<script>
alert('${sysMacId}');
alert("Unable to process your request as System MacId is not mapped with the Dispensary");
...
if(fingerPrint!=null && fingerPrint!='')
{
	...
	document.forms[0].action="bioMetricAction.do?actionFlag=disprecordAttendance&otherUserId="+otherUserId;
...
var otherUserId = document.getElementById("userId").value;
	...
	url = "bioMetricAction.do?actionFlag=dispvalidateAttendance&attendType="+attendType+"&hospId="+hospId+"&otherUser="+otherUserId;
	...
	var sysMacId='${sysMacId}';
	 ...
	 if(loginName==null ||loginName==""){
		 alert("Please enter any search criteria");
		    ...
		    document.forms[0].action='bioMetricAction.do?actionFlag=searchEmployee&bioAtt=Y&loginName='+loginName+'&sysMacId='+sysMacId;
		   	document.forms[0].submit();
	 }
...
function getOthersList()
	{
		...
		url = "bioMetricAction.do?actionFlag=getOthersList&hospId="+hospId+"&otherUser="+otherUser;
		xmlhttp.open("Post",url,true);
		xmlhttp.send(null);
...
var otherUser=document.forms[0].otherUser.value;
	...
	url = "bioMetricAction.do?actionFlag=checkOthers&hospId="+hospId+"&otherUser="+otherUser;
	xmlhttp.open("Post",url,true);
	xmlhttp.send(null);
	}
}
