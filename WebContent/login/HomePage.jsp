...
function fn_stkIndented()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getstkIndentedReport';
}
...
<div id="container" style="padding-top:0px;margin:0px auto;">
   ...
   <td class="menu">
            ...
            <div class="item">
                ...
                <div class="item_content">
                    <a href="redirectDefaultPage()" title="Home">Home</a>
                </div>
            </div> 	 
...
<c:forEach items="${ActionbarModulesLst}" varStatus="loop">			
			<li>
				<c:set var="tmpModVO" value="${ActionbarModulesLst[loop.index]}" />
								<c:out value="test${tmpModVO.MODNAME"/>
				<c:if test="${fn:length(tmpModVO.listSubMenu) gt 0}">
				  	<a> ${tmpModVO.MODNAME}</a>
				</c:if> <c:if test="${fn:length(tmpModVO.listSubMenu) eq 0}">
				<c:choose>
				<c:when test="${tmpModVO.MODNAME eq 'Low Stock Alert' && lowStockAlert eq 'Y'}">
				<li><a id="blink" href="${tmpModVO.MODURL}"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:otherwise>
		...
		<ul>
		    <li style="float:right;list-style-type:none;font-weight:bold;margin:7px 10px 0 0;"><a id="modules">Modules List</a>
