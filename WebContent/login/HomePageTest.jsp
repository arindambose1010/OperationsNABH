...
if(userNameId!=null && userNameId!='' && userNameId.indexOf('INCHARGE') > -1){
	document.getElementById('lowstockID').style.display='';
	document.getElementById('criticalstockID').style.display='';//Chandana - 9008 - Added this to show the critical indent drugs
}
if(userNameId!=null && userNameId!='' && userNameId=='STORE_KP'){
	document.getElementById('UpdateDrugScreen').style.display='none';
	document.getElementById('ExpiryofDrugs').style.display='none';
	document.getElementById('AddDrugs').style.display='';
}
});
...
{ 
	setInterval(function() {
		if(document.getElementById('blink1') !=null)
	 		document.getElementById('blink1').style.opacity = (document.getElementById('blink1').style.opacity == 0 ? 1 : 0);
		if(document.getElementById('blink2') !=null)
	 		document.getElementById('blink2').style.opacity = (document.getElementById('blink2').style.opacity == 0 ? 1 : 0);
	 }, 1000); 
  var gp = '${groupList}';
  gp=gp.trim();
  if(gp.indexOf('GP789~')>=0){
	  var newheight= $( document ).height();
		document.getElementById("middleFrame").style.height=(newheight-(120))+"px";
		fn_registerPatientDisp();
  
	} 
   if(gp.indexOf('GP813~')>=0)
  {
	 	var newheight= $( document ).height();
		document.getElementById("middleFrame").style.height=(newheight-(120))+"px";
		fn_viewRegisteredPatientsDisp();
  }
  else if(gp.indexOf('GP801~')>=0)
  {
	 	var newheight= $( document ).height();
		document.getElementById("middleFrame").style.height=(newheight-(120))+"px";
		fn_dispatchDrugsDisp();
  }
  /*  sowmya for off1_lhs and off2_lhs onload */
  else if(gp.indexOf('GP824~')>=0)
  {
	 	var newheight= $( document ).height();
		document.getElementById("middleFrame").style.height=(newheight-(120))+"px";
		fn_viewLHSRegistredMembers();
  }
  else if(gp.indexOf('GP825~')>=0)
  {
	 	var newheight= $( document ).height();
		document.getElementById("middleFrame").style.height=(newheight-(120))+"px";
		fn_approveLHSRegistredMembers();
  }
...
function fn_changePassword() {
	...
	document.getElementById("changePasswordDiv").src='loginAction.do?actionFlag=ChangePwd&username='+userName;
}
...
// nims ayush services
function fn_viewAppointmentsaAisRegNimsAyush()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=openPatRegForm&aisFlag=Y&fromDispnsry=Y&aisFlagAis=Y&aisDG=Y&nimsAyush=Y';
}

//added sowmya 08/07
function fn_viewAppointmentsLegReg()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=openPatRegForm&aisFlag=Y&fromDispnsry=Y&legCard=Y';
}
//Tejasri- CR-8812 Wellness Centers - Attendance dashboard to Wellness Centre Admin
function fn_WellnessCentreReport()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getWellnessCentreList';
}

//added sowmya 09/07 for LHS creation
function fn_viewAppointmentsCrtLHS()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewLegislativeReg';
}

//added sowmya 09/07 for LHS Checker- upload signed document
function fn_viewLHSRegistredMembers()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewLHSRegisteredMembers';
}

//added sowmya 04/08 for LHS Checker- second leven approve
function fn_approveLHSRegistredMembers()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewLHSRegisteredMembers&approveFlag=Y';
}

//to view registered members only
function fn_viewRegistredMembers()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewLHSRegisteredMembers&regFlag=Y';
}
...
function fn_getNextPatient(arg)
	{
		...
		if(xmlhttp.readyState==4)
		{
    	    ...
    	    resultArray = resultArray.replace("@]","");   */           
    	    var flag='${isReferredDoc}';
			if(resultArray!=null && flag=='Y')
				{
				resultArray = resultArray.replace("*","");
				if(resultArray!="")
					{
					document.getElementById("tokenNo").innerHTML="Token:"+resultArray; 
		    	    document.getElementById("tokenNo").style.color = "white";				
					}
				 <%--    document.forms[0].action="loginAction.do?actionFlag=reloadHomePage";
					document.forms[0].submit(); --%>
				}
		...
		if(arg=="INC"){
			var url =  "loginAction.do?actionFlag=incrementTokenNo&arg=inc";
			xmlhttp.open("Post",url,true);
			xmlhttp.send(null);
		}
		
		if(arg=="ONLOAD"){
			var url =  "loginAction.do?actionFlag=incrementTokenNo&arg=onload";
			xmlhttp.open("Post",url,true);
			xmlhttp.send(null);
		}
...
function fn_ahcClaimPayments(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='ahcClaimsAction.do?actionFlag=claimPayments&status=CD385';
	}
function fn_biomOtherVisitsReport()
	{
		fn_loadImage();
		document.getElementById("middleFrame").src='bioMetricAction.do?actionFlag=openVisitorsReport';
	}
...
function fn_editDrugsInventory()
{ 
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getDrugInvEditReport';
}
...
function fn_lowStockReport(arg){
	fn_loadImage();
	if(arg==1){
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockReport&indentedStock=Y&lowStock=Y';
	}else{
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockIndent&indentedStock=Y&lowStock=Y';
	}
}
//Chandana - 9008 - Below new function for Critical drug indent
function fn_critcalStockReport(arg){
	fn_loadImage();
	if(arg==1){
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockReport&indentedStock=Y&lowStockCrtitical=Y&lowStock=Y';
	}else{
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockIndent&indentedStock=Y&lowStockCrtitical=Y&lowStock=Y';
	}
}
//Chandana - 9008 - Below new function for Non critical drug indent
function fn_nonCritcalStockReport(arg){
	fn_loadImage();
	if(arg==1){
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockReport&indentedStock=Y&lowStockCrtitical=N&lowStock=Y';
	}else{
		document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewlowStockIndent&indentedStock=Y&lowStockCrtitical=N&lowStock=Y';
	}
}
...
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501');
  document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501'; 
...
function fn_viewTdInvstRegPtLst()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=ViewRegisteredPatients&fromDispnsry=Y&advSearch=false&tdLabflg=Y';
}
...
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=claims&pendingFlag=Y');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=claim&pendingFlag=Y';
...
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=preauth&pendingFlag=Y');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=preauth&pendingFlag=Y';
...
function fn_dentalPendingCases()
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=preauth&pendingFlag=Y');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&dentalFlag=Y&module=preauth&pendingFlag=Y';
}
function fn_followUpPendingCases()
{

	 fn_loadImage();
		document.getElementById("middleFrame").src='followUpAction.do?actionFlag=caseSearch&casesForApproval=Y&patientScheme=CD501&pendingFlag=Y';
}
...
function fn_chronicPaymentsSearch(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='ClaimsFlow.do?actionFlag=getChronicClaimsPaymentRecrds';
	}
function fn_enhancePreauthCaseNew(autoCaseId)
{
	fn_loadImage();
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseOpen&caseSearchType=caseIp&casesForApproval=Y&enhanceflag=Y&module=enhancepreauth&autoCaseId='+autoCaseId;
}
function fn_casesForApproval()
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=preauth');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=preauth';
...
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&casesForApproval=Y&module=enhancepreauth');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&casesForApproval=Y&enhanceflag=Y&module=enhancepreauth';
}
...
{
	fn_loadImage();
	setGlobalUrl('casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=claim');
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&module=claim';
	}
...
{
	fn_loadImage();
  document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseOpen&medFlg=Y&caseSearchType=caseIp&casesForApproval=Y&medFlag=O&module=claimMed&autoCaseId='+autoCaseId;
}
...
{
	fn_loadImage();
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&errClaimType=S&casesForApproval=Y&errSearchType=Y&module=claim';  
}
function fn_dentalErroneousCasesForApprovalClaim()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='casesSearchAction.do?actionFlag=OnloadCaseSearch&caseSearchType=caseIp&patientScheme=CD501&casesForApproval=Y&errDentalSearchType=Y&module=claim';  
}
...
function fn_caseForApprovalFollowUpNew(autoCaseId)
{
	...
	fn_loadImage();
	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=OnloadCaseOpen&casesForApproval=Y&module=followUp&patientScheme=CD501&autoCaseId='+autoCaseId;
}

function fn_followUpCaseSearch(){ 
	fn_loadImage();
	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=caseSearch&patientScheme=CD501';
	}
  
  function fn_followUpCaseSearchForApproval(){ 
	  fn_loadImage();
	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=caseSearch&casesForApproval=Y&patientScheme=CD501';
	}
  ...
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=OnloadCaseOpen&casesForApproval=Y&module=followUpJournal&patientScheme=CD502&autoCaseId='+autoCaseId;
  }

  function fn_JournalistFollowUpCaseSearch(){ 
  	fn_loadImage();
  	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=caseSearch&patientScheme=CD502';
  	}
    
    function fn_JournalistFollowUpCaseSearchForApproval(){ 
  	  fn_loadImage();
  	document.getElementById("middleFrame").src='followUpAction.do?actionFlag=caseSearch&casesForApproval=Y&patientScheme=CD502';
  	}
  ...
  function fn_RejClaimPayments(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='ClaimsFlow.do?actionFlag=getClaimsPaymentRecrds&caseStatus=CD98';
	}
  ...
  function fn_RejErrClaimPayments(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='ClaimsFlow.do?actionFlag=getErrPaymentRecrds&errStatusId=CD117';	
	}	
  ...
  function fn_RejFollowUpPayments(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='followUpAction.do?actionFlag=getFPPaymentRecrds&followUpStatus=CD108';
		}

	
  function fn_followUpApprovals(){
	  fn_loadImage();
		document.getElementById("middleFrame").src='followUpAction.do?actionFlag=getFPPaymentRecrds&userGrp=GP9&followUpStatus=CD65';
		}
  function fn_panelDocPayments()
	{
	  fn_loadImage();
		document.getElementById("middleFrame").src='panelDocPay.do?actionFlag=getCases';
	}
  function fn_panelDocPaymentsSearch()
	{
	  fn_loadImage();
	  document.getElementById("middleFrame").src='panelDocPay.do?actionFlag=panelDocPayHome&searchFlag=Y';
	}
  ...
  function fn_maWorklistByJeo(){
		fn_loadImage();
		document.getElementById("middleFrame").src='medicalAudit.do?actionFlag=getMAworklist&backFlag=no&cmaDeoFlag=N';
		
	}
  function fn_maWorklistByDeoToCma(){
		fn_loadImage();
		document.getElementById("middleFrame").src='medicalAudit.do?actionFlag=getMAworklist&backFlag=no&cmaDeoFlag=Y';
		
	}
  
  function fn_maWorklistByCma(){
		fn_loadImage();
		document.getElementById("middleFrame").src='medicalAudit.do?actionFlag=getMAworklist&backFlag=no&cmaDeoFlag=C';
  ...
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=openPatRegForm&aisFlag=N';
  }
  function fn_viewRegisteredPatients()
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=ViewRegisteredPatients&advSearch=false';
  }
  function fn_aisRegView()
  {
   ...
   function fn_dispatchDrugsDisp()
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=ViewRegisteredPatients&fromDispnsry=Y';
  } 
  ...
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=getOPCases&advSearch=false&patientScheme=CD501';
  }
  function fn_opRegPatientsViewJHS()
  {
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=getOPCases&advSearch=false&patientScheme=CD502';
  }

function fn_chronicOPPatientsView(){
	//document.getElementById("middleFrame").src='chronicAction.do?actionFlag=viewChronicOPPatients';
	setGlobalUrl('chronicAction.do?actionFlag=chronicOPCasesView&regCaseFlg=Y');
	document.getElementById("middleFrame").src='chronicAction.do?actionFlag=chronicOPCasesView&regCaseFlg=Y';
	fn_loadImage();
}

function fn_chronicOPCasesView(){
	fn_loadImage();
	setGlobalUrl('chronicAction.do?actionFlag=chronicOPCasesView');
	document.getElementById("middleFrame").src='chronicAction.do?actionFlag=chronicOPCasesView';
	
}
function fn_coClaimCases(){
	fn_loadImage();
	setGlobalUrl('chronicAction.do?actionFlag=chronicOPCasesView&caseApprovalFlag=Y');
	//document.getElementById("middleFrame").src='chronicAction.do?actionFlag=COClaimCases';
	document.getElementById("middleFrame").src='chronicAction.do?actionFlag=chronicOPCasesView&caseApprovalFlag=Y';
	
}

function fn_coClaimCasesPending(){
	fn_loadImage();
	setGlobalUrl('chronicAction.do?actionFlag=chronicOPCasesView&caseApprovalFlag=Y&pendingFlag=Y');
	//document.getElementById("middleFrame").src='chronicAction.do?actionFlag=COClaimCases';
	document.getElementById("middleFrame").src='chronicAction.do?actionFlag=chronicOPCasesView&caseApprovalFlag=Y&pendingFlag=Y';
...
function fn_chronicCasesForApprovalNew(autoCaseId)
{
	...
	document.getElementById("middleFrame").src='chronicAction.do?actionFlag=OnloadCaseOpen&caseSearchType=caseIp&casesForApproval=Y&module=preauth&autoCaseId='+autoCaseId;
}
...
function fn_ahcCasesSearch(){
  ...
  function fn_annualRegisteredPatientsView(){
	  fn_loadImage();
	  setGlobalUrl('annualCheckUpAction.do?actionFlag=getAnnualPatientsView');
	  document.getElementById("middleFrame").src='annualCheckUpAction.do?actionFlag=getAnnualPatientsView';
	
	  }
  function fn_telephonicEntry(){
  	fn_loadImage();
  	document.getElementById("middleFrame").src='patientDetails.do?actionFlag=telephonicEntry';
  }
  function fn_pnlDocBankDtls()
  {
fn_loadImage();
document.getElementById("middleFrame").src='panelDoc.do?actionFlag=panelDocSearch';
  } 
  function fn_telephonicIntimation(){
  	 fn_loadImage();
  	document.getElementById("middleFrame").src='telephonicAction.do?actionFlag=telephonicIntimationSearch';
  }
  function fn_telephonicIntimationRaised(){
  	fn_loadImage();
  	document.getElementById("middleFrame").src='telephonicAction.do?actionFlag=telephonicIntimationRaised';
  }
  function fn_viewFlaggedCases()
  {
	  fn_loadImage();
  	document.getElementById("middleFrame").src='flaggingAction.do?actionFlag=viewFlaggedCases&flaggedCasesForApproval=N&newFlag=N';  
  }
  function fn_flaggedCasesForApproval()
  {
	  fn_loadImage();
  	document.getElementById("middleFrame").src='flaggingAction.do?actionFlag=viewFlaggedCases&flaggedCasesForApproval=Y&newFlag=Y';  
  }
  ...
  function fn_openSchedulerLinks()
	  ...
	  function refresh()
	  {
		 // alert(document.URL);
		  ...
		  document.forms[0].action='loginAction.do?actionFlag=checkLogin&refreshFlag=Y';
...
{ 
	 fn_loadImage();
 	  document.getElementById("middleFrame").src='bioMetricAction.do?actionFlag=bioMetricAttendance&sysMacId='+isMacId;
}}

function fn_biometricReport()
{
	 fn_loadImage();
	  document.getElementById("middleFrame").src='bioMetricAction.do?actionFlag=bioMetricReport';
}
...
function fn_drugsInventoryDispNew()
{fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewDrugInventoryFormNew';  
}
...
function fn_getNewAttach()
{
	...
	var url='loginAction.do?actionFlag=viewAttachment&newAttchFlg=Y';
	document.getElementById("attachFrameNew").src=url;
	
}
function fn_drugsTransfer()
{fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewDrugTransferForm';  
}
function fn_drugsTransferredList()
{fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewDrugTransferredList';  
...
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getledBalReport';
}

function fn_lowStkUpdt()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getlowStkUpdt';
}
function fn_expDtRpt()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getexpDtRpt';
}
...
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getExpReport';
}
function fn_drugExpired()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getExpiredReport';
}
function fn_drugsReturnToVendor()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getDrugsExpiredForRTV&search=N';
}
function fn_CreditNoteRqst()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getDrugsInvoiceList&search=N';
}

function fn_returnVendor()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getReturnVendorReport';
}
function fn_creditNote()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getCreditNoteReport';
}
function fn_indentRpt()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getIndentReport';
}
function fn_indentRptNew()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getIndentReportNew';
}
function fn_drugsManfctNew()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewDrugManfctFormNew';  
}
function fn_drugsDistrNew()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=viewDrugDistrFormNew';  
}
function fn_patDrugs()
{
	fn_loadImage();
	document.getElementById("middleFrame").src='patientDetailsNew.do?actionFlag=getPatDrugReport';
}
//CR 8601 function for ADD Drugs for Vendor 
function fn_vendorDrugsInventory()
	{fn_loadImage();
  	document.getElementById("middleFrame").src='vendorActionNew.do?actionFlag=viewVendorDrugInventoryForm';  
}
function fn_storeDrugsInventoryDisp()
{fn_loadImage();
	document.getElementById("middleFrame").src='vendorActionNew.do?actionFlag=viewStoreKeeperDrugInventoryForm';  
}
function fn_uploadInvoiceDocument()
{fn_loadImage();
	document.getElementById("middleFrame").src='vendorActionNew.do?actionFlag=uploadInvoiceDocument';  
}

</script>
</head>
<body onload="javascript:redirectDefaultPage();javascript:fn_getNextPatient('ONLOAD')">
...
<div class="navbar navbar-default page-header">
	<div class="pull-left">
<c:if test="${dsgnName ne 'NIMS'}">
      <h1>Employees Health Scheme</h1>
      </c:if>
      	<c:if test="${dsgnName eq 'NIMS'}">
      <h1>Integrated Wellness Center-NIMS</h1>
      </c:if>
    </div>
    <div class="navbar-buttons pull-right" role="navigation">
    <ul class="nav custom-nav">
                       
	                       <c:if test="${isMedco ne 'Y' && dsgnName ne 'NIMS'}">
	                       <li >
							...
							<li class="dropdown">
								...
								<ul class="dropdown-menu-right dropdown-navbar dropdown-menu dropdown-caret dropdown-close" id="dashboardList">
									...
									<li >
									 ...
									 <c:if test="${isReferredDoc eq 'Y' }">
									 <c:if test="${grp ne 'GP797' }">
										<li >
											<a href="#nextPatient" onclick="javascript:fn_getNextPatient('INC')" data-toggle="modal" title="Next Patient">									
													Next Patient
											</a>
									</li>
									</c:if>
									<li>
									<span id="tokenNo" style="font-color:#fff"> </span> 
									</li>
									</c:if>
									<c:if test="${dsgnName ne 'NIMS'}">
					<li class="dropdown">
    ...
    <div class="collapse navbar-collapse" id="navbarCollapse">
      <ul class="nav navbar-nav">
		...
		<c:forEach items="${ActionbarModulesLst}" varStatus="loop">	
				...
				<c:if test="${fn:length(tmpModVO.listSubMenu) eq 0}">				
				<li>
				<c:choose>
				<c:when test="${fn:contains(tmpModVO.MODNAME,'Low Stock Alert') && (lowStockAlert eq 'Y' || lowStockAlert eq 'N')||fn:contains(tmpModVO.MODNAME,'Stock Indent Approval')}">
				<li><a class="blinkred" id="blink1" href="${tmpModVO.MODURL}"  data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:when test="${fn:contains(tmpModVO.MODNAME,'Low Stock-Indent') && (lowStockAlert eq 'NY' || lowStockAlert eq 'YY')}">
				<li style="display:none" id="lowstockID"><a class="blinkred" id="blink2" href="${tmpModVO.MODURL}"  data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:when test="${fn:contains(tmpModVO.MODNAME,'Non Critical Drugs Indent')}">
				<li id="nonCriticalstockID"><a id="blink2" style="color:red;font-weight:bold;animation: blink 1.5s infinite;" href="${tmpModVO.MODURL}"  data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:when test="${fn:contains(tmpModVO.MODNAME,'Critical Drugs Indent')}">
				<li style="display:none" id="criticalstockID"><a style="color:red;font-weight:bold;animation: blink 1.5s infinite;" href="${tmpModVO.MODURL}"  data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:when test="${fn:contains(tmpModVO.MODNAME,'Expiry of Drugs in 120 Days')}">
				<li id="ExpiryofDrugs" style="display:none"><a class="blinkred" id="blink2" href="${tmpModVO.MODURL}"  data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:when>
				<c:otherwise><a href="${tmpModVO.MODURL}" data-toggle="collapse" data-target="#navbarCollapse"> ${tmpModVO.MODNAME}</a>
				</c:otherwise>
				</c:choose>
				</c:if>
				...
				<c:if test="${fn:length(lSubMenuList) gt 0}">
					<ul class="dropdown-menu multi-level" role="menu">
						<c:forEach items="${lSubMenuList}" varStatus="status">
							...
							<c:choose>
							<c:when test="${fn:contains(tmpSubModVO.MODNAME,'Drugs Updating Screen')}">
							<li id="UpdateDrugScreen" style="display:none" class="dd"><a href="${tmpSubModVO.MODURL}" > ${tmpSubModVO.MODNAME} </a>				
							</c:when>
							<c:when test="${fn:contains(tmpSubModVO.MODNAME,'Add Drugs')}">
							<li id="AddDrugs" style="display:" class="dd"><a href="${tmpSubModVO.MODURL}" > ${tmpSubModVO.MODNAME} </a>				
							</c:when>
							<c:otherwise>
    ...
    <div class="modal-content" >
      <div class="modal-header">
		...
		<tr id="clinic-dialog2" style="display:none;">
			<c:choose>
			<c:when test="${fn:length(expireCasesList) gt 0}">
				<td><b><font size="3" color="#ff6666">Following cases are going to be auto cancelled please update the status</font>:</b></td>
			</c:when>
			<c:otherwise>
		...
		<tr id="clinic-dialog3" style="display:none;">
			<td style="color:black;" >
				<table class="table table-bordered" style="width:99%"    >
					 <c:forEach  items="${expireCasesList}" var="caseslist" varStatus="x">
...
<p align="left">
 ...
 <button type="button" class="btn btn-warning" data-dismiss="modal" id="closeBut" onclick="fn_enableClose();fn_getNewAttach();fn_diableClose1();">Close</button>
 </div>
 </div></div></div>
...
<span > </span><label>CIRCULAR ON MEDICAL MANAGMENET CASES BEYOND RS.50,0000 FOR NON CRITICAL CASES AND RS.1,00,000 FOR CRITICAL CASES BY NABH SUPER SPECIALITY HOSPITALS UNDER EJHS</label>
 ...
 <button type="button" class="btn btn-warning" data-dismiss="modal" id="closeBut1" onclick="fn_enableClose1();fn_loadClinincalNotes();">Close</button>
...
<div>
  <iframe id="middleFrame" class="embed-responsive-item" height="100%" width="100%" scrolling="yes" frameborder="0" onload="javascript:fn_removeLoadingImage();fn_diableClose();"></iframe>
</div>
<footer>
...
{
	var url="clinicalNotesAction.do?actionFlag=clinicalNotes&caseId="+caseId+"&caseApprovalFlag=Y";
	window.open(url,"_blank",'width=900, height=600, top=60,left=70,resizable=yes,titlebar=yes,status=yes,scrollbars=yes');
}
...
{	
	...
	if(parent.parent.globalURl==''){
		 caseSearchURl="casesSearchAction.do?actionFlag=OnloadCaseSearch&searchFlag=Y&module="+module+"&casesForApproval=Y";
	...
	else
	{
	 ...
	 url = 'casesSearchAction.do?actionFlag=setViewFlag&CaseId='+caseId+'&flag=Y';
	 xmlhttp.onreadystatechange=function()
		{
		    if(xmlhttp.readyState==4)
		    {

		    	var url="casesApprovalAction.do?actionFlag=getCaseDtls&CaseId="+caseId+"&status="+arg+"&casesForApproval=Y&ipOpType="+ipOpType+"&module="+module;
			  	window.open(url,"_blank",'width=900, height=600, top=60,left=70,resizable=yes,titlebar=yes,status=yes,scrollbars=yes');
...
{
	var url='loginAction.do?actionFlag=viewAttachment&newAttchFlg=N';
...
if('${dsgnName}'!=null && '${dsgnName}'!="" && '${dsgnName}'=="MEDCO"){
	...
	if(document.getElementById("medcoNotice")!=null)
	document.getElementById("medcoNotice").style.display="";
	if(document.getElementById("clinic-dialog")!=null)
	document.getElementById("clinic-dialog").style.display="none";
	if(document.getElementById("clinic-dialog1")!=null)
