...
{ 
    ...
    if(checkSimilarUpload(attchcnt,type,'CaseAttachments') == 1 )
    {
        ...
        {
       		 document.FileAttachment.action="attachmentAction.do?actionFlag=upldEnrollAttach&actionVal=caseAttachments&openWin=${winOpen}&chronicId=${chronicId}&UpdType=${UpdType}&newBornBaby="+newBornBaby;
        }
        else
        {
        	document.FileAttachment.action="attachmentAction.do?actionFlag=upldEnrollAttach&actionVal=caseAttachments&cochRem=${cochRem}&openWin=${winOpen}&chronicId=${chronicId}&caseId=${caseId}&UpdType=${UpdType}&newBornBaby="+newBornBaby+"&cochCount="+cochCount+"&drugsBreakFlag="+drugbrFlag;
        }
...
function fn_deleteAttachmentsConfirm(result)
{
	...
	if('${UpdType}'!=null && ('${UpdType}'=='ehfFollowUp' || '${UpdType}'=='ehfCochlearFollowUp'))
		document.FileAttachment.action="attachmentAction.do?actionFlag=removefollowUpAttachments&cochRem=${cochRem}&openWin=${winOpen}&UpdType=${UpdType}&newBornbaby="+newBornbaby+"&cochCount="+cochCount;

		else if(('${UpdType}'!=null && '${UpdType}'=='chronicAttach'))
				document.FileAttachment.action="attachmentAction.do?actionFlag=removechronicAttachments&cochRem=${cochRem}&openWin=${winOpen}&UpdType=${UpdType}&chronicId=${chronicId}&newBornbaby="+newBornbaby;

		else
	    document.FileAttachment.action="attachmentAction.do?actionFlag=removePreauthAttachments&cochRem=${cochRem}&openWin=${winOpen}&UpdType=${UpdType}&newBornbaby="+newBornbaby;
...
<c:set var="attachmentDocType" value="${attachmentDocType}~${AddattMap.docType}" />
  ...
  &nbsp;<input type="file" name='attachedIndex[${attachCount}]' id='${AddattMap.docType}1' class="FieldBlack"    > </input>
						...
						<c:if test="${AddattMap.docType != 'patientPhoto'}">			
...
<font size="2"><a href="javascript:" onclick="javascript:this.style.color = 'brown';fn_caseSheet('${subAttMap.docType}');fn_openAtachment('${subAttMap.savedName}');return false;">${subAttMap.fileName}&nbsp;&nbsp;${subAttMap.fileCrtTime}</a></font>
 </c:when> 
<c:otherwise> 
...
<font size="2"><a href="javascript:" onclick="javascript:this.style.color = 'brown';fn_openAtachment('${subAttMap.savedName}');return false;">${subAttMap.fileName}&nbsp;&nbsp;${subAttMap.fileCrtTime}</a></font>
 </c:when> 
<c:otherwise> 
