...
{ 
    ...
    if(checkSimilarUpload(attchcnt,type,'CaseAttachments') == 1 )
    {
        ...
        {
       		 document.FileAttachment.action="attachmentAction.do?actionFlag=upldEnrollAttach&actionVal=caseAttachments&openWin=${winOpen}&chronicId=${chronicId}&UpdType=${UpdType}";
        }
        else
        {
        	document.FileAttachment.action="attachmentAction.do?actionFlag=upldEnrollAttach&actionVal=caseAttachments&openWin=${winOpen}&chronicId=${chronicId}&caseId=${caseId}&UpdType=${UpdType}";
        }
...
function fn_deleteAttachmentsConfirm(result)
{
	...
	if('${UpdType}'!=null && '${UpdType}'=='ehfFollowUp')
		document.FileAttachment.action="attachmentAction.do?actionFlag=removefollowUpAttachments&openWin=${winOpen}&UpdType=${UpdType}";

		else if(('${UpdType}'!=null && '${UpdType}'=='chronicAttach'))
				document.FileAttachment.action="attachmentAction.do?actionFlag=removechronicAttachments&openWin=${winOpen}&UpdType=${UpdType}&chronicId=${chronicId}";

		else
	    document.FileAttachment.action="attachmentAction.do?actionFlag=removePreauthAttachments&openWin=${winOpen}&UpdType=${UpdType}";
...
function fn_getPatDetails(){
	...
	var url="chronicAction.do?actionFlag=casePrintForm&chronicId="+chronicId+"&type=patView";
...
<font size="2"><a href="javascript:this.style.color = 'brown';fn_openAtachment('${subAttMap.savedName}');return false;">${subAttMap.fileName}&nbsp;&nbsp;${subAttMap.fileCrtTime}</a></font>
 </c:when> 
<c:otherwise> 
